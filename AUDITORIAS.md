...
| AUD-DEPS | Dependências e supply-chain | Pins imutáveis no `FetchContent` (SDL3, RmlUi por SHA, glintfx por tag), libs vendorizadas em `third_party/` com licença registrada; CVE das deps; licenças compatíveis com Apache-2.0 | build reprodutível | `trivy fs`, `syft`, `scancode-toolkit`, revisão de advisories |
