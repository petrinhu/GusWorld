...
| glintfx | MPL-2.0 | petrinhu (<https://github.com/petrinhu/glintfx>) | Motor de UI/HUD em embed mode (`glintfx::UiLayer`), ADR-010. Via FetchContent, pin `v0.27.0` em `GusEngine/CMakeLists.txt` (bump BUMP-GLINTFX-V027, 2026-07-30; conferido contra a árvore baixada e contra o `config.hpp` gerado) (`GLINTFX_BACKEND_GLFW=OFF`, embed-only), linkado SO em `app/` (GATE de 4 camadas: nunca `core/`/`domain/`). MPL-2.0 é compatível com a Apache License 2.0 do jogo (MPL 3.3, cláusula "Larger Work": arquivos sob MPL continuam MPL, o resto do binário pode ser Apache-2.0). |
