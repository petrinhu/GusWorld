...
| glintfx | MPL-2.0 | petrinhu (<https://github.com/petrinhu/glintfx>) | Motor de UI/HUD em embed mode (`glintfx::UiLayer`), ADR-010. Via FetchContent, pin `v0.27.0` em `GusEngine/CMakeLists.txt` (bump BUMP-GLINTFX-V027, 2026-07-30; conferido contra a árvore baixada e contra o `config.hpp` gerado) (`GLINTFX_BACKEND_GLFW=OFF`, embed-only), linkado SO em `app/` (GATE de 4 camadas: nunca `core/`/`domain/`). MPL-2.0 é compatível com o GPLv3 do jogo (MPL 3.3, cláusula "Larger Work": arquivos sob MPL continuam MPL, o resto do binário pode ser GPLv3). |
...
**Precisão de inventário sobre o `stb` (2026-07-28, atualizado no bump v0.27.0 em 2026-07-30): o binário carrega DUAS cópias dele.** A linha acima descreve a nossa, em `GusEngine/third_party/stb/`, hoje com **dois** single-headers: `stb_image.h` (decode, camada de render2d) e `stb_image_write.h` (encode de PNG para screenshot/probe). O glintfx vendoriza as SUAS PRÓPRIAS cópias dos MESMOS dois arquivos e compila as duas implementações — `stb_image_impl.cpp` (desde sempre, ver `glintfx/src/render_gl3.cpp:25-26`) e **`stb_image_write_impl.cpp` (novo na v0.27.0, IMG-ENCODE)** —, ambos dentro da OBJECT library `glintfx_image`, que **não tem flag de desligar**: as duas entram no binário distribuído sempre. Mesma biblioteca, mesmo autor, mesma licença e — conferido byte-a-byte neste bump — **exatamente a mesma versão** (`stb_image_write` v1.16 nas duas cópias), logo não há obrigação adicional nem risco de divergência de comportamento; fica registrado para o inventário bater com o que o binário realmente contém. Nota de link, medida por `nm` no executável final: só **uma** definição de `stbi_write_png` sobra no binário (o objeto do lado que o linker resolve primeiro satisfaz o símbolo e o membro de arquivo do outro lado nunca é puxado), então a dupla vendorização não gera símbolo duplicado nem inflação do binário pelo dobro do stb.
