...
| M8 Decommission | ✅ | **FECHADO 2026-07-22.** Godot + C# + addons apagados; repo compila e roda sem nenhum bit do stack antigo. Faseado em 3 fases + 1 correção: F1 moveu dados VIVOS de `game/` pra `resources/` (`resources/translations/`, `resources/dialogues/`); F2 removeu o submódulo `engine/` (C# legado, repo próprio) + `.gitmodules` + `global.json`; F3 apagou `game/` morto (172 arquivos, 20816 linhas); correção fez a limpeza física (lixeira, 30M). Tag `pre-m8-godot-legacy` preserva o legado pra recuperação. Gate de build Windows já PRÉ-CUMPRIDO (CI Windows real MSVC verde desde 2026-07-14, e verde de novo na F1 do M8). Desfecho do repo `petrinhu/gus_dragon-engine` (fora deste repo): foi **apagado** do Codeberg (não arquivado, como se havia planejado); a tag `pre-m8-godot-legacy` preserva só o ponteiro do submódulo, não os arquivos; o código C# original não é mais recuperável. Sem perda funcional: a porta pro C++ já estava completa e testada desde o M3 e o M5 |
| M9 Higienização | ✅ | **EXECUTADO 2026-07-22** (`f87cad4`, CI Windows verde): purga de 31 libs vendorizadas mortas (só `monocypher`+`stb` sobrevivem), 9 `.gitkeep` obsoletos, `icon.svg` resíduo Godot, docs vivos que ainda ensinavam Qt6 corrigidos, `.gitignore` sem a seção Godot morta, `reorder_actor` privatizada. **Bug achado de brinde: o gate de i18n estava CEGO desde o M8** (o hook vigiava `game/translations/`, caminho que sumiu na F1). 600.565 linhas a menos. Verificado por `qa-engineer` independente: build LIMPO do zero (1044 alvos), 2394/2394, ASan/UBSan nas 4 suítes. **Auditoria independente de 2026-07-28: PARCIAL.** O código passou, mas a purga não foi refletida na documentação de dependências (`ACKNOWLEDGMENTS.md` ainda creditava 5 libs apagadas como vendorizadas, incluindo o miniaudio, que hoje vem do `glintfx::Audio`). Achado corrigido em 2026-07-28. **Aguarda aval do líder** **FECHADO 2026-07-28:** auditado de forma independente e reconferido em re-auditoria depois da correção do único achado (o `ACKNOWLEDGMENTS.md` creditava como vendorizadas 5 libs que o próprio M9 apagou). Aval do líder dado. **Com ele, o board M0-M9 da migração da engine fecha inteiro.** |

Ordem real de execução: M0, depois M1 e M3 em paralelo, depois M2, M4, M5, M6, M7, M8, M9. **M0-M9 ENTREGUES (board fechado 2026-07-28) — M7 (paridade) FECHADO 2026-07-21 pelo playthrough ao vivo (Gus); M8 (decommission) FECHADO 2026-07-22. O M9 foi executado em 2026-07-22 e está EM VERIFICAÇÃO (🔍), aguardando aval do líder: a auditoria independente de 2026-07-28 o deixou PARCIAL por um achado de documentação, já corrigido.**
...
Depois que o slice provar o jogo, a engine atingir paridade jogável (M7, fechado) e o decommission (M8, fechado) completar com a higienização (M9, executado em 2026-07-22 e em verificação, aguardando aval do líder), o foco vira conteúdo e release. Mapeamento de marco para versão em [CHANGELOG.md](CHANGELOG.md) (seção "Marcos planejados").
...
- **Wiki + doc para iniciante:** uma wiki inicial ("starter") foi publicada em 2026-07-14, em duas edições que viviam em hosts diferentes (a técnica em inglês no Codeberg, a de leigo/iniciante em pt-br no GitHub). Com a migração de 2026-07-25, as duas passaram a conviver na **wiki única do GitHub**: as 4 páginas técnicas em inglês foram migradas (`Home-English`, `Building-and-Running`, `Tech-Stack-and-Credits`, `Contributing`) e coexistem com as 6 páginas em pt-br, separadas por seção no `_Sidebar`. A versão completa e ainda mais didática, cobrindo tudo em detalhe (derivada de `docs/`, não duplica), fica para depois da tag de versão.
...
**Atualizado 2026-07-22 (M8 FECHADO — decommission Godot/C# concluído; M7 fechado 2026-07-21 pelo playthrough ao vivo; mirror+Wiki publicados; ondas `CARDS`/`CARDS-HW` em curso; sincronizado com [TODO.md](TODO.md)).** M0-M8 entregues; o **M9 (higienização) foi executado em 2026-07-22 e está em verificação** (🔍, aguardando aval do líder; a auditoria independente de 2026-07-28 o deixou PARCIAL por um achado de documentação, já corrigido). É o último item do board da migração. Em paralelo ao board, a onda `CARDS` (motor de cartas techMagic) avança um `EffectKind` por vez.

| Now (CONCLUÍDO) | Next (próxima onda) | Later |
|---|---|---|
| **M0-M8 motor+plataforma portados** (save/i18n/progression/templates/combat/crypto/áudio) em C++20+SDL3; M3, M5-DMG, M6, **M7 (paridade jogável)** validados ao vivo e **M8 (decommission)** fechado | **M8 FECHADO 2026-07-22** (Godot/C#/addons apagados, repo sem nenhum bit do stack antigo, tag `pre-m8-godot-legacy` preserva o legado); 3 feedbacks de polish na INBOX (M7-FB1/2/3) | **M9 Higienização**: executado 2026-07-22, 🔍 em verificação (aguarda aval do líder); último item do board M0-M9 |
...
| **Wiki bilíngue (PT-br + EN) + AI-DISCLOSURE.md** publicados; **GitHub é o host único** desde 2026-07-25 (o mirror de 2026-07-14 virou o repositório canônico e a wiki EN veio junto); CI Windows real (MSVC) verde | **Estética "terminal"** para logs de combate e telas de sistema (design fechado, implementação em curso) | Sistema de deck/mão (brainstorm inicial, ainda sem decisão fechada) |
