...
- **Licença do código rotacionada de GPLv3 para Apache License 2.0** (2026-07-31, item `LIC-APACHE` do `TODO.md`; ADR de superação em preparação, ver `docs/tech/adr/ADR-005-license-gpl3-assets-ccbysa.md`, que fica como registro histórico da decisão original com nota de superação). Motivo resumido: adoção por terceiros sem exigência de copyleft e sem CLA (seção 5, Submission of Contributions), viabilidade em lojas como Steamworks, e a concessão explícita de patente da seção 3 (Grant of Patent License) da Apache 2.0. Assets seguem **CC-BY-SA 4.0** e os livros Vol1/Vol2 seguem em **direitos reservados**; nenhum dos dois entra nesta rotação (decisão independente do líder). `LICENSE` e `NOTICE` atualizados; ver `THIRD-PARTY-LICENSES.md` para a atribuição de terceiros sob a licença nova.
