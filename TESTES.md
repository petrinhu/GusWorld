...
**Ferramenta:** Google Test.

**Critério de aprovação:** 0 falhas.
...
2. **Container-level (lento, idêntico).** Rodar a imagem Docker que o CI usa via `act` (GitHub Actions) ou `docker run` direto. Necessário quando se suspeita de deps de SO (ex.: bindings gráficos + libGL).
