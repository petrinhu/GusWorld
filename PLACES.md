...
| **Pântano da Atelaiá** (designação descritiva de trabalho; nome próprio pendente de decisão do líder, `TODO.md` item `G30`) | Sub-local (Selve Sombria); região aberta atravessável com encontros, **NÃO é dungeon** (distinto da dungeon nº9, tipo mista, já existente na Selve Sombria, que continua intacta) | 3 (dano de -3, manifestação atual) | **Em volta da Atelaiá** (catedral menor, subroutine de irrigação, saqueada por Sterling em -3, ver linha Atelaiá acima), borda Norte da Selve Sombria. Cadeia causal canônica: o saque quebrou a subroutine de irrigação, a água nunca mais parou, o terreno afogou, e o ciclo de decomposição da Selve travou no mesmo loop. Sem relação com a Catedral-Mãe (Selve profunda, lacrada -750): hipótese considerada e descartada | ✅ canônico (sub-local, decisão do líder 31/08/2026, por `AskUserQuestion`) | `docs/narrative/environments/02-selve-sombria.md` §11 + `docs/design/brainstorm-backlog.md` §11 + `docs/design/pillars.md` Pillar 2 |
