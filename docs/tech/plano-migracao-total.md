...
C5 CUTOVER (F4-3/F4-4) .. BLOQUEADO por FRAMEGRAB-TEX (bloqueante declarado por eles)
                          ...
                          SDL app 24 arq + SDL platform 6 arq + o glad VENDOR (gl3_loader +
                          RmlUi_Include_GL3.h + dlfcn/windows.h; sobra vocabulário mínimo
                          ~7 protótipos por contrato do host, resposta A3) + GL cru do gl3
                          (94) + stb_truetype/font_atlas + render2d_sdl/render2d_gl3 +
                          FetchContent SDL3
```
...
**A2 - Três contornos de BUG DE LAYOUT do RmlUi em `system_menu_rml.cpp`. Classe (b) - encaminhar.** A família que "ninguém chamou de contorno" porque parece CSS de design, e é compensação (o perfil exato do `cell_px=16`):
  ...
  - **O pedido (b), um só, em lote: ENVIADO em 2026-07-30** (`9a95e431` no bus), com os três casos, os pointers de código e o custo declarado: duas larguras absolutas acopladas (546dp/558dp) que quebram em silêncio se o painel mudar de tamanho, e um cabeçalho que precisa imitar o corredor da lista para as colunas alinharem. Não propomos conserto nosso; a decisão patch-local vs upstream é deles.
  - ⚠️ **A suspeita do report perdido CONFIRMOU:** o orquestrador conferiu o bus inteiro, inclusive o arquivo morto - **zero menções** ao caso do scroll. O líder relatou olhando a tela, alguém anotou "bug reportado ao vivo pelo líder" no comentário de código, e o sinal morreu ali: contorno aplicado, custo pago, fornecedor nunca soube. Encaminhado no lote como **falha nossa** (sem data inventada - não se sabe de quando era o relato). É a segunda violação retroativa da lei encontrada em 24h (a primeira foi o S2-captura classificado errado), e as duas apareceram porque alguém foi PROCURAR, não porque doeram - o argumento definitivo de que censo de workaround é atividade recorrente, não evento.

**A3 - Os 7 GL cru do begin-frame embed** (`render2d_glintfx.cpp:119-125`; era a seção 4.4). **Classe (b) formal - ENVIADA e RESPONDIDA em 2026-07-30: "contrato do host".** O estado GL de entrada do embed é responsabilidade nossa; o glintfx não vai expor prepara-alvo/clear. **Consequência estrutural, agora fechada:** o glad NÃO morre por completo na F4-4 - o `RmlUi_Include_GL3.h` (o vendor de 1450 linhas) morre, mas sobra um **vocabulário GL mínimo PERMANENTE do host**, encolhendo de ~30 protótipos para **perto de 7** (os do begin-frame). Isso é contrato declarado pelo fornecedor, não contorno: entra na tabela de exceções nomeadas do `GATE(framework-only)` (plano-fim §7) como "vocabulário GL do contrato de host, só em `platform/`, teto de símbolos travado" quando a fatia do cutover desenhar o header mínimo.
