...
> **STATUS: SUPERADO (2026-06-23).** Este documento descreve a arquitetura do stack **Godot 4 + C# .NET 8 AOT** (5 camadas, AutoLoad, signals, Resources, save JSON), **aposentado** pelo pivot para engine própria C++/Qt6 (engine-design.md, 2026-06-21) e depois pelo [ADR-008](adr/ADR-008-repivot-qt-to-sdl3.md) (plataforma SDL3 + RmlUi + miniaudio, 2026-06-22). **NÃO é mais a arquitetura canônica.** A fonte canônica atual é [`docs/tech/pivot/engine-design.md`](pivot/engine-design.md) (4 camadas C++20: `core/`+`domain/` POCO puro, `platform/` SDL3, `app/`). O decommission do stack Godot/C# fechou no marco **M8 (2026-07-22)**; este arquivo fica como **registro histórico PERMANENTE** (não editar como se fosse vigente, não mover nem apagar; preserva o link do "cemitério de ideias mortas" do projeto).
>
