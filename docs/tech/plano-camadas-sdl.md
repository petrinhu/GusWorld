...
⚠️ Divergência com o número "36" do brief: minha medição própria (instrução explícita: "refaça a medição você mesmo") deu **32 arquivos de produção**. A diferença provavelmente é escopo (a medição original pode ter incluído 4 arquivos de `tools/` que fazem parte de fluxo já commitado) ou deriva da árvore (17 commits tocaram `app/` hoje até agora, incluindo os de outros agentes ativos nesta sessão). **O número que uso daqui pra frente é 32 (produção real, shippada)** — é o que importa para o binário do jogo; `tests/` e `tools/` são discutidos à parte na seção 6 (exceções).
...
- **`app/tools/` (14 arquivos, 453 ocorrências): exceção legítima por construção, não por complacência.** Não é `add_subdirectory` do build principal (`app/CMakeLists.txt` só inclui `tests`), tem `CMakeLists.txt` próprio que importa os `.a` já compilados do preset `linux-release` — são probes de verificação visual/diagnóstico, descartáveis, fora do binário shippado. **Não bloqueiam nenhum gate de `app/` de produção.** Se um gate futuro varrer o diretório `app/` inteiro por engano, precisa excluir `app/tools/` explicitamente (não por regra geral "tools é sempre ok", mas porque **este** `tools/` específico está provado fora do link do jogo).
...
**Proposta: dois gates, escopos diferentes, entrando em fatias diferentes.**
