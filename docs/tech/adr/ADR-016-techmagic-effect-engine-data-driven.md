...
O item TECHMAGIC-EXECUTOR (`techmagic.md`) nomeou um fork: (a) resolvedor data-driven vs (b) interpretador/VM de conjuro (bytecode). O contexto de valor e o subtexto canonico "magia = software" (o executor techMagic ↔ Tavus-Drive do Gus).
...
> **NOTA DE ESCOPO (25/08/2026).** A premissa "sem craft/modding/UGC em G1" acima ficou desatualizada pela revogação do `C-03` (`GODS_LAWS.md` L-29, 25/08/2026): o jogo hoje TEM crafting, inclusive de cartas (comuns craftaveis; especiais/super tem copia pirata craftavel, sempre inferior a original, ver `docs/design/gdd.md` "Crafting de cartas"). O vocabulario de efeitos continua fechado, porque crafting usa receitas e cartas pre-definidas, nao gera efeito novo e nao e UGC, entao o argumento central (nao existe caso de uso que justifique interpretador) nao foi contestado por este achado. Mas a premissa como estava escrita nao e mais verdadeira, e a reavaliacao da decisao, se necessaria, e do lider/arquitetura, nao deste agente (L-14).

**O subtexto fica LITERAL no codigo hoje, risco zero:** o executor chama-se `techMagic`, a carta E um programa declarativo (lista ordenada de instrucoes que o Tavus-Drive "executa"). **Porta aberta pra VM futura sem reescrita:** a lista `EffectSpec` e exatamente a IR que um compilador de conjuros emitiria; se v2 tiver consumidor real (editor de conjuros, modding), a VM nasce POR CIMA, sem tocar as cartas nem o resolvedor.
...
**Regra de VFX (ordem do lider 2026-07-14):** todo efeito VISUAL que o glintfx possa servir (screen-wave do Einstein, glow de carta, sprite-eco do clone) vai como PROMPT pro dev do glintfx, NUNCA hardcoded no nosso GL/render (`feedback_glintfx_nao_mexer_so_pedir`). O dominio/gameplay dispara o efeito; a apresentacao e da lib.
...
1. Grafia canonica do simbolo: `techMagic` vs `TechnoMagik` (item TECHMAGIC-CANON) - o batismo do subtexto e do lider.
...
- **Positivas:** custo BAIXO-MEDIO, risco BAIXO; numeros 100% tunaveis pra playtest; testavel por unit test por EffectKind; subtexto entregue hoje; nao fecha porta pra VM. Cada carta = 5-15 linhas de dado.
