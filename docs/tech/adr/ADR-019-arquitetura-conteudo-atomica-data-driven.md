...
2. **`-Werror=switch` é a rede de exaustividade.** Instalada como fix sistêmico na onda `HUD-STATUS-ICONS-STALE` (2026-07-18, commit `620b304`) — antes disso o warning de `switch` não exaustivo só existia no `__DEP_REMOVIDA__` vendorizado, não nos alvos do projeto. Todo `switch` sobre um enum de conteúdo agora FALHA a compilação se um caso novo não for tratado.
...
O princípio não é exclusivo de domínio/lógica — vale para UI. Telas devem ser **composição de módulos atômicos** (modelo de dados + layout __DEP_REMOVIDA__ + input + loop de render), cada módulo com seu próprio self-test, em vez de um host monolítico que acumula responsabilidades a cada feature nova. O cockpit de batalha já é dirigido por data-model (`battle_hud_model`) nesse espírito.
