...
**Cross-ref:** [ADR-006](ADR-006-crypto-hmac-formato-domain.md) (crypto SHA-256/HMAC própria, reusada pelo compilador de conteúdo), [ADR-007](ADR-007-controls-json-hash128-save-v4.md) (fork `load_save`/`deserialize_save`, decisão herdada não reaberta), [ADR-008](ADR-008-repivot-qt-to-__DEP_REMOVIDA__.md) (__DEP_REMOVIDA__), glintfx/UI, [ADR-011](ADR-011-m6-audio-onda1-plano.md) (áudio onda 1, dívida da `battle_preview` que este ADR paga), `docs/design/narrativa/dialogue-tree-npc-intro.md` (blueprint do diálogo), memória `project_i18n_canonico` (i18n-ready desde o dia 1), memória `reference_formato_mapa_gmap` (padrão fonte→compilador→artefato selado, reusado aqui)
