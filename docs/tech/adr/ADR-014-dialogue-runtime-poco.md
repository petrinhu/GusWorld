...
Namespace `gus::domain::dialogue`. Camada `domain/` PURA (ZERO Qt, ZERO __DEP_REMOVIDA__, ZERO I/O — mesma invariante de `save/`, `map/`, `combat/`). Igualdade por valor e `validate()` fail-fast na mesma disciplina de `TileMap::validate()`/`SaveData::validate()`.
...
**Positivas:**
- O runtime nasce POCO/puro em `domain/`, testável em TDD sem __DEP_REMOVIDA__/UI, seguindo a disciplina já provada de `save/`/`map/`/`combat/`.
