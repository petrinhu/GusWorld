...
- **Restrição operacional:** F2-S.1 (setup técnico da Fase 2) está formalmente bloqueada por F1-DL.9 no TODO.md atual ("PAUSADA até deep-lore F1-DL.9 completa").
...
2. **Risco de tunneling.** Continuar lore sem nenhum código rodando = arquitetura permanece teoria. Pillar 3 (triângulo de hardware) ainda não foi validado em protótipo nenhum. Câmera orbital 3/4 (uma decisão visual central) nunca foi tocada em código.
...
6. **One-way door inverso (perigoso):** continuar 111k pal antes de tocar código pode resultar em descobrir, em mês 3 de F2, que uma decisão arquitetural (ex: turn-based hooks) precisa retrofit em 20 docs de lore. Caro.
...
1. **F2-S.1 (setup técnico da Fase 2):** dependência `Pré-requisito: F1-DL.9` **removida**. Pré-requisito atualizado pra `—` (nenhum). Liberado pra execução imediata.
...
**Contras:**
- ~3-6 meses adicionais antes de **primeira linha de código rodando**.
...
- **Context-switch cost.** Alternar entre código e prose Eco Adso tem custo cognitivo. Mitigação: sessões inteiras dedicadas, não micro-alternância (alternância intra-sessão proibida).
...
4. **Risco baixo:** decisões técnicas de engine forçarem reescritas em lore (ex: turn-based combat real revelar que carta canônica X é injogável). **Mitigação:** lore atual canônica é descritiva ("Compilação Rúnica combina cartas com tags compatíveis"), não prescritiva ("carta X faz Y dano em Z range"). Espaço de manobra mecânica preservado.
