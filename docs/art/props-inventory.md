...
| `logos_glyphs/` | 7 | cripto-glifo, grade do cripto-glifo, selo glyph Vyr, Sterling sigma logo, partitura última frequência, selo pergunta, QRCode |
...
## Elementos velados embutidos (não rotular em doc público)

- **`flora/` + `fauna/`** carregam o padrão numérico recursivo velado (recursão, fractais,
  L-system, Mandelbrot/Perlin/Voronoi).
- **`architecture/` + `traditions/`** carregam o conjunto arquitetural-ritual velado (ashlar
