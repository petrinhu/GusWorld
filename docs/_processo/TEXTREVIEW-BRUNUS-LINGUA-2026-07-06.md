...
> **RESGATADO em 25/08/2026 do `gusworld_legacy`. ACHADOS AINDA VIVOS, conferidos contra a árvore atual em 25/08/2026 pelo `technical-writer`.** Os dois achados críticos abaixo **continuam sem correção** em `docs/narrative/characters/brunus-vetorial.md` e `CHARS.md` linha 86: (1) `CHARS.md` credita "Bruno Vettore (agradecimentos do README)", mas `README.md` **não tem nenhuma ocorrência** desse nome hoje — ponteiro morto, precisa decisão do líder (também toca a L-16: homenagem a pessoa real só com aceite confirmado); (2) a ampola-relíquia do clímax de Brunus **não tem entrada em `docs/narrative/foreshadowing.md`** (que já tem F063/F093 para outras ampolas, nenhuma para esta). Os achados LEVES/MÉDIOS de gramática (capitalização pós dois-pontos em `brunus-vetorial.md:174` e `brunus-vetorial-conto.md:15`; regência em `brunus-vetorial.md:89`) também seguem sem correção. Nenhuma edição foi aplicada por este resgate — correção de prosa é trabalho do `narrative-writer`, correção de canon é decisão do líder.
