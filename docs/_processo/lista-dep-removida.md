...
|---|---|---|---|
| `docs/tech/adr/o arquivo do ADR-008, com o nome corrompido` | 1,10,12,13,14,16,21,23,24,25,26,30,32,36,40,41 | 29 | Título: "Re-pivot da camada de plataforma anterior para a seguinte, mais motor de UI declarativo". Corpo inteiro do ADR descreve a dependência (render 2D, gamepad, áudio, ABI portável, ganho sobre a camada anterior) |
...
- **2 achados laterais fora da contagem principal:** nome de arquivo corrompido em o arquivo do `ADR-008`, com o nome corrompido e em `difficulty_menu_____.hpp/.cpp` (citado dentro de `modos-morte.md:166`) — não capturados pela busca de string porque o nome de arquivo colapsou para `_`, não para o marcador completo.
