...
> **RESGATADO em 25/08/2026 do `gusworld_legacy` pelo `technical-writer`, na varredura profunda de narrativa.** Este é o dossiê **v1** (`F5-BK.AUDIT.FULL T1`, 22/05/2026), a primeira passada de auditoria cross-doc de nomes de personagem. Ele foi **sucedido, dentro do próprio legacy, pelo `AUDIT-T1-NOMES-V2.md`** (também resgatado, ver `docs/_processo/AUDIT-T1-NOMES-V2.md`), que registra "eficácia da cascata T1 v1: ~93%" — ou seja, a maior parte dos 98 achados abaixo já foi corrigida no próprio legacy antes mesmo dele fechar. Os poucos resíduos que sobreviveram até a v2 (Felícia Tarsila, Tarsila Atelaiá-Verônica -25, linhagem patrilinear Chevalier, o arquivo `11-venea-vance-mae.md`) foram **conferidos por este agente em 25/08/2026 contra a árvore atual do GusWorld** e estão **todos resolvidos**: `ordem-recursiva.md:266` e `268` já dizem "Felícia Tarsila"; `era-2-boom-tecnico.md:719` já traz a linhagem patrilinear completa e datação "-25"; o arquivo já se chama `11-gargi-vance-mae.md`.
>
> **Por que este documento v1, mesmo assim, não foi descartado:** por ordem do líder (L-14 do `GODS_LAWS.md`), nenhum agente declara documento morto. Ele fica como registro do processo de auditoria (98 issues levantadas, a metodologia de amostragem cross-doc usada), não como lista de pendências ativas. **Nenhum item deste documento deve ser tratado como achado vivo sem reconferir contra a árvore atual primeiro** — a esmagadora maioria já foi corrigida.
>
...
graph TD
    %% Lista negra ativa
    LN[Lista Negra Ativa] --> P1["Patrício Vance (5 ocorrências)<br/>→ Pyotor Vance"]
    LN --> P2["Belorian Vance (2 ocorrências)<br/>→ Belinor Vance"]
    LN --> P3["Atelaiá Chevarier (1 typo CHARS)<br/>→ Atelaiá Chevalier"]
    LN --> P4["Adila Koslov (1 BIBLE-V2)<br/>→ Adila Murmúrio"]

    P1 --> D1["lore-bible.md:520"]
    P1 --> D2["timeline.md:115"]
    P1 --> D3["in-world-docs.md:797"]
    P1 --> D4["era-2-boom-tecnico.md:691,953"]
    P1 --> D5["BIBLE-V2-CAPA.md:67"]

    P2 --> D6["era-2-boom-tecnico.md:279,285"]
    P3 --> D7["CHARS.md:66"]
    P4 --> D8["BIBLE-V2-APENDICE-H.md:22"]

    %% NPCs não-canonizados
    NC[NPCs canon não-CHARS] --> N1["Otmar Neumann (15+ refs)"]
    NC --> N2["Lavínia Sevra (6 refs)"]
    NC --> N3["Ardenia Falke (8 refs)"]
    NC --> N4["Cândido Rui (6 refs)"]
    NC --> N5["Felippe Garante (5 refs)"]
    NC --> N6["Vico + Inê Bento (filhos Mara)"]
    NC --> N7["Família Zacheu Vanderbist-Pelicano (5 NPCs)"]
    NC --> N8["Inês Marçal (Dutos)"]
    NC --> N9["Soraia (doc 7)"]

    %% Colisões homônimas
    CH[Colisões Homônimas] --> H1["Hilário Murch 47 (Mirage)<br/>vs Hilário Murch 64 (Periferia)<br/>CRÍTICO idade"]
    CH --> H2["Bartolo Penkin: tio Mateus (CHARS)<br/>vs pai Mateus (deep-lore)<br/>CRÍTICO C4"]

    %% Cronologia conflitante
    CR[Cronologia Conflitante] --> C1["Verônica Atelaiá: -30 morta (lore-bible)<br/>vs ativa -45 (timeline)<br/>vs ~-1 (sterling-deep)"]
    CR --> C2["Atelaiá Chevalier: -78 (CHARS)<br/>vs ~-80 (natureza-deep)"]
    CR --> C3["Bartolo Penkin: -12 desaparece (timeline)<br/>vs -16 (in-world-docs)"]
    CR --> C4["Gustaf I Tavus Vance: -150 (CHARS)<br/>vs -148 (ordem-recursiva)<br/>vs -140 (era-2)"]

    %% Idades conflitantes Sterling
    ST[Idades Sterling INCOHERENCES C7] --> S1["Octávia Penedo: 52 (lore-bible)<br/>vs 55 (CHARS)"]
    ST --> S2["Theodoro Calveri: 57 (lore-bible)<br/>vs 52 (CHARS)"]
    ST --> S3["Solange Vix: 49 (lore-bible)<br/>vs 46 (CHARS)"]

    %% Cascata fix proposta
    F[Cascata Fix Total] --> F1["6 substituições nominais Patrício→Pyotor"]
    F --> F2["2 substituições Belorian→Belinor"]
    F --> F3["1 typo Chevarier→Chevalier CHARS"]
    F --> F4["1 typo Adila Koslov→Murmúrio"]
    F --> F5["~15 entries CHARS novos (NPCs)"]
    F --> F6["~5 decisões one-way door criador"]
    F --> F7["~10 retrofit idades canon"]

    classDef critical fill:#ff6b6b,stroke:#c92a2a,color:#fff
    classDef medium fill:#ffd43b,stroke:#fab005,color:#000
    classDef leve fill:#a9e34b,stroke:#5c940d,color:#000
    classDef resolvido fill:#74c0fc,stroke:#1971c2,color:#fff

    class LN,P1,P2,P3,P4,NC,N1,N2,N3,N4,N5,N6,N7,N8,N9,CH,H1,H2,CR,C1,C2,C3,C4,ST,S1,S2,S3 critical
    class F,F1,F2,F3,F4,F5,F6,F7 resolvido
```
