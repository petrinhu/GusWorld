...
- FATO: bus, mensagem `inbox/gusworld/2026-08-20-cripto-DENTRO-decisao-do-lider.md`: o líder decidiu que **criptografia está DENTRO do escopo do GlintFx 1.0** (uma biblioteca de criptografia de terceiro a absorver, tabela da mensagem de 19/08 02:10, linha 26). Estado FECHADO. A mesma mensagem registra a ressalva técnica do CTO do GlintFx: implementação própria de cripto falha por canal lateral e comparação que vaza tempo.
...
- FATO: `modos-morte.md` referencia infraestrutura do projeto ANTERIOR (ADR-006, ADR-014, `FsSaveStore`, "uma biblioteca de criptografia de terceiro vendorizada", `title_menu.hpp`). Pela L-01 nada disso existe como código; pela L-13 é canon a re-derivar; pela L-14 eu não declaro o documento morto, apenas aponto: o DESIGN (intenção das 3 camadas) parece vigente, as REFERÊNCIAS de implementação apontam para código que não existe. Cabe ao líder confirmar o que sobrevive.
...
- **Payload = serialização binária versionada dos POCOs de domínio** (L-17: um estado canônico só torna o save completo por construção). Escrita em casa, determinística, campo a campo com ordem fixa: é código puro de `domain/`, não precisa de GlintFx nem de terceiro, e evita depender de lib de schema externa (uma biblioteca de schema de terceiro seria um terceiro elo, o que a LEI ZERO não dá de graça).
...
| Alvo | Mecanismo |
|---|---|
| Save normal | Envelope com **AEAD XChaCha20-Poly1305** (cifra + autenticação num passo; construção recomendada pela referência canônica dessa cifra para uso local, nonce de 192 bits seguro para nonce aleatório: [doc oficial](https://outra biblioteca de criptografia de terceiro.gitbook.io/doc/secret-key_cryptography/aead/chacha20-poly1305/xchacha20-poly1305_construction)). Chave derivada de constante do build + tipo do arquivo |
...
- FATO: o líder já decidiu, no bus, que criptografia está DENTRO do escopo do GlintFx 1.0, com uma biblioteca de criptografia de terceiro como biblioteca a absorver (`inbox/gusworld/2026-08-20-cripto-DENTRO-decisao-do-lider.md`, estado FECHADO). FATO: o GlintFx foi reiniciado do zero em 21/08/2026 e hoje tem um único header. INFERÊNCIA: a decisão de escopo é do líder e presumo que sobrevive ao reboot, mas isso é confirmação que cabe a ele, não a mim (L-14).
...
- Sub-caminho (iii-b), **vendorizar uma biblioteca auditada pequena** (a mesma que o GlintFx pretende absorver, ou outra cuja documentação seja a referência das construções citadas: [doc](https://outra biblioteca de criptografia de terceiro.gitbook.io/doc/secret-key_cryptography/aead/chacha20-poly1305/xchacha20-poly1305_construction)). Tecnicamente são as mesmas primitivas do caminho (i), disponíveis imediatamente. MAS: é um terceiro elo além de GlintFx e SO, portanto **só existe se o líder conceder exceção nominal, caso a caso, como a própria LEI ZERO prevê**. Nota de contexto: o projeto anterior fez exatamente isso (uma biblioteca de criptografia de terceiro vendorizado, ADR-014), e o bus registra que aquela exceção era "do líder deles" e independente; pelo L-01 aquilo não é base, e a exceção não se herda sozinha, se re-decide.
...
5. **Canon desatualizado (L-13/L-14):** `modos-morte.md` referencia ADR-006/ADR-014, `FsSaveStore` e "uma biblioteca de criptografia de terceiro vendorizada" do projeto anterior, que não existem (L-01). Não declaro o documento morto; aponto que a decisão desta rodada deve dizer explicitamente o que daquele desenho sobrevive como intenção e o que se re-deriva, senão o próximo agente vai implementar contra referência fantasma.
