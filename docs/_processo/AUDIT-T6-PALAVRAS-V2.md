...
> **RESGATADO em 25/08/2026 do `gusworld_legacy` pelo `technical-writer`, na varredura profunda de narrativa. ACHADO PRINCIPAL VERIFICADO E EM GRANDE PARTE RESOLVIDO, conferido contra a árvore atual em 25/08/2026.** Este dossiê (`F5-BK.AUDIT.FULL T6 v2`, 22/05/2026) reporta "BLOQUEADO MASSIVO", 1.060+ issues, das quais **992 eram ocorrências de travessão (em-dash, "—") em prosa canônica**, contando `factions.md` (128), `lore-bible.md` (78) e `BIBLE-V2-STRUCTURE.md` (48) como maiores ofensores.
>
> **Verificação por contagem em 25/08/2026:** `grep -o "—" docs/narrative/factions.md` e o mesmo para `lore-bible.md` e `foreshadowing.md` (também citado como ofensor) devolvem **zero** ocorrências nos três. A contagem total de travessão em `docs/narrative/` inteiro, hoje, é **130** — uma redução de ~87% frente aos 992 originais. Isto bate com a regra de estilo do projeto contra em-dash (registrada como anti-padrão em outros documentos da casa). **Não foi feita, aqui, uma varredura arquivo a arquivo dos 130 remanescentes** para dizer se são uso legítimo, texto novo escrito depois do dossiê, ou resíduo — só a contagem agregada.
>
> **Os 9 críticos "maçom literal" e as decisões de terminologia in-world (one-way doors) NÃO foram reconferidos** por este agente. Ficam como estavam no legacy.
>
> **Nada aqui foi declarado morto por este agente** (L-14 do `GODS_LAWS.md`). Se o líder quiser fechar o item por completo, um `grep -rn "—" docs/narrative/` mais uma decisão sobre os 130 remanescentes encerraria o achado principal.
>
