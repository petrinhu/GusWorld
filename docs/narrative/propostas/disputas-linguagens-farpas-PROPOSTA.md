...
|---|---|---|
| **X-01** | **O bordão do Cauã é "compila e roda"** (`vozes-party.md:145-152` e `:212`), e o Bento diz na Cena 2 "PYTHIA SEQUER COMPILA". Duas peças canônicas aprovadas que se chocam de frente, e **ninguém no jogo jamais apontou isso**. | **Não corrigir. Explorar.** Sob a LEI, o moleque da linguagem interpretada ter como bordão o verbo que ele não pratica é ouro cômico, e é a semente pronta de uma cena Cauã x Bento (par vazio, L-07). Melhor achado do inventário. |
| **X-02** | O payoff da **Cena 7** chama "Use Pythia" de "vitória da Jaci na flame war". Sob a LEI, a Pythia não vence flame war no eixo da execução. | Ler como vitória de ergonomia resolve, e **não toquei na cena**. Sugiro uma palavra na nota de payoff numa passada futura, pra ninguém usar a Cena 7 como precedente pra inverter o eixo. Decisão do líder. |
| **X-03** | `vozes-party.md:137`, B2 do Cauã: **"Rodou RÁPIDO."** Se lido como execução, **inverte o eixo**. No contexto o sentido é "ficou pronto rápido", e o B6 (`:172-179`) confirma essa leitura. | A frase isolada é ambígua, e um implementador pode transformá-la em bark de combate e furar a lei sem perceber. Sugiro **nota de leitura de uma linha** no guia do Cauã. Não editei. |
| **X-04** | Dois relógios pro vazamento de C-Arcane do Dante: a §6 (`:1205`) proíbe "compila/otimiza/roda" antes de ~75%, mas a §1 (`:994`) observa que "force push" (Cena 4, bem antes) **já é** vocabulário de controle de versão, logo C-Arcane. O próprio guia reconhece a tensão. | **Segui o desenho existente**, sem inventar calibragem: Cena 4 é o vazamento fundador já canonizado, e todo vazamento novo meu está na fase (c). Ver §6. |
| **X-05** | Cena 2 define que o Asmódico separa instruções por TABULAÇÕES. Assembly real não é sensível a tabulação (isso é Python e Makefile). | Liberdade diegética legítima (Asmódico não é assembly, é o análogo do mundo). Registro só pra ninguém "corrigir" a Cena 2 no futuro achando que achou um bug. |
| **X-06** | Cena 2, "PYTHIA SEQUER COMPILA": falso como fato (o CPython compila pra bytecode), verdadeiro como juízo. | Passa na régua fato x juízo dos apartes (`gus-apartes-c-arcane.md:10-15`). Registro pelo mesmo motivo do X-05. |
| **X-07** | **Cauã x Iara e Jaci x Linda não existem na matriz de dinâmicas** (`party.md:58-71`). Escrever farpa pra eles é **criar relação canônica nova**, não preencher lacuna de redação. | **Decisão do líder.** Escrevi os dois porque foram pedidos, com eixo explícito proposto (§4.4 e §4.5), e **não editei `party.md`**. |
