...
- **Conteúdo (layer 2):** text rendering on-demand via a UI da engine própria (glintfx sobre C++23 + __DEP_REMOVIDA__; ver ADR-008). Cache de strings recentes. Nunca pre-renderiza todas as entries simultaneamente.
...
|---|---|---|
| **PC Linux (AppImage / .tar.gz)** | Keyboard + mouse + gamepad (Xbox/PS4/PS5 nativo via __DEP_REMOVIDA__) | Decisão Fase 1 canônica. |
