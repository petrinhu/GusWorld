...
>
> Contexto assumido pela pesquisa ao escrever: o motor **teria** o contrato de telegraph (`enemy_brain.hpp` + `combat_records.hpp`) e **já usaria** o glintfx pra UI (`app/`). As recomendações abaixo foram desenhadas para casar com essa premissa, não para propor reescrever nada dela.
>
> ⚠️ **Não verificado contra código.** Este contexto vem da análise de projeto, não de medição em código executado. O código deste projeto nasce do zero (GODS_LAWS.md, L-01), e a fonte que a redação original citava (`enemy_brain.hpp`, `combat_records.hpp`) não existe. Revalidar quando houver implementação.
...
A pesquisa presumiu que o motor **já modelaria isso**, com `enemy_brain.hpp` declarando (descrição de intenção, código ainda não escrito — ver L-01 em `GODS_LAWS.md`):
...
E `combat_records.hpp` teria o dado:
...
Isso seria **exatamente** o padrão Slay the Spire (número exato) + Into the Breach (`predicted_shape`/`predicted_target_id` = geometria do alvo, não só dano) + o ícone "Unknown" (`is_chaotic`). O comentário no header, como a redação original o citava ("TODA AI é obrigada a expor IntentPreview... um inimigo sem intent legível quebraria o pillar de informação"), seria a mesma filosofia de "perfect information" do Into the Breach.

> ⚠️ **Não verificado contra código.** As duas estruturas de código acima, a assinatura de `preview_intent` e a citação do comentário de header vêm da análise de projeto, não de medição em código executado. O código deste projeto nasce do zero (GODS_LAWS.md, L-01), e a fonte que a redação original citava (`enemy_brain.hpp`, `combat_records.hpp`) não existe. Os campos e a forma continuam válidos como descrição de intenção; revalidar quando houver implementação.
...
2. **Ícone + número, glintfx-nativo.** Um `<span>`/elemento por intent, com classe RCSS por `predicted_action_id`/categoria (ataque/buff/debuff/passar) — cor consistente por categoria é tema comum aos dois temas desta pesquisa (ver TEMA 2-B). O número (`predicted_damage`) via data-binding (`bind_number`, já documentado na API do glintfx).
...
5. **Gambito = a mecânica de "pré-visualizar reordenação de turno" que já é padrão de tactics genérico** (TV Tropes acima) — o campo `reorder_delta` teria existido em `CombatAction` (`combat_records.hpp:136`, referência não verificada, ver ressalva abaixo). Sugiro (não decisão, só leitura de padrão): ao jogador armar uma ação de Gambito (antes de confirmar), a UI de fila de turno mostra a ordem PROJETADA pós-reorder ao lado da ordem atual, do jeito que "alguns jogos deixam pré-visualizar a fila mudando antes de confirmar" — reaproveita o mesmo layout de fila-de-retratos que qualquer UI de turn-order tactics já teria, só adiciona o estado "prévia".

   > ⚠️ **Não verificado contra código.** A citação de `combat_records.hpp:136` vem da análise de projeto, não de medição em código executado. O código deste projeto nasce do zero (GODS_LAWS.md, L-01), e a fonte que a redação original citava não existe. Revalidar quando houver implementação.
...
**BARATO, segundo a premissa da pesquisa.** O dado existiria no domínio (`IntentPreview`), o contrato obrigaria toda IA a expô-lo, e o `battle_scene.hpp` teria `intent_for(...)` retornando `std::optional<IntentPreview>` (comentário citado pela redação original: "a cena mostra o ícone sobre o inimigo"). O trabalho que falta é: (a) confirmar recálculo ao vivo (teste, não código novo), (b) bind via glintfx de ícone+número+cor por categoria (RCSS + data model, técnica padrão já em uso no projeto para outras telas), (c) highlight de alvo por `TargetShape` (RCSS de borda em slot, sem asset 3D), (d) prévia de reorder de fila pro Gambito (reaproveita `reorder_delta`, ver ressalva acima). Nenhum item exigiria asset novo, shader novo, ou pedido ao dev do glintfx.

> ⚠️ **Não verificado contra código.** As citações de `IntentPreview`, `battle_scene.hpp`/`intent_for(...)` e do comentário de header vêm da análise de projeto, não de medição em código executado. O código deste projeto nasce do zero (GODS_LAWS.md, L-01), e a fonte que a redação original citava não existe. O veredito de custo ("BARATO") e a lista de trabalho continuam válidos como leitura de projeto; revalidar quando houver implementação.
...
**Contagem numérica animada ("tweening" de número) — 2 caminhos técnicos, e nenhum é nativo do motor de UI do glintfx.**
...
- **O motor de UI do glintfx não tem `@property`/CSS counters** (é um subconjunto de CSS, não um browser) — então o caminho aplicável ao GusWorld é o equivalente ao caminho JS: o HOST (C++) escreve o número interpolado a cada frame via data-binding, não a UI sozinha.
...
**RCSS — o que já é suportado nativamente (fonte primária, doc oficial do glintfx).**

Extração da documentação de animações/transições/transforms do RCSS:
...
- **`transform`** com `translate`/`translate3d`, `scale`/`scale3d`, `rotate`/`rotate3d`, `skew`, `matrix`, `perspective` — com `transform-origin` configurável. A doc declara **"suporte total de interpolação para transforms"**, ou seja, animar `transform` é um caso de primeira classe, não workaround.
...
Mapeamento explícito **glintfx-nativo vs. pedido-glintfx**, por componente de juice:

| Componente de juice | Como construir | glintfx-nativo ou pedido-glintfx? |
|---|---|---|
| Cor consistente por categoria (dano/cura/buff/debuff/crítico) | Classe RCSS por categoria, cor fixa no stylesheet | **glintfx-nativo** (é CSS puro, já suportado) |
| Contagem numérica animada (HP/dano subindo ou descendo) | Host C++ interpola o valor por N frames e escreve via `bind_number`/`set_number` (mesmo padrão já documentado no `reference_glintfx_api.md` para outros data models) | **glintfx-nativo** — é lógica do HOST, não feature nova da lib; a API de data-binding já existe |
| Screen shake (do painel/log, não câmera 3D) | `@keyframes` com `transform: translate()` alternando offsets pequenos, disparado por `add_class("shake")` via `set_property`/`add_class` (API v0.9.0 já existe) e removido após a duração | **glintfx-nativo** — `transform` tem "suporte total de interpolação" pela doc oficial |
| Hit-pause / freeze-frame (60-90ms) | **NÃO é UI** — é pausar a SIMULAÇÃO de combate (o game loop C++) por N ms no frame de impacto, mantendo o RENDER da UI rodando (senão a tela trava de vez, o que é diferente de hitstop) | **Fora do glintfx inteiramente** — é lógica de `combat_state_machine`/loop de app, zero dependência de glintfx |
| Pop/squash-stretch em elemento (crítico, combo) | `transform: scale()` com easing `back-out` ou `elastic-out` (nativos!) disparado por toggle de classe | **glintfx-nativo** — as curvas de easing certas já existem prontas, não precisa aproximar |
| Sequência causal A→B→C (combo de cartas, Joker-style) | `animation-delay` crescente por elemento (0ms, staggered), ou `transition-delay` por classe aplicada em sequência pelo host | **glintfx-nativo** — `@keyframes` + `delay` cobre isso sem JS/lógica extra |
| Glow/destaque em número crítico | `filter: drop-shadow(cor x y blur)` **em um elemento-caixa** (span com padding), NUNCA em texto solto | **glintfx-nativo, COM RESSALVA:** lembrar o gotcha "sem text-shadow" — precisa envolver o número num elemento-caixa, não é um bug a reportar, é um padrão de markup a seguir |
| Glow/destaque animado (pulsar) | Mesma técnica acima + `animation` no `filter`, já que "filters" está na lista de propriedades animáveis | **glintfx-nativo** |
| Sombra "encaixotada" de elemento (não glow) | `box-shadow` **estático** funciona; mas **não pode animar** `box-shadow` (limitação documentada) — se precisar animar sombra, trocar por `filter: drop-shadow()` (que É animável) | **glintfx-nativo com workaround conhecido** (trocar de propriedade, não pedido a dev) |
| Entrada/saída de linha do log-terminal | `transition` de `opacity`+`transform:translateY()` disparado por classe (`.entering`/`.entered`) | **glintfx-nativo** |

**Conclusão do mapeamento: nenhum componente de juice do TEMA 2 exige pedido ao dev do glintfx pra este slice.** Toda a superfície necessária (`transform`, `filter`, `transition`, `animation`, `add_class`/`remove_class`, `set_number`) já está na v0.9.2 pinada (`CMakeLists.txt`), confirmada em `reference_glintfx_api.md`. **[opinião]** O único cenário que justificaria um pedido futuro é performance em máquina fraca com MUITAS animações simultâneas (ex.: 6 companions + inimigos todos "pulsando" ao mesmo tempo) — isso é pergunta de profiling empírico pós-implementação, não uma lacuna de feature conhecida hoje.
...
**BARATO, quase inteiramente.** É a conclusão mais forte desta pesquisa: **cada item do checklist de juice mapeia 1:1 pra uma feature já documentada e já pinada do glintfx**, exceto o hit-pause (que não é UI, é lógica de loop C++, também barata). Não há necessidade de pedido de feature nova ao dev do glintfx para completar este slice — só disciplina de RCSS + um pouco de lógica de tween/hit-pause no lado app (`app/`).
...
> ⚠️ **Não verificado contra código.** Os itens #2 e #10 abaixo repetem a premissa de
> que `IntentPreview` e `reorder_delta` "já existem" no domínio. Essa premissa vem da
...
2. **Ícone+número de intent + highlight de alvo por `TargetShape`** (TEMA 1) — dado já existe no domínio (`IntentPreview`), só falta bind via glintfx. BARATO. Depende de #1 pra cor por categoria do ícone.
...
- Todas as fontes com URL foram abertas via WebSearch/WebFetch nesta sessão (2026-07). A doc oficial do motor de UI usado internamente pelo glintfx foi lida diretamente — é a fonte de maior confiança deste documento por ser a própria referência técnica da lib que o glintfx embrulha.
