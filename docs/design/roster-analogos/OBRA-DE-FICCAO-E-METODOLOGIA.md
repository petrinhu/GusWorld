...
> Documento bilíngue: bloco EN completo, depois bloco PT completo (espelho), depois o apêndice em pt-br. / Bilingual document: full EN block, then full PT block (mirror), then the pt-br appendix.

**EN**
- [Fiction disclaimer (read this first)](#fiction-disclaimer-read-this-first)
- [What this is](#what-this-is)
- [This is fiction, not biography](#this-is-fiction-not-biography)
- [Methodology (how the roster is built)](#methodology-how-the-roster-is-built)
- [Method calibration](#method-calibration)
  - [Axis weights v2 (GAME-ONLY scalar refactor, 2026-07-11)](#axis-weights-v2-game-only-scalar-refactor-2026-07-11)
  - [The v3 full re-run (persona model change to Fable + re-test of the whole roster, 2026-07-11)](#the-v3-full-re-run-persona-model-change-to-fable--re-test-of-the-whole-roster-2026-07-11)
- [Sources and attribution](#sources-and-attribution)
- [A note on the one living inspiration](#a-note-on-the-one-living-inspiration)
- [A note of respect](#a-note-of-respect)
- [Questions or concerns](#questions-or-concerns)

**PT**
- [Aviso de Obra de Ficção e Metodologia: o Roster de Figuras Históricas](#aviso-de-obra-de-ficção-e-metodologia-o-roster-de-figuras-históricas)
- [Aviso de ficção (leia isto primeiro)](#aviso-de-ficção-leia-isto-primeiro)
- [O que é isto](#o-que-é-isto)
- [Isto é ficção, não biografia](#isto-é-ficção-não-biografia)
- [Metodologia (como o roster é construído)](#metodologia-como-o-roster-é-construído)
- [Calibração do método](#calibração-do-método)
  - [Pesos dos eixos v2 (refatoração do escalar, EXCLUSIVA DO JOGO, 2026-07-11)](#pesos-dos-eixos-v2-refatoração-do-escalar-exclusiva-do-jogo-2026-07-11)
  - [O reteste completo v3 (mudança do modelo dos personas para Fable + re-rodada do roster inteiro, 2026-07-11)](#o-reteste-completo-v3-mudança-do-modelo-dos-personas-para-fable--re-rodada-do-roster-inteiro-2026-07-11)
- [Fontes e atribuição](#fontes-e-atribuição)
- [Uma nota sobre a única inspiração viva](#uma-nota-sobre-a-única-inspiração-viva)
- [Uma nota de respeito](#uma-nota-de-respeito)
- [Dúvidas ou preocupações](#dúvidas-ou-preocupações)

**Apêndice / Appendix**
- [Appendix: Per-character record / Apêndice: Registro por personagem](#appendix-per-character-record--apêndice-registro-por-personagem)
  - [Record template / Template do registro](#record-template--template-do-registro)
...
**Resultado da calibração v3 (personas Fable, pesos v2, 2026-07-11).** Conjunto de direita: Mises `0.706`, Hayek `0.590 / 0.601 / 0.608` em três rodadas independentes (mediana `0.601`). Conjunto de esquerda: Marx leftness `0.703`, Mao `0.732`, Einstein `0.716`. Convergência `= min(0.706, 0.601, 0.703, 0.732, 0.716) = 0.601 > 0.599`: o método **permanece validado sob personas Fable**. Dois achados: (a) a troca Opus para Fable moveu cada âncora em no máximo cerca de 0.03 (Hayek caiu de 0.612 para 0.601; a leftness do Marx de 0.730 para 0.703), então Fable é substituto sólido de persona; (b) o Hayek fica exatamente no limiar, o que é fiel, não um defeito: ele é o membro mais moderado do conjunto de direita (aceita rede de segurança, gradualista, socialmente neutro), então mal passar de 0.599 é o resultado correto. O criador aceitou a validação pela mediana (AMB-08).
...
| **ESQUERDA (2)** | Einstein 0.284 · Tesla 0.370 (heróis, mau-trágico, AMB-10/11) |
...
| **ESQUERDA, mau-trágico (2)** | Einstein, Tesla | mau trágico/equivocado (AMB-10/11, decidido 2026-07-11): confronto de tom compassivo, nobre-equivocado, não vilão puro |

Total 20 figuras (Tusk fica de fora por fiat, blindado). Distribuição de sidequest: 11 ajuda, 7 puzzle, 2 confronto mau-trágico (Tesla, Einstein; AMB-10/11). A axiologia (direita/conservador = bom) e a salvaguarda de herói governam; o algoritmo dá a base econômica, o desempate por tradição/autoridade resolve o centro; a salvaguarda de herói amansa o tom dos dois de esquerda para "trágico-equivocado", não vilão puro.
...
- **(b) 8values result / Resultado 8values:** the four axes as percentages (Economic, Diplomatic, Civil/Govt, Societal), the closest ideology label, the `rightness` scalar (0.0 to 1.0, v1 and v2), and the 70-token answer array. / os quatro eixos em % (Econômico, Diplomático, Civil/Governo, Social), o rótulo de ideologia mais próxima, o escalar `rightness` (0.0 a 1.0, v1 e v2), e o array de 70 respostas.
- **(b2) Engine descriptive block / Bloco descritivo do engine:** the verbatim engine output block (both poles of all four axes with labels + ideology + rightness). / o bloco de saída do engine verbatim (ambos os polos dos quatro eixos com rótulos + ideologia + rightness).
- **Redos (center protocol) / Redos (protocolo de centro):** for center figures, the 3 independent runs + the median that decides. / para figuras de centro, as 3 rodadas independentes + a mediana que decide.
- **Insights / Insights:** the persona-agent's non-obvious notes. / as notas não-óbvias do persona-agent.
...
**AMB-10 (2026-07-11): Tesla (ELM-03), herói de esquerda, enquadramento in-fiction (estava DEFERIDO, decidido na hora da prosa).** Tesla v3 `rightness 0.370` (ESQUERDA, dentro da zona Einstein re-ancorada).
- Opção 1, neutro→puzzle: a nuance individualista/pró-patente desempata pro centro; evita vilanizar o gênio azarão.
- Opção 2, bom/aliado trágico: o sonho de energia universal o redime pro lado bom (exceção a bom=direita), mancha eugênica como sombra.
- Opção 3, mau trágico/equivocado: idealista nobre que errou (energia coletivizada + a mancha eugênica dos anos 1930); confronto, mas não vilão puro, tom de queda.
- **Decisão do criador: opção 3 (MAU trágico).** Tesla = ESQUERDA / mau-trágico, desafio sombrio de tom compassivo (queda de um idealista), não vilão puro. Aplica esquerda=mau com a nuance.

**AMB-11 (2026-07-11): Einstein (FIS-01), herói de esquerda, enquadramento in-fiction (estava DEFERIDO).** Einstein v3 `rightness 0.284` (`leftness 0.716`, ESQUERDA; âncora da zona re-ancorada).
- Opção 1, neutro→puzzle: a nuance civil-libertária/humanista desempata pro centro; evita vilanizar o herói amado.
- Opção 2, bom/aliado: o humanismo + o anti-autoritarismo o redimem pro lado bom (exceção a bom=direita).
...
**AMB-14 (2026-07-11, loop de prosa, Planck FIS-03): elo lore↔mecânica novo: AP inteiro do Tavus-Drive justificado in-world pela quantização do Planck.** A âncora Pillar 3 do §6 do Planck (paridade de série, ruling do CPO no gate) amarra "energia que se recusa a dar meio passo, sempre inteira, sempre contável" ao porquê de o Tavus-Drive gastar AP em pontos INTEIROS (nunca meio ponto): o chão quantizado do Planck é a razão in-world da economia de AP ser severa e contável. **Decisão AUTÔNOMA (autorização "decida o que puder sem mim"); confirmar retroativamente.** Reversível, mas é canonização implícita de um elo lore-mecânica bonito, vale o líder saber.
...
**AMB-17 (2026-07-11, loop de prosa, Bruno OCU-01): nível de velado do elo Bruno↔multiverso secreto.** Bruno é a figura MAIS próxima do segredo (a tese "outra pedra canta a mesma nota" / mundos paralelos É o multiverso). Diferente do Gödel, o design doc `14-giordano-bruno.md` JÁ tem 2 decisões do líder (2026-07-09): fogueira dura + a ironia trágica (a Ordem o queimou porque chegou perto da verdade selada do Encapsulamento de Helíaco Vyr). **Decisão do CPO (NÃO escalada, fundamentada): nível VELADO**: Bruno PREGA a tese (heresia pública in-fiction), a Ordem o queima, mas fica velado/ambíguo se ele estava CERTO; o cronista NUNCA confirma o multiverso nem nomeia "Encapsulamento/verdade selada" como fato. Racional: `cosmologia-origem-deep.md §1/§5` (CANON) manda que o segredo "aflora por descoberta, nunca por exposição narrada" e é "revelação de fim de jogo": um doc de mestre confirmá-lo VIOLARIA o canon; ou seja, o velado não é discricionário, é mandado. Escolher explícito é que exigiria escalada. Velar = reversível. **Confirmar retroativamente na canonização.**
- **AMB-17b (sussurro):** "Bruno nunca soube o quanto acertou" entra só como **nota marginal apócrifa, não assinada, SEM objeto do acerto** ("o que a Ordem fez com este, as atas não explicam"), no padrão do Cronista não identificado da cosmologia §4 + narrador não-confiável (INCOHERENCES C2).
- **AMB-17c (litania):** Bruno é séculos ANTERIOR à rede itinerante Era 2 → a litania de pares NÃO é item-a-item (anacronismo); vira UMA frase de contraste (ele deixou uma pergunta enterrada em cinza, não uma ferramenta pra rede). Decisão do CPO.
- **AMB-17d (eco §5):** o eco pessoal do Bruno é distinto da tese pública dele (acordar com a certeza de já ter morrido velho numa cama de fronteira que nunca viu); fallback B (gosto de fruta de pomar que nunca comeu) se A ecoar demais.
- **Pendente (jusante):** carta OCU-01 A/B/C aberta (recomendação B, eco do próprio Gus); e o texto in-game da descoberta (a roda entrega a hipótese, nunca a confirmação) segue a MESMA regra de velado.

**AMB-18 (2026-07-11, loop de prosa, Dee OCU-02): velado da quase-testemunha + 3 decisões de execução.** Dee é a "quase-testemunha acidental da fratura" (decisão do líder 2026-07-09): as "vozes de anjos" que ele ouvia pelo espelho eram, na verdade, ecos do Estilhaçamento/outros ramos mal-entendidos. **VELADO canon-mandado** (mesmo nível do Bruno): o cronista registra as vozes SÓ em nível de fonte ("os fragmentos afirmam"), NUNCA confirma o multiverso nem nomeia Estilhaçamento/ramos como o que ele ouviu, e NUNCA justapõe as vozes à tese do Bruno (regra anti-triangulação do cluster OCU: dois docs roçando o segredo não podem, juntos, confirmá-lo). Base: cosmologia §1/§5 mandam o velado. NÃO escalada (já decidido). **3 decisões de execução AUTÔNOMAS** (reversíveis, "decida o que puder sem mim"): (a) carta OCU-02 fica PENDENTE como todas (prosa descreve o núcleo genérico, sem comprometer A/B/C; recomendação do CPO/design = B, foresight); (b) o vidente/médium (análogo do Kelley) foi OMITIDO (fidelidade a "o que ELE ouvia"; intermediário diluiria o velado); (c) enquadre de serviço = cifrador/restaurador das catedrais menores (sem inventar autoridade/corte nova na Era 1). **Anti-triangulação vale também pro Pitágoras (OCU-03).**

**AMB-19 (2026-07-11, loop de prosa, Pitágoras OCU-03): elo Vyr + conteúdo velado + 2 decisões editoriais.** (a) **Elo Helíaco Vyr:** o design doc §30 faz a Ordem ensinar Pythagoras e Vyr lado a lado como "parábola de espelho" (o cantor cego que uniu pedra e som até ROMPER a realidade; o filósofo que uniu número e som até TOCAR a mesma verdade sem nunca romper nada). **Decisão do CPO: NÃO escalar como one-way door separado** (a parábola de espelho já é mecanismo canônico da cosmologia §2; o pareamento entra pelo fluxo normal, nasce PROPOSTA e só vira canon com aprovação do líder). Velado o multiverso (a "mesma pergunta em qualquer ramo" NÃO é confirmada; fica como o nó estrutural que a Ordem observa); anti-triangulação com Bruno/Dee; Pythagoras JAMAIS toca a fratura (contraste "nunca rompeu nada" explícito). (b) **Conteúdo velado arquitetural/numérico (risco MÁXIMO por ser a figura da numerologia):** teto duro de 1-2 elementos velados além do triângulo de dez pontos; NUNCA 89/21 em contexto de contenção/levante. (c) **2 decisões editoriais AUTÔNOMAS:** "tetraktys" descrito-não-nomeado ("triângulo de dez pontos em quatro fileiras"); "monocórdio" descrito-não-nomeado ("uma corda só sobre madeira"). (d) narrador semilendário-INVERTIDO (Pythagoras sofre de EXCESSO de lenda, não escassez; abertura distinta dos pilotos). Carta OCU-03 pendente (prosa genérica). **Confirmar retroativamente na canonização.**

**AMB-20 (2026-07-11, loop de prosa, Menger ECO-03): exposição da captura do Crédito pela Sterling Corp na crônica Era 2 + calibrações de execução.** A amarração canônica (decisão do líder 2026-07-09, `19-menger.md` §2): o crédito de hoje (lore-bible §11.4, digital, gerido por Sterling Corp, rastreável) é o MESMO instrumento espontâneo documentado por Menger, capturado por decreto corporativo gerações depois; token-rádio anônimo = eco moderno exilado do princípio de troca livre; Mercado da Sucata Honesta = eco vivo. **Decisão do CPO (NÃO escalada, fundamentada): a captura entra VELADA, sem nomear Sterling Corp no corpo.** O cronista pode aludir ("gerações depois, alguém tentaria recentralizar o que nasceu livre, e chamar de seu o que ninguém cunhou"), sem nome, sem data. Racional: (a) crônica de registro Era 2 nomeando ator do presente Era 3 = anacronismo de registro; (b) axiologia MOSTRADA, não pregada (anti-panfleto, padrão Hayek/Mises); (c) velar é reversível, nomear na prosa canônica é one-way; (d) o elo explícito Sterling↔captura já vive no design doc e pode chegar ao jogador por outra via (in-world doc Era 3 / descoberta), não pelo registro do mestre. **Sub-decisões de execução autônomas (reversíveis):** (b) o Mercado da Sucata Honesta entra SÓ na batida 6 (presente do jogo, eco vivo, grafia exata), NUNCA como lugar visitado por Menger (na Era 2 existem pátios de sucata e oficinas cooperativas, não o Mercado); (c) o nome "crédito" (minúsculo, como lore-bible §11.4) pode nascer na crônica como a unidade que pegou sem decreto, sem afirmar que já era digital/rastreável; (d) litania da batida 6 item-a-item (Menger é rede Era 2; cadeia do Mises §6 + item novo do próprio Mises), SEM ordem temporal entre pares (convenção 2, proibido enquadrá-lo como "fundador antes de"); (e) token-rádio: alusão velada só na batida 6, sem tese; (f) tutor-de-herdeiro real OMITIDO (inventaria corte/autoridade que a reconstrução cooperativa Era 2 não tem). Carta ECO-03 pendente como todas (prosa descreve núcleo genérico, sem comprometer A/B/C; recomendação do design = B). **Confirmar retroativamente na canonização.**

**AMB-21 (2026-07-11, loop de prosa, Bastiat ECO-04): Sterling velado + sátira sem tract + fecho do cluster ECO + petição in-world.** (a) **Sterling/compadrio velado (decisão do CPO, NÃO escalada, mesmo racional do AMB-20):** o design doc `20-bastiat.md` §2 liga Bastiat ao compadrio de Sterling Locke na Era 3; na crônica Era 2 a ligação entra VELADA, uma alusão só no §6 ("gerações depois, alguém aprenderia a vestir de favor a lei que tira, em escala que Bastiat só viu em miniatura de vilarejo"), sem nomear Sterling Corp, sem datar. Anti-triangulação ECO: somada à alusão do Menger §4 ("recentralizar o que nasceu livre"), o par continua sem nome e sem época (verificado pelo revisor-textual na canonização). (b) **Anti-panfleto EXTRA:** Bastiat é panfletário real; antídoto = a sátira e o não-visto entram como veículos NARRATIVOS (cena da vidraça quebrada, petição absurda in-world, édito de pedágio refutado em assembleia); o cronista nunca declara veredito; vocabulário ideológico real banido; "espoliação legal" transposta como conceito descrito, nunca como termo. Achado de auditoria remediado (rodada 1): o fecho do §6 trazia cláusula prescritiva ("julgar bem significa contar os dois") que passava no teste-editorial; cortada, restando só o fato descritivo do mecanismo (padrão dos fechos Hayek/Mises/Menger). (c) **Fecho do cluster (decisão do CPO):** litania permanece item-a-item (Bastiat é rede Era 2, sem anacronismo), 15 pares + item novo do Menger + fecho do Bastiat; ganha UMA nota diegética a mais (a herança de Bastiat = a pergunta que audita as outras, "o que não estou vendo?"), sem "último" nem ordem temporal. (d) **Petição absurda in-world = opção A "os aguadeiros contra a chuva"** (decisão autônoma do main thread sob mandato amplo; flavor reversível, sem entidade nomeada nova nem fato de timeline): os carregadores de água pedem à assembleia um édito obrigando a cobrir telhados e tapar calhas porque a chuva "inunda o mercado com água de graça". NÃO usa velas-contra-o-sol (não decalca a obra real). (e) Dois focos de religião real (§1 "internato de monges"; rodapé "internato religioso") pegos na auditoria e trocados por "internato de província" (secular); regra dura vale até no rodapé (precedente Newton/Planck). Carta ECO-04 pendente como todas (prosa descreve núcleo genérico "O Que Não Se Vê", sem comprometer A/B/C; recomendação do design = A). **Confirmar retroativamente na canonização.**

**AMB-22 (2026-07-11, loop de prosa, Tesla ELM-03): decisões de craft do PRIMEIRO mau-trágico (executa AMB-10, não o reabre).** Enquadramento travado pelo líder (AMB-10): esquerda/mau-trágico, confronto compassivo, queda de um sonhador. Decisões de execução do CPO, todas confirmar retroativamente: (a) **Tom:** cronista idêntico ao da série; o julgamento entra pela contabilidade (nos irmãos bons o custo é pago pelo próprio mestre; no Tesla, por terceiros que sustentaram a torre); ZERO veredito moral abstrato; compaixão pelos detalhes registrados (sonho nobre mostrado, fim humano: pássaro ferido, morte pobre). Na auditoria, uma glosa avaliativa vazou ("O horror pequeno e concreto dessas páginas é esse:") e foi cortada na rodada 1 (o fato fala sozinho). (b) **Mancha transposta (risco máximo):** entra VELADA E BREVE (4 frases no §4, sombra, não set-piece) como doutrina tardia de "depurar a rede / podar os nós que não ressoam", vocabulário 100% de rede/sinal/impedância, JAMAIS linhagem/sangue/espécie/grupo humano; contrapeso = a rede recusou copiar essas páginas; a não-nomear vale até no rodapé (precedente Newton/Planck/Bastiat). O CPO registrou o risco de leitura fora de contexto em repo público e recomendou incluir mesmo assim (omitir esvaziaria o mau-trágico decidido no AMB-10); confirmação do líder na canonização. (c) **Energia coletivizada velada (anti-panfleto):** o erro emerge só de consequência (a torre que dá sem medir consome quem a sustenta e não entrega; a rede que troca peça por peça sobrevive); vocabulário ideológico real banido; "dar" não é vício, "dar sem medida" quebra. (d) **Litania §6:** item-a-item MANTIDA (Tesla é rede Era 2, sem anacronismo; cortar a litania no primeiro mau-trágico leria como excomunhão, contra o compassivo mandado); cadeia do Bastiat §6 verbatim + item DUPLO do Tesla (herança: a ressonância virou o pulso em círculos do presente, velado, sem nomear personagem atual; aviso: a torre cobra de quem sustenta alcance sem medida). (e) **Eco §5:** visão eidética da torre pronta e acesa que nunca existiu, com defeito de junção que ele só acharia anos depois na torre real; velado, sem rótulo, sem confirmar multiverso. (f) **Carta ELM-03-R_v1.0 pendente** como todas (núcleo genérico do arco que salta e enfraquece, sem alvos/AP/passiva; recomendação do design = A). (g) **Sem dica-pro-Tusk** na prosa (feat separada). (h) **Espelho de Faraday:** cash-in por método (quieto-perto vs longe-sem-fio), sem ordem temporal. (i) **Convenção de numeração:** a partir do cluster OCU os docs usam numeração RELATIVA ao cluster (ex.: "Quarto e último doc do cluster eletromag ELM"), não o ordinal global da subsérie (resiliente a inserções fora de ordem); o Tesla foi corrigido de "Décimo sétimo doc" pra a forma cluster-relativa. **Confirmar retroativamente na canonização.**

**AMB-23 (2026-07-12, loop de prosa, Einstein FIS-01): decisões de craft do SEGUNDO mau-trágico e ÚLTIMO doc do loop (executa AMB-11 e a decisão do líder sobre o Nível 2; não os reabre).** Enquadramento travado pelo líder (AMB-11): esquerda/mau-trágico, confronto compassivo, "a falha do sábio humanista". Decisões de execução do CPO (Capitolino), todas confirmar retroativamente: (a) **Reconciliação vítima×falha (a ironia central):** o design doc §2 faz Einstein VÍTIMA de um enclave coletivista que o expulsa; o AMB-11 faz a falha trágica dele ser o próprio ideal econômico coletivista. Reconciliado em §4(a): o homem ferido pela tirania de um enclave que decidia por todos manteve fé num arranjo de fundo comum distribuído por mesa central, cego pra semente comum entre o desenho dos próprios cadernos e o enclave que o feriu; falha mostrada SÓ por consequência in-world (oficinas do arranjo definham; feiras livres ao lado prosperam; ele lê o fracasso como falta de virtude, nunca como defeito de desenho); cronista registra os fatos lado a lado e não fecha a conta; vocabulário ideológico real = 0, regime real = 0, até no rodapé. (b) **Cicatriz cósmica = Nível 2 VELADO (decisão do líder, espelha AMB-16 do Gödel):** o §5 mostra que ele achou, pela matemática, uma descontinuidade herdada na geometria do mundo, que não soube nomear e não publicou (páginas costuradas no último caderno); PROIBIDO confirmar/nomear/datar/ligar (grep de segredo = 0, incluindo Ordem Recursiva/Catedral-Mãe/Helíaco Vyr/Verônica Atelaiá); a convergência com a tradição fica implícita e inominada; a encenação em jogo (beat/gate/rota Bronze-Prata-Ouro) permanece reservada ao `narrative-designer`. **Anti-triangulação FIS:** zero referência à suspeita do Newton (AMB-12); Einstein resolve sozinho, os dois docs juntos não confirmam nada (Newton só na litania). (c) **Dualidade poder-responsabilidade: ENTRA, velada e mínima (decisão do CPO):** §4(b), um parágrafo; corolário de força imensa da própria geometria + carta de alerta a um conselho da rede + arrependimento ("a única página que tentou pedir de volta"); NENHUM evento destrutivo encenado, NENHUM artefato construído, NENHUM fato novo de timeline; destino da carta não-documentado; rima com o Gus implícita. (d) **Eco §5 distinto dos 16 anteriores:** não é déjà-vu de sensação, é o resíduo que a matemática devolve (a cicatriz); fecho em UMA anotação marginal sem glosa, teto de ambiguidade máximo. (e) **Litania §6:** cadeia do Bastiat §6 verbatim + item Bastiat; **SEM item do Tesla** (Tesla e Einstein são os últimos, paralelos, sem ordem entre si); item final do Einstein = iniciativa por tempo relativo VELADA (sem "turno"/mecânica/número) + segundo tempo tocando a cicatriz no limite do Nível 2. (f) **Carta FIS-01-R_v1.0 "Dilatação Temporal" pendente** como todas (núcleo genérico, referenciais em ritmos diferentes, sem comprometer A/B/C; recomendação do design = A). (g) **Numeração cluster-relativa:** "Terceiro e último doc do cluster física FIS" (com os dois irmãos citados); header registra "segundo dos dois registros mau-trágicos (AMB-11)". (h) **Sem dica-pro-Tusk**; herança de Maxwell cash-in sem ordem temporal; digest RAG NULO (0.080) declarado no rodapé. **Este AMB fecha o loop de prosa dos 17 mestres do Codex (Tusk excluído por fiat).** **Confirmar retroativamente na canonização.**

**AMB-24 (2026-07-12, descoberta por artefato × encontro com o mestre = Tavus-Eco; DECISÃO DO LÍDER na canonização #1).** **Lacuna descoberta:** os design docs do roster descreviam a descoberta de cada carta como achado de ruína/cadernos/puzzle, e o loop dos 20 docs escreveu cada mestre como figura histórica morta cujo legado sobrevive só como a carta; MAS o projeto original do criador sempre foi a party ENCONTRAR cada mestre após a missão e RECEBER a carta da mão dele. O descompasso nasceu da lacuna nos design docs, não dos textos dos mestres. **Decisão do criador (travada):** mecanismo **Tavus-Eco** (nomeado pelo criador; raiz Sylvarin `tavus-` "o pulso que executa", padrão Tavus-Drive). O mestre, diante da morte, compila a si mesmo (self executável) no artefato; resolvido o puzzle da missão, e com o rádio do padre Landell sintonizado nele, o eco roda e o mestre aparece (projeção compilada) pra entregar a carta e conversar. Os mestres continuam MORTOS na Era 1/2 (biografias intactas). Casa com Pillar 1 (magia = software). Opções descartadas pelo criador: retconar vivos/imortais (destrói biografias); fantasma genérico (viola Pillar 1). **Nuance por era (CPO):** Era 2 (17) = eco compilado pleno; Era 1 (3 OCU) = variante cerimonial (gravado em canto/pedra/roda), mesma lógica de ativação, a taxonomia Era 3 classifica ambos como Tavus-Eco. **Guarda do segredo (CPO):** o Tavus-Eco é camada 1 (computável); a camada 3 (origem/Estilhaçamento) é incompilável por definição, logo o eco NÃO pode confirmar nem expor cosmologia de origem; o momento §5 "O eco" de cada mestre não entra na compilação; Tavus-Eco e o eco do §5 são fenômenos DISTINTOS, nunca ligados em texto (caso sensível: Einstein, o eco não expõe a cicatriz do §5). **Execução (feita 2026-07-12):** (1) passe leve nos 20 §6 semeando o Tavus-Eco no beat final (fórmula do CPO, §1-§5 e litania intactos, 6 agentes narrative-writer por cluster); (2) definição canônica em `docs/design/technomagik.md` (seção "Tavus-Eco"); (3) nota no `CHARS.md` §8d por mestre. **Entregável novo derivado:** a CENA DE ENCONTRO (diálogo do eco entregando a carta) é feat própria, casada com as cartas (#2); fios abertos (o eco persiste vs dissipa; o nome Sylvarin puro do fenômeno, candidato `lhin-`) pra lead-game-designer/narrative-designer com o criador. Correção de conlang associada: a raiz Sylvarin #11 `gala-` (palavrão pt-br, flag do criador) foi trocada por `eryn-` (sindarin bosque/mata). Os detalhes de craft (fórmula, nuance de era, guarda) do CPO ficam **confirmar retroativamente**; o mecanismo Tavus-Eco é decisão do criador.
...
**Nota de ordenação / Ordering note:** os 23 registros abaixo estão ordenados por domínio/ID (ELM, FIS, MAT, CMP, OCU, ECO, CAP, depois os 2 controles externos [CAL]). Helion Tusk (CAP-01) é o único registro por **fiat autoral puro** (figura viva, nunca perfilada nem submetida ao quiz); Marx e Mao Tsé-tung ([CAL]) são **controles externos de calibração**, fora do roster do jogo. Todos os demais 20 registros passaram pelo algoritmo completo (pesquisa, persona, 8values, tripla checagem). / The 23 records below are sorted by domain/ID (ELM, FIS, MAT, CMP, OCU, ECO, CAP, then the 2 external [CAL] controls). Helion Tusk (CAP-01) is the only record set by **pure authorial fiat** (living figure, never profiled or quizzed); Marx and Mao Zedong ([CAL]) are **external calibration controls**, outside the game roster. All other 20 records went through the full algorithm (research, persona, 8values, triple check).
...
> **RODADA VIGENTE = v3 (persona Fable, 2026-07-11): `rightness_v2 0.370` = ESQUERDA, DENTRO da zona Einstein re-ancorada [0.184, 0.384].** Perfil v3: Econômico Equality 69.9% (Social) · Diplomático Peace 68.3% · Civil Liberty 58.2% (Moderate) · Social Progress 72.1%; ideologia Social Democracy. Array Fable (70): `["a","a","d","d","sa","d","d","d","n","n","sa","d","d","sa","d","d","n","a","d","a","a","d","n","d","a","sa","a","a","d","d","a","a","n","d","d","d","d","a","n","d","n","n","a","sa","d","a","sd","a","a","a","d","d","sa","a","d","n","d","n","a","d","n","a","n","n","d","a","a","a","a","sd"]`. Heroico + na zona: consulta ao criador disparada; resolvido em AMB-10 como MAU trágico/equivocado (confronto compassivo, não vilão puro). O bloco Opus abaixo fica como histórico.
...
- **(c) Conclusion / Conclusão:** **LEFT / ESQUERDA** no eixo econômico (`leftness 0.718`), classificação econômica registrada. **Enquadramento in-fiction (bom/mau/neutro) e tipo de desafio: MAU trágico/equivocado (decisão do criador, AMB-10, 2026-07-11)**: idealista nobre que errou (energia coletivizada + a mancha eugênica dos anos 1930); confronto de tom compassivo, queda de um sonhador, não vilão puro; aplica esquerda=mau com a nuance da salvaguarda de herói (ver `feedback_heroi_nao_vira_vilao`). **Tripla checagem:** (1) 8values = leftness 0.718, "Social Democracy"; (2) web (Wikipedia) = "tecno-utopista, energia universal, explorado pelo capital industrial, sem socialismo doutrinário, individualista"; (3) **convergem**. / In-fiction framing: tragic-misguided villain (AMB-10, same call as Einstein). **v2 (pesos do jogo / game weights): `rightness 0.354`, ESQUERDA mantida; cai DENTRO da zona Einstein re-ancorada [0.187, 0.387], então o deferimento passa de exceção pontual a aplicação de regra.** **v3 (Fable, RODADA VIGENTE): `rightness 0.370` (mediana de 3 rodadas), ESQUERDA, ainda dentro da zona re-ancorada.**
...
- **(c) Conclusion / Conclusão:** **LEFT / ESQUERDA**. `leftness 0.904` ≫ 0.599. **Tripla checagem:** (1) 8values = leftness 0.904, "Libertarian Socialism"; (2) web (Wikipedia, "Political views of Albert Einstein") = "socialista democrata, 'Why Socialism?', anti-autoritário, libertário nas liberdades civis"; (3) **convergem**. Nota fina: a ideologia "Libertarian Socialism" captura exatamente a tensão dele: **esquerda no econômico** (o eixo que define `rightness`) + **libertário no civil** (Liberty 80.1%). Também é **ponto de validação de esquerda** da calibração. **Enquadramento in-fiction (bom/mau/neutro) e tipo de desafio: MAU trágico/equivocado (decisão do criador, AMB-11, 2026-07-11)**: o socialismo econômico é a "falha do sábio humanista"; confronto de tom compassivo, nobre-equivocado, não vilão puro; aplica esquerda=mau com nuance (a salvaguarda de herói amansa o tom, não muda a classificação). A classificação econômica do algoritmo permanece registrada; ver `feedback_heroi_nao_vira_vilao`. / In-fiction framing (good/bad/neutral) and challenge type: **tragic-misguided villain (AMB-11): the economic socialism is the flaw of the humanist sage, a compassionate confrontation, not a pure villain; left=bad applied with nuance.** **v2 (pesos do jogo / game weights): `rightness 0.287` (leftness 0.713), ESQUERDA confirmada; o escore dele sob a fórmula vigente é a âncora da zona Einstein re-ancorada [0.187, 0.387].**
...
- **(c) Conclusion / Conclusão:** **RIGHT / DIREITA**, in-fiction "good / bom" (boa evolução). `rightness 0.686` > 0.599. **Tripla checagem:** (1) 8values = 0.686, "Classical Liberalism"; (2) web (Wikipedia) = "right-libertarian classical liberal, influenciou Thatcher/Reagan, NÃO anarco-capitalista"; (3) **convergem**. Corretamente menos extremo que o Mises (0.686 < 0.846), refletindo o gradualismo/rede-mínima. Âncora de calibração de direita. **v2 (pesos do jogo): `rightness 0.612`, DIREITA confirmada** (âncora re-validada sob v2). **v3 (Fable, RODADA VIGENTE): `rightness 0.601`** (mediana de 3 rodadas; DIREITA no limiar, ver Calibração v3 / AMB-08). Approved by creator / aprovado pelo criador: 2026-07-11 (triagem óbvia-direita) + confirmado pelo algoritmo.
...
- **(c) Conclusion / Conclusão:** **LEFT / ESQUERDA**. `leftness 0.878` > 0.599. **Tripla checagem:** (1) 8values = leftness 0.878, "State Socialism"; (2) web (Wikipedia) = "far-left autoritário, totalitário de partido único"; (3) **convergem**. Nota fina: o método captura o Mao **estatista no eixo civil** (Authority 70.3%), contraste correto com o Marx libertário-civil: mesma esquerda econômica, eixos civis opostos. **Controle externo de calibração de esquerda** (não é figura do jogo, sem enquadramento in-fiction). **v2 (pesos do jogo): `leftness 0.735`, ESQUERDA confirmada.** **v3 (Fable, RODADA VIGENTE): `leftness 0.732`, ESQUERDA.** Approved by creator / aprovado pelo criador: 2026-07-11 (controle conhecido-esquerda) + confirmado pelo algoritmo.
