...
**Consumidor A, o HUD de produção (RmlUi via glintfx). É este que manda.**

| Fato | Onde |
|---|---|
| `#log { font-size: 10dp; }` | `GusEngine/app/src/screens/battle_cockpit_rml.cpp:243` |
...
| avanço de glifo `= kMonoAdvanceRatio * px_size`, com `kMonoAdvanceRatio = 0.5f` (PixelOperatorMono é monoespaçada) | `GusEngine/platform/include/gus/platform/render2d/text_metrics.hpp:32` |
...
**Ressalva honesta:** eu **não executei a fonte** para conferir o avanço real. O `0.5` é a constante que o repositório declara e usa no próprio `text_width`; o caminho do RmlUi desenha por FreeType com a métrica real da fonte, que numa face monoespaçada deve bater com `0.5em`, mas isso não foi medido aqui. Se o avanço real for `0.55em`, o teto cai para cerca de 41. **Por isso recomendo escrever com teto de 42:** o texto sobrevive às duas leituras.
