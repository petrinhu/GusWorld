...
>
> ### ✅ PRÉ-REGISTRO DA FASE C-BIS (escrito em 2026-08-11): ASSINADO PELO LÍDER
>
> A seção **§9** deste documento é um pré-registro NOVO, referente à rodada
> **`PACING-FASE-C-BIS`** (ação Provocar + soft enrage + critério do cap). Ele é o item 6
> da sequência de fatias da spec `spec-provocar-soft-enrage-criterio-cap.md` §6
> (*"PROTO-1 (documento): pré-registro novo [...] assinado pelo líder ANTES de rodar"*).
>
> | Peça fixada em §9 | Valor |
> |---|---|
> | Teto de taxa de evento raro (lutas no cap, P6) | **1,0 x 10⁻⁴** |
> | Confiança da decisão | **95%** (limite superior de Poisson exato) |
> | N por ponto | **300.000** |
> | Gate irmão de percentil | **p99 de duração no P6 <= 20 rodadas** |
> | Gates da ação Provocar | **G1 a G6** (§9.3) |
> | Gates do soft enrage | **H1 a H5** (§9.4) |
> | Faixa-alvo de golpes no provocador (P7) | **70% a 85%** |
>
> **✅ Assinado em 2026-08-11.** Mesma disciplina do pré-registro de 2026-08-01 acima e da
> regra da casa: o critério se fixou ANTES de o dado existir. O harness já foi CONSTRUÍDO
> (fatias PROV-1, PROV-2, ENRAGE-1, ENRAGE-2, SIM-P7) e verificado por QA adversarial
> (QA-MOTOR, QA-SIM) antes de qualquer run oficial ser executada ou citada como evidência.
>
...
| **E7** | Cap de 30 rodadas batido (P6) | Regressão: o turtle-empate tem que continuar morto | ~0% de cap; e a derrota do turtle não pode virar massacre em ≤3 rodadas (lição tem tempo de chegar). **⚠️ A régua "~0% de cap" está SUPERADA a partir da Fase C-bis:** ela é um critério de zero estrito, proibido pelo princípio do evento raro (§4.2). Vale como está para as Fases A/B/C já rodadas (não se reinterpreta dado antigo); da C-bis em diante, E7 se decide pela tripla de §9.2 mais o gate irmão de percentil |
...
### 4.2 Princípio do evento raro (acréscimo metodológico, 2026-08-11)

Origem: `spec-provocar-soft-enrage-criterio-cap.md` §4.4, reproduzido aqui como regra
permanente do protocolo. Nasce de uma dor concreta e medida: na Fase C, **uma única luta
em 240.000** reprovou dois candidatos inteiros, porque o critério em vigor era, na
prática, "zero eventos".

> **Princípio do evento raro.** Nenhum critério do estudo pode exigir "zero eventos" de
> forma estrita. Todo gate de evento raro se declara como uma tripla, fixada JUNTA no
> pré-registro, antes de qualquer dado existir:
> **(teto de taxa `p`; confiança de 95%; `N >= 3/p`)**.
> A decisão usa o **limite superior de 95% de Poisson exato** para o `k` observado, nunca
> a taxa pontual. O relatório imprime sempre `k`, a taxa observada e o limite superior,
> inclusive quando `k = 0` (zero declarado distingue "não houve evento" de "ninguém
> contou"). Quando existir uma métrica de PERCENTIL que meça o mesmo fenômeno, ela entra
> como gate irmão obrigatório, porque percentil não depende da sorte da cauda.
>
> **Justificativa (fontes levantadas na Fase C §8):** regra de três de Hanley &
> Lippman-Hand (JAMA, 1983): zero em N demonstra no máximo taxa <= 3/N; law of truly
> large numbers (Diaconis & Mosteller, JASA, 1989): critério de "zero em N" fica mais
> difícil conforme N cresce, ou seja, mede o tamanho da amostra e não o jogo; c=0
> acceptance sampling (ASQ/Squeglia; ISO 28594): mesmo o controle de qualidade que "exige
> zero defeitos na amostra" é formalizado como decisão sob risco quantificado por curva
> OC, nunca como demonstração de ausência.

Dito na régua do projeto, sem estatística: **"zero em N" não é propriedade do jogo, é
propriedade da amostra.** Quanto maior a amostra, mais difícil passar, sem que nada tenha
piorado no sistema medido. Um critério assim mede quanto tempo a máquina rodou, não se o
combate está bom, e portanto premia quem simula pouco. É o irmão exato do erro que a casa
já registrou em outro contexto: **a checagem que confirma o que você espera é a que menos
protege.**

Três consequências operacionais, válidas para TODO gate futuro deste protocolo e não só
para o cap de rodadas:

1. **Nenhuma régua nova pode ser escrita como "0%", "nenhuma ocorrência" ou "~0%".** Se o
   fenômeno é raro, a régua é a tripla. Se o autor não consegue nomear o teto de taxa,
   ele ainda não sabe o que está medindo.
2. **O relatório imprime a linha mesmo quando `k = 0`** (`k`, taxa, `UB95`, `N`), pelo
   mesmo motivo que a casa exige "erros internos: 0 de N" impresso: zero declarado
   distingue "não houve" de "ninguém olhou".
3. **Percentil manda mais que contagem de cauda.** Sempre que existir um percentil que
   meça o mesmo fenômeno, ele é gate obrigatório ao lado da taxa, e os dois têm de
   aprovar. Motivo medido: o candidato 39/11 da Fase C (mediana 10, p90 15) seria pego por
   um gate de p99 **sem depender de nenhuma luta chegar ao cap**.

**Isto não reinterpreta as Fases A, A-bis, B e C.** O veredicto delas foi dado pelo
critério que estava escrito na época e assim permanece. O princípio vale da Fase C-bis em
diante.
...
| V7 | Cap de rodadas | 30, "empate técnico" | Herdado; trava de segurança. **Nota 2026-08-11:** o cap de 30 é do HARNESS (`kRoundCap` em `mira_sim_harness.hpp`, test-only); em produção não existe cap nenhum hoje. Ele permanece inalterado na Fase C-bis, como INSTRUMENTO DE MEDIÇÃO, para não quebrar a comparabilidade com as Fases A/A-bis/B/C. O que muda na C-bis não é o sensor, é o CRITÉRIO de leitura dele (§9.2) |
...
8. **Fora de escopo blindado:** nenhum número deste estudo entra em `combat.md` sem
   ...
   anterior.
9. **Pré-registro por rodada, não uma vez só (acréscimo 2026-08-11):** cada rodada nova
   do estudo tem o pré-registro DELA fechado e assinado antes de rodar. O da Fase C-bis
   é a **§9**. Um pré-registro assinado não cobre uma rodada que mudou de cenário, de
   mecânica ou de critério.

---

## 9. PRÉ-REGISTRO DA FASE C-BIS (Provocar + soft enrage + critério do cap)

> **Status: PRÉ-REGISTRO, escrito em 2026-08-11, ASSINADO PELO LÍDER em 2026-08-11 (§9.7).**
> A run oficial da `PACING-FASE-C-BIS` está liberada quanto a este pré-registro.
> Autor: `lead-game-designer`. Fonte primária de todo número desta seção:
> `spec-provocar-soft-enrage-criterio-cap.md` (2026-08-11), com a seção de origem citada
> em cada item. Onde a spec e este pré-registro divergirem, **a spec é a fonte** e a
> divergência é erro a corrigir aqui, não decisão nova.

### 9.1 Por que existe uma Fase C-bis, e o que ela mede

A Fase C reprovou **0 de 4** candidatos de statline por três motivos, dois de DESENHO e um
de CRITÉRIO (`analise-pacing-fase-c-20260811.md` §6/§7/§8). A C-bis não repete a C: ela
roda os **mesmos 4 candidatos** (33/12 controle, 34/12, 33/13, 39/11), porque nenhum foi
derrubado por ser um número ruim, e sim por propriedades do sistema em volta deles. O que
muda é o sistema em volta (spec §5):

| Cenário | Composição | Para que existe |
|---|---|---|
| **P7 (novo): parede com provocação** | 3 inimigos; tanque-proxy **provoca + defende**; os outros dois atacam | Mede se a correção funcionou (gates G1 a G6). É o P2 com a alavanca ligada |
| **P7b (novo): provocar sem defender** | idem, mas o provocador só provoca | Isola o efeito da penalidade de Def; alimenta a decisão Q2 da spec |
| **P6 (alterado): tartaruga com enrage** | inalterado, com o soft enrage ligado | Gates H3, H4, H5 |
| **P1/P2 (inalterados) com enrage ligado** | inalterados | Gates H1 e H2: prova de que a luta saudável não sente o enrage |
| Braços de mira | os mesmos dois: `F_sem_F4` (titular) e `C_F4_suave` (vizinho) | G5 exige os dois. O vizinho segue sendo **teste de robustez**, nunca candidato a produção |

Papéis novos no harness: `ProvokeThenAttack` e `ProvokeAndDefend`, espelhando os papéis
existentes `DefendThenAttack` / `DefendOnlyTurtle`. **O harness continua chamando a função
de produção, nunca reimplementando a regra** (a trava de paridade
`mira_harness_production_parity_test.cpp` tem de falhar antes de passar na fatia PROV-2,
senão não está medindo nada).

**Ordem obrigatória, herdada da Fase C §9:** o desenho da defesa (a spec) se decide ANTES
de ligar qualquer mira em produção (`IA-ALVO-PRIMEIRO-DA-LISTA`). Ligar a mira ponderada e
depois trocá-la seria retrabalho garantido.

### 9.2 A tripla do evento raro (decisão do líder, 2026-08-11)

Aplicação do princípio de §4.2 ao gate do cap de rodadas. **Decidido pelo líder em
2026-08-11**, respondendo a pergunta Q6 da spec (que oferecia `10⁻⁴` com N = 300.000 ou
`10⁻⁵` com N = 500.000):

| Peça | Valor fixado | Origem |
|---|---|---|
| **Teto de taxa** de lutas que batem o cap (P6) | **1,0 x 10⁻⁴** (1 em 10.000 encontros) | spec §4.1 |
| **Confiança** | **95%**, limite superior de **Poisson exato**, nunca a taxa pontual | spec §4.1 |
| **N por ponto** | **300.000** (a Fase C usou 240.000) | spec §4.1 |
| **Gate irmão de percentil** | **p99 de duração no P6 <= 20 rodadas** | spec §4.1 e §4.3 |
| **Regra de leitura** | reprova se `UB95(k)/N > teto`; aprova caso contrário | spec §4.1 |
| **Impressão obrigatória** | sempre `k`, taxa observada, `UB95` e `N`, **inclusive quando `k = 0`** | spec §4.1 (zero declarado) |

Limites superiores de Poisson exato usados na leitura (spec §4.2): `UB(k=0) = 3,00`;
`UB(k=1) = 4,74`; `UB(k=2) = 6,30`.

**Quantos eventos ainda aprovam, com N = 300.000:** até **k = 20** (k=20 dá 9,7 x 10⁻⁵;
k=21 dá 1,008 x 10⁻⁴, e reprova). É a folga que conserta a dor de origem, em que **uma
única luta em 240.000 reprovou dois candidatos**.

**O critério novo não absolve quem merecia reprovar** (o teste de "conserto interesseiro",
spec §4.2): o candidato 39/11 esponja mediu 35 lutas capadas em 240.000 (1,46 x 10⁻⁴), o
que em 300.000 equivale a ~44 eventos, muito acima do limite de 20. **Continua reprovado.**
O teto novo absolve a flutuação de cauda e só ela.

**Custo de CPU não é argumento em nenhuma direção, e isto foi medido:** a 41.000 a 75.000
lutas/s, uma grade de 48 pontos a 300.000 lutas dá 14,4 milhões de lutas, ou 3 a 6 minutos
(spec §4.2). O número exato de células da C-bis sai do desenho final da grade (os cenários
P7/P7b e o braço com enrage mudam a contagem) e **não é fixado aqui**; o que é fixado é o
N por ponto.

**Significado para o jogador, que é a régua honesta:** um playthrough tem da ordem de 300 a
600 encontros de trash. A 10⁻⁴, a chance de um playthrough inteiro ver UM empate técnico é
~3% a 6%. E a conta é pessimista por construção, porque o P6 é adversarial (a party inteira
só defende, o tempo todo), coisa que nenhum jogador real executa por muitas rodadas
seguidas. Somado ao soft enrage, a taxa verdadeira esperada é **zero estrutural**, e o gate
passa a ser piso de segurança, não alvo.

**Por que o gate irmão de percentil importa mais que o de taxa** (spec §4.3): gate de
evento raro depende da sorte da cauda; percentil, não. O p99 mede a forma inteira da cauda
e teria pego o 39/11 (mediana 10, p90 15, logo p99 seguramente acima de 20) **sem depender
de nenhuma luta chegar a 30**. Calibração do 20: com enrage ligado, o teto analítico de
pior caso é a **rodada 15** (spec §3.4), e 20 dá 5 rodadas de folga para variância de RNG e
para as configurações que a prova analítica não cobre.

### 9.3 Gates G1 a G6: a ação PROVOCAR

Reprodução fiel de `spec-provocar-soft-enrage-criterio-cap.md` §2.8. Todos medidos no
cenário P7 (e P7b onde indicado), nos DOIS braços de mira.

| Gate | O que mede | Critério |
|---|---|---|
| **G1** | Fatia dos golpes inimigos que vão ao provocador (cenário P7) | **70% a 85%**, nos DOIS braços de mira |
...
| **G5** | Sensibilidade ao braço de mira: diferença de vitória entre titular e vizinha no P7 | **<= 5pp** (o piso de relevância, §5) |
| **G6** | Duração (E2) no P7 | continua na janela **3-5** |

**G5 é o gate que responde a pergunta que a Fase C fez.** Hoje a diferença entre braços é
de 5,85 a 6,77 pontos percentuais (Fase C §4). Se, com Provocar, ela cair abaixo do piso de
relevância, o desenho ficou robusto e o assunto encerra: com uma alavanca explícita no jogo,
o resultado deixa de depender da esperteza da IA.

**Correções pré-declaradas (fixadas agora, para não serem inventadas depois de ver o
dado):**

- **Se G3 reprovar** (a combinação Provocar + Defender trivializa a luta), a correção é
  torná-las **mutuamente exclusivas**, e **não** mexer no multiplicador de mira. A spec
  declara essa combinação como *"a decisão desta spec com maior risco de estar errada"*
  (spec §2.7 e Q2).
- **Se G4 reprovar por REPROVAÇÃO NUMÉRICA** (o provocador cai, mas menos que deveria —
  passa de 0% mas fica abaixo do esperado), o parâmetro a mexer é `kProvokeDefFactor`
  (preço em pele), não `kMiraProvokeMultiplier`.
- **Se G4 EMPATAR em 0%×0%** (o provocador não cai NEM no P7 NEM no P2, com o statline
  real do Bento — HP 55/Atk 10/Def 13/SPD 5, aprovado pelo líder em 2026-08-11, ver
  `BENTO-STATLINE-COMBATE`), **a correção pré-declarada é `G4 vira informativo`** — ele
  continua medido e impresso, mas deixa de decidir se o candidato sobrevive. Achado do
  `QA-SIM` (fatia `QA-MOTOR`/`QA-SIM`): com a Jaci-proxy (Def 10) o empate 0%×0% já era
  estrutural (o tanque-proxy nunca caía em nenhum dos dois cenários), e mexer em
  `kProvokeDefFactor` não resolveria — reduzir a Def do provocador reduz também o pool do
  Shield na mesma proporção, então ele continua absorvendo o dano todo. Com o Bento real
  isso pode não se repetir (Def 13, mais exposto sob enrage do que a Jaci era), mas o
  plano B fica registrado ANTES do run, não inventado depois de ver o dado.
- **Se G1 ficar fora da faixa**, o parâmetro a mexer é `kMiraProvokeMultiplier` (faixa útil
  6,0 a 20,0, spec §2.4), e só ele.

### 9.4 Gates H1 a H5: o SOFT ENRAGE

Reprodução fiel de `spec-provocar-soft-enrage-criterio-cap.md` §3.8.
...
| **H4** | Lutas que batem o cap de 30 no P6 | pela tripla de §9.2 (teto de taxa 1,0 x 10⁻⁴, UB95, N = 300.000) |
| **H5** | Duração máxima observada em toda a grade | **< 20 rodadas**, coerente com o teto analítico de 15 da spec §3.4 |

Parâmetros do enrage sob os quais os gates são medidos (spec §3.1 e §3.2), para o tier
trash/elite: `R0` (`kSoftEnrageStartRound`) = **8**; `pct` (`kSoftEnrageAtkPctPerRound`) =
**15**, linear sobre o Atk BASE, com **piso de +1 por rodada**; aplicado **só ao lado
inimigo**. `R0` nasce do descritor do ENCONTRO (dado por tier), não de constante global.
**Boss e boss final ficam FORA da C-bis** (spec §3.2 e Q7): a regra proposta
`R0 = teto da janela-alvo + 3` daria 21 e 33, mas isso é decisão do
`BOSSES-ZELDA-BRAINSTORM`, não deste pré-registro.

**Condição de validade a monitorar durante a rodada** (spec §3.4): a prova de término
depende de a mitigação da party ser LIMITADA por construção (Shield com pool = Def, cura da
Jaci limitada pela bateria, sem regeneração ilimitada). Se qualquer mecânica de mitigação
que ESCALE sem teto entrar no motor antes da run, a prova cai e H3/H5 perdem a âncora
analítica. Declarar no relatório que a condição foi conferida na data da run.

### 9.5 Pré-condições da run (o que tem de estar resolvido antes)

1. **Assinatura de §9.7 pelo líder.** Sem ela, nada roda.
2. **As perguntas Q1 a Q5 e Q7 da spec respondidas** (identidade e escopo, não número):
   convivência entre a ação Provocar e a carta honeypot F5; Provocar + Defender no mesmo
   turno; quem pode provocar; nome do comando; existência ou não de cap em produção;
   enrage em boss. Elas mudam o QUE se simula, então respondê-las depois da run invalida a
   run.
3. **Q8 (statline do Bento) decidida OU o viés reafirmado por escrito. ✅ SATISFEITA em
   2026-08-11** (item `BENTO-STATLINE-COMBATE`): o statline foi aprovado pelo líder,
   **HP 55 / Atk 10 / Def 13 / SPD 5, Cinético** (`combat.md` §17), e o harness da C-bis
   ...
   analítico com o Bento: 84,3% no P7b, dentro da faixa, folga de 0,7pp) e o critério do
   G4 segue como escrito, com o empate 0%×0% agora analiticamente esperado (o provocador
   do P7 precisa de ~7 rodadas de foco para cair numa luta que dura 4) e coberto pelo
   plano B pré-declarado de §9.3, a aplicar SE o run oficial confirmar o empate.
4. **Fatias PROV-1, PROV-2, ENRAGE-1, ENRAGE-2 e SIM-1 entregues e verificadas** (spec §6),
   com a trava de paridade harness/motor atualizada e falhando-antes-de-passar, e com os
   dois previews de dano (`preview_basic_attack_damage`, `estimate_card_damage`) enxergando
   o Atk efetivo do enrage. **Se o preview não enxergar o enrage, o jogo mente para o
   jogador na tela de mira**, e a run mediria um jogo que ninguém joga.
5. **Impressão dos gates com veredicto aplicado e visível**, não só os números crus: para
   cada gate, o valor medido, o critério e o PASS/FAIL. O entregável desta rodada é o
   RELATÓRIO, não o harness.
6. **Seeds determinísticas e pareadas** entre pontos do mesmo cenário, como nas fases
   anteriores (§8, cláusula 5).

### 9.6 Ambiguidades registradas (não resolvidas por conta própria)

Regra da casa: ambiguidade vai para o documento com as opções e o motivo, e a decisão é do
líder. As quatro abaixo foram encontradas ao transpor a spec para este pré-registro e **não
foram resolvidas por invenção de número**.

| # | Ambiguidade | Opções | Adotado provisoriamente aqui |
|---|---|---|---|
| **AMB-01** | Faixa de golpes no provocador: a tabela-resumo da spec (§0) diz *"calibrado para **73-85%**"*, enquanto §2.5 e o gate G1 (§2.8) dizem *"faixa-alvo pré-registrada: **70% a 85%**"* | (a) 70-85% como gate; (b) 73-85% como gate | **70% a 85%** (é a faixa declarada como PRÉ-REGISTRO; o 73-85% de §0 descreve o que a conta analítica do multiplicador 12,0 produz, que é resultado esperado e não régua). **CONFIRMADO na assinatura de 2026-08-11 (líder, via AskUserQuestion).** |
| **AMB-02** | G6 diz apenas *"continua na janela 3-5"*, sem percentual | (a) herdar a régua de E2 (maximizar % na janela, p10 >= 2, p90 <= 7); (b) fixar um percentual mínimo próprio para o P7 | **(a)**, herdar E2 sem régua nova. **CONFIRMADO na assinatura de 2026-08-11 (líder, via AskUserQuestion).** |
| **AMB-03** | H5 (*máximo observado em toda a grade* < 20) é mais estrito que H3 (*p99 no P6* <= 20) e que H4 (cap de 30): se H5 valer, H3 nunca reprova sozinho e H4 tem `k = 0` por construção | (a) H5 é gate de reprovação e H3/H4 seguem medidos e impressos como zero declarado; (b) H5 é observação informativa e a reprovação mora em H3/H4 | **(a)**, com H3 e H4 impressos sempre (inclusive `k = 0`), porque o teto analítico de 15 rodadas depende de premissas que podem cair (§9.4) e, se H5 reprovar, é H3/H4 que dizem ONDE a cauda está. **CONFIRMADO na assinatura de 2026-08-11 (líder, via AskUserQuestion).** |
| **AMB-04** | Total de lutas da C-bis: N por ponto está fixado (300.000), mas a contagem de células muda com P7/P7b e com o braço de enrage | (a) fixar a contagem só quando a grade estiver desenhada; (b) fixar agora um teto de células | **(a)**. A spec estima 3 a 6 minutos para 48 pontos; custo de CPU não é critério de decisão em nenhuma direção |
...
*§4.2 e §9 acrescentadas pelo `lead-game-designer` em 2026-08-11, transpondo
`spec-provocar-soft-enrage-criterio-cap.md` §2.8, §3.8 e §4. **§9 é PRÉ-REGISTRO e aguarda
assinatura do líder**; nenhum número dela entra no motor nem em `combat.md`.*
