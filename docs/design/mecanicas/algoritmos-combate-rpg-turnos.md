...
> **RESGATADO em 25/08/2026 do `gusworld_legacy` (o projeto anterior a LEI ZERO).** Pesquisa consolidada de algoritmos de RPG por turnos (IA de inimigo, atributos, targeting, anti-stall, balanceamento, chefes, validação estatística), declarada pelo próprio texto como **"agnóstica de engine"**. O código de exemplo e as referências a `petrinhu/GusWorld` + `petrinhu/glintfx` datam de antes do reset de código do L-01: nenhuma classe, arquivo ou caminho citado aqui existe. Trate como **referência de design e de algoritmo**, nunca como código a religar. Onde o texto cita `docs/design/mecanicas/combat.md` ou outro doc que já existe neste repositório, o doc vivo vence qualquer divergência.
