...
Nas dungeons, espaço feito à mão descrito em `docs/design/mundo-topologia.md` §4, a regra é restritiva. **Todas as 13 dungeons têm o campo que restringe o save, mas com intensidade diferente por dungeon** (decisão do líder, 21/08/2026, por `AskUserQuestion`):
...
- **Intensidade fraca:** o campo bloqueia só o **autosave** dentro da dungeon; o jogador ainda pode **salvar manualmente na porta**.
...
**O sinal é o próprio menu de save.** O jogador descobre se está sob o campo do PEM olhando o menu: se a opção de salvar aparece disponível ou bloqueada ali é o que comunica o estado, sem uma linha de diálogo ou um aviso explicando a regra. É o jogador quem aprende, jogando, que "dentro da dungeon o menu muda" e que existe algo a fazer sobre isso.

**Em aberto:** a forma exata da UI (ícone, texto do item desabilitado, indicador ambiental além do menu) é implementação e nasce com a camada `present/`, quando o GlintFx tiver janela, texto na tela e tradutor de marcação (**L-06**, **L-27**). Este documento fixa apenas que o veículo do sinal é o menu de save, não que existe algo além dele.
...
1. **A área carregada precisa expor mais que geometria.** O código que representa "onde o jogador está" não pode ser só malha e colisão: ele precisa carregar um dado que diga se aquele lugar é cidade ou dungeon, e, sendo dungeon, se o PEM local está ativo, desativado, ou nunca existiu ali. Essa informação é regra de jogo (camada `domain/`, por L-17), consultada pela UI de save, não decidida por ela.
...
2. **Qual das 13 dungeons recebe qual intensidade (fraca ou total), e com que critério.** A Seção 1.2 fixa que todas as dungeons têm o campo e que a intensidade varia; **qual** dungeon fica em cada intensidade não foi decidido. É balanceamento de dificuldade e, por isso, decisão do líder, inclusive o próprio critério de escolha (dificuldade pretendida, posição na campanha, tipo de dungeon conforme `docs/design/mundo-topologia.md` §4, ou outro).
...
4. **A forma exata do sinal diegético além do menu de save, incluindo como comunicar fraca versus total.** Este documento fixa que o menu é o veículo do sinal (Seção 3); ícone, texto de item desabilitado ou qualquer indicador ambiental adicional não estão decididos e nascem com a camada `present/`. A intensidade variável (Seção 1.2) acrescenta uma exigência a essa forma ainda não desenhada: o sinal precisa comunicar não só "pode salvar" ou "não pode salvar", mas também **qual das duas restrições está em vigor** ali, sem recorrer a tutorial em parede de texto (corte C-16 da L-29).
