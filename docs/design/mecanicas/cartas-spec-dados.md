...
- Forma-alvo referenciada por este doc (descrição de intenção, código ainda não escrito — ver L-01 em `GODS_LAWS.md`):
  ...
  `deck_records.hpp` (`struct CardInstance`),
  `card_collection.hpp` (agregado deck ativo/morto),
  `deck_constants.hpp` (padrão `//PLAYTEST`),
  `save_data.hpp` + `save_migrators.hpp` (save versionado V6, `CardCollectionState`, chain de migrators).
...
- **UI/RunaDex, minigame do adware (X de fechar), tela de diagnóstico do Turing** — `app/`+`platform/` (glintfx), fora de `core/`+`domain/`.
...
3. **`is_infected` nunca é lido cru pela UI antes de `is_diagnosed == true`.** Este é um contrato de FRONTEIRA (domain armazena a verdade; apresentação decide quando revelar), não um campo — não há como "esconder" tecnicamente o valor dentro do próprio struct (não é criptografado nem deveria ser, o save inteiro já é AEAD/HMAC). A obrigação é do consumidor de UI: renderizar `is_infected` como se fosse `false` sempre que `is_diagnosed == false`. Reforçar em code review / teste de UI quando a feature for implementada.
...
- **AMB-DADOS-03 — se `is_burned_out` (falha de cura OU arma-Sterling) EXCLUI a carta do `CardCollection` ativo automaticamente, ou fica "presente mas inutilizável" até o jogador a descartar manualmente:** o doc-fonte (§11) diz "conviver, arriscar remoção, ou descartar no ferro-velho" — sugere que fica no inventário, inerte, até ação do jogador. Assumido como default de leitura, mas vale confirmação (afeta se `is_burned_out=true` numa `CardTier::Especial` viola inv.9 "nunca sai por descarte" — se a carta morre sozinha por queima, isso NÃO é "sair por descarte" no sentido do inv.9, que é sobre a AÇÃO do jogador de descartar; recomendo tratar como estados ortogonais, mas é o líder quem fecha).
  - **RESOLVIDO (líder, 2026-07-28): a carta FICA no inventário, inerte, até o jogador descartar.** O default de leitura da spec foi confirmado, e é o que o doc-fonte §11 já sugeria ("conviver, arriscar remoção, ou descartar no ferro-velho"). Consequência de design que motiva a escolha: **a decisão de conviver com o dano é do jogador**, e some se a carta evaporar sozinha. Consequência técnica: `is_burned_out=true` e a permanência no `CardCollection` são **estados ORTOGONAIS**, como a spec recomendava; queimar não é "sair por descarte" no sentido do inv.9, que fala da AÇÃO do jogador, então uma `CardTier::Especial` queimada continua na coleção sem violar o invariante.
- **AMB-DADOS-04 — reflash de homebrew já possuída:** o doc-fonte descreve a EPROM como "regravável, mas desgasta até queimar" (8 regravações, `cartas-numeros-proposta.md` §2). Este doc assumiu, como default de MVP, que `card_id` de uma `CardInstance` NUNCA muda depois de criada (cada compilação-na-bancada = uma `CardInstance` NOVA), e que "regravações" é textura narrativa/técnica do porquê o EPROM é instável, não uma ação repetível do jogador de trocar o conjuro de uma carta já possuída. Se o líder quiser a ação "levo minha EPROM de volta à bancada e regravo com outro conjuro" como mecânica real, o modelo precisa mudar (campo `card_id` deixa de ser efetivamente imutável, ganha uma API de "reflash" com guard de ciclos restantes) — mudança não-trivial, melhor decidir antes de implementar do que depois.
  - **RESOLVIDO (líder, 2026-07-28): NÃO é mecânica. O default de MVP da spec fica.** As 8 regravações são **textura narrativa e técnica** que explica por que a EPROM é instável, não uma ação repetível do jogador. Portanto: **`card_id` de uma `CardInstance` permanece efetivamente imutável**, cada compilação-na-bancada cria uma `CardInstance` NOVA, e **não** se cria API de reflash nem guard de ciclos restantes. A constante `kEpromRewriteCyclesBeforeBurnout = 8` (§9) fica como número de lore, sem consumidor de gameplay: se um dia virar mecânica, é decisão nova e o modelo muda junto.
