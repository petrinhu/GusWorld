...
2. **O dado existiria no motor, segundo a análise de projeto.** `IntentPreview`
   (`combat_records.hpp:80`) carregaria `predicted_action_id`, `predicted_damage`,
   `predicted_shape`, **`predicted_target_id`** e `is_chaotic`. O `ScriptedBrain`
   já saberia responder quem ele vai atacar.
3. **A tela já saberia pedir.** `BattleScene::intent_for()` (`battle_scene.cpp:658`)
   montaria a `CombatState` e chamaria `preview_intent`. **Só que nada chamaria
   `intent_for` para desenhar**: a função existiria e nunca seria usada no render.

> ⚠️ **Não verificado contra código.** Os itens 2 e 3 acima, com as citações de
> `combat_records.hpp:80` e `battle_scene.cpp:658`, vêm da análise de projeto, não de
> medição em código executado. O código deste projeto nasce do zero (GODS_LAWS.md,
> L-01), e a fonte que a redação original citava não existe. A forma descrita
> continua válida como descrição de intenção; revalidar quando houver implementação.

Conclusão (segundo a premissa acima, não verificada): a peça cara (motor prever a
intenção) estaria pronta. Falta **desenhar** e falta a mira valer a pena ser lida. O
Patch-Zero já tem tratamento canônico previsto (`is_chaotic` → ícone de ruído
embaralhado), então o caso especial também já está pensado.

---

## §1. O sistema de peso (quem o inimigo escolhe)

Segundo a análise de projeto: hoje `scripted_brain.cpp:26` faria
`const CombatActor* target = players.front();`, sempre o primeiro vivo da lista. Foi
isso que, segundo a leitura do playtest, fez o inimigo bater só no Cauã a batalha
inteira.

> ⚠️ **Não verificado contra código.** A citação de `scripted_brain.cpp:26` e o trecho
> de código acima vêm da análise de projeto, não de medição em código executado. O
> código deste projeto nasce do zero (GODS_LAWS.md, L-01), e a fonte que a redação
> original citava não existe. A observação do playtest (o inimigo concentrou os
> ataques em um só personagem) continua válida como fato observado pelo líder;
> revalidar a causa no código quando houver implementação.
