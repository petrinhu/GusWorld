...
- **Duas lacunas abertas pela revisão adversarial de 05/09/2026, ambas registradas e propostas em `docs/narrative/environments/02-selve-sombria.md` §11, não aqui:** (1) o HP com que o zumbi reanima depois de cair sem `SigKill` ativo não estava definido em lugar nenhum, e sem esse número não dá pra calcular o custo de limpar a área; (2) o trunfo do Gödel (`NullProof`, §2.3/§6 acima, que reescreve `multFraqueza`) não tem hoje nenhum caso em que seja necessário contra um zumbi, porque o `SigKill` funciona igual contra qualquer inimigo da área independente de resistência. As duas ficam pendentes de decisão do líder; este documento não resolve nem uma nem outra.
