...
- [`../technomagik.md`](../technomagik.md): o guarda-chuva "TechnoMagik" (grafia canônica confirmada pelo líder em 25/08/2026, fechando o item `TECHMAGIC-CANON` para prosa/lore; o corpo daquele doc, renomeado de `techmagic.md`, foi reconciliado nesta data), a metafísica de 3 camadas, e o Tavus-Eco.
...
- [`cartas-hardware-pirataria-energia.md`](cartas-hardware-pirataria-energia.md): a **camada física/hardware** da carta (ROM/EPROM, memória em **Runa** = r/R, conector **RSB**, bateria CR2032), o sistema de **pirataria / vírus / mercado negro / adware**, a **RunaDex** (índice estilo Pokédex) e o **arco econômico Bastiat** (brainstorm líder 2026-07-18). O anti-pillar acima é **preservado**: o homebrew COPIA um conjuro existente (pirataria = duplicação), nunca cria carta nova numa bancada. **PRINCÍPIO CANÔNICO (líder, 2026-08-08, §3 daquele doc): todo `EffectKind`/mecanismo de USO de carta futuro precisa responder à originalidade da carta (original × pirata × homebrew) E à degradação da bateria (SoH) — as duas variáveis juntas, nunca uma sozinha nem hardcoded para um efeito só.**
...
- **"Compilar" = anexar o modificador no cast.** A compilação acontece em runtime, no momento de jogar a carta (combat.md §7: "modificadores são anexados em runtime, não pré-bakeados na carta"). Essa compilação-no-cast é o craft **da COMUM**: não existe uma segunda etapa de montagem da COMUM fora disso. **(Correção 25/08/2026: até aqui isto era descrito como "o único craft que existe no jogo"; deixou de ser exato. Hoje existe um SEGUNDO craft, de outra natureza — a forja de CÓPIAS PIRATAS de ESPECIAL e SUPER na bancada do mercado negro, sempre inferior ao original, nunca a própria original. Ver `cartas-hardware-pirataria-energia.md` §15 e §2.4 abaixo.)** Isso segue consistente com o anti-pillar do gdd.md no que toca às ORIGINAIS ("cartas são obtidas, não craftadas"): o jogador nunca monta a ESPECIAL ou a SUPER original numa bancada; ele obtém a original pronta, por progresso narrativo, e ao jogá-la em combate (quando aplicável) o Tavus-Drive resolve a compilação do modificador. O que ele pode montar na bancada é sempre uma imitação inferior, nunca o original.
...
- **Baseline numérica (framework fechado pelo criador 2026-07-12, via economy-designer; ancorada em Trash 55 HP / Def 8, Atk médio de companion ~10, fórmula combat.md §11, TTK-alvo trash 3-5 turnos):**
  ...
  | Arquétipo | ChargeCost | Power / Magnitude | Nota |
  |---|---|---|---|
  ...
  O range TTK 3-5 emerge sozinho variando tier × fraqueza (nada forçado). Os Powers 3/5/8 caem na mesma escada numérica velada do HP 34/55/89/144. Modificadores continuam somando por cima (Object +1 / Stream +2 / Null +1, combat.md §8). **FRAMEWORK:** as 3 faixas de ChargeCost + a derivação de Power a partir do HP-Trash. **AJUSTE FINO (playtest N=3):** os valores 3/5/8 podem mexer ±1-2; a proporção entre tiers é o que se mantém.
...
- **Custo de Mana premium (arcabouço fechado pelo criador 2026-07-12, via economy-designer):** **`ChargeCost` fixo ≈ 6** (faixa aceitável 5-7; número exato dentro da faixa fica pra playtest). Racional: `chargeDrawMax = 2 + turnoIndex`, então 6 destrava por volta da **rodada 4**, gastando quase todo o pool no turno (tensão real: jogar a especial = não sobra mana), alcançável em elite/mini-boss/boss onde ela deve brilhar, sem virar troféu que nunca dispara. ⚠️ **Este racional foi calibrado sobre um teto de vazão fixo; com a decisão do líder de 02/09/2026, o teto passa a depender do CCA efetivo da bateria em uso, e a rodada em que a especial destrava também passa a depender dela: RECALIBRAÇÃO PENDENTE do economy-designer** (`cartas-hardware-pirataria-energia.md` §5). Anti-abuso já coberto por 3 travas independentes: 1×/batalha + sem modificador + mana sem carry-over (impossível bankar pra abrir com ela no turno 1). ⚠️ **Volta é exceção deliberada à trava 1×/batalha (decisão do líder, 31/08/2026):** a carta é 1×/**bateria**, não 1×/batalha — repetível na mesma batalha enquanto houver bateria disponível. Detalhe em `docs/design/mecanicas/cartas/volta.md`. Trunfos-fora-da-roda (Gödel etc.) NÃO recebem taxa de mana extra por padrão; só reavaliar (+1) se o playtest mostrar que a versão fora-da-roda supera sistematicamente a de-dentro contra Fraco.
- **Quatro sub-categorias (economy-designer + criador 2026-07-12; 4ª adicionada 2026-07-14 por PS-Y1):** ao statear as 20, elas se dividem, e o custo depende disso:
  ...
  - **PASSIVA (dois gastos de mana/bateria, equip-only):** sempre ligadas enquanto ocupam 1 slot do Codex; nunca têm custo zero, carregam um gasto MENOR e contínuo para ficar ativas em standby, e um gasto MAIOR no momento em que o efeito é exercido (decisão do líder, 02/09/2026, revoga a leitura anterior de custo zero; números pendentes do `economy-designer`, ver `cartas-hardware-pirataria-energia.md` §5, "Passiva tem dois gastos: standby e disparo"). Além dos dois gastos, o "custo" delas também inclui o SLOT ocupado. NÃO competem com o "1×/batalha". Fiel ao fenômeno real (a Gaiola de Faraday é proteção estrutural, não uma ação disparada). Ex.: Ada, Pythagoras, Hayek, Mises, Bastiat, Planck, Turing (lista atualizada 2026-07-14, achado PS-G3; ver `_EFEITOS-ESCOLHIDOS.md` pra a classificação final das 20). **Gödel saiu desta lista em 2026-07-15** — virou ATIVA (ChargeCost 0, mas castável, não equip-only), ver nota abaixo.
  ...
  - **HÍBRIDA (decisão do criador 2026-07-14, achado PS-Y1):** cartas com um efeito PASSIVO sempre-ligado + um efeito ATIVO no MESMO card. Ocupam **1 slot** do deck de combate. O **passivo fica sempre ligado** enquanto equipada (NÃO sofre o limite de 1×/batalha); a **ativa** paga o mana premium ~6 e obedece o **1×/batalha** normal. Modela as cartas de efeito duplo: **Maxwell, Newton, von Neumann, John Dee** (ver `_EFEITOS-ESCOLHIDOS.md`). No `Card` record isso é um flag/composição (passivo + ativo coexistem), não uma carta a mais. **A face passiva também carrega os dois gastos de mana/bateria, standby contínuo e disparo, como toda passiva (decisão do líder, 03/09/2026, `cartas-hardware-pirataria-energia.md` §5); números pendentes do `economy-designer`.**
...
- **Carta ORIGINAL, nunca forjada (correção 25/08/2026, decisão do líder):** a SUPER não é criada nem montada por ninguém, nem pela party, nem por Tusk **na bancada**. Ela é a peça original, escrita em **asmódico puro**, num **compilador próprio que se perdeu ao longo do tempo**. Nenhuma bancada a gera: a party não a fabrica combinando as 20 ESPECIAIS como componente de um ritual de criação. **As 20 ESPECIAIS coletadas continuam sendo pré-requisito duro**, mas como condição de **acesso** à missão-capstone, não como matéria-prima que vira a carta.
- **Autoria (fechada 25/08/2026, rodada 3, decisão do líder):** foi o próprio **Helion Tusk quem escreveu a carta**, em asmódico puro, e quem escreveu o **compilador** dela, em asmódico e C-Arcane. O compilador não se "perdeu" por acidente: **Tusk mesmo o apagou**, ato deliberado, ao medir o perigo que ele seria pro GusWorld se caísse nas mãos de **Patch-Zero** ou de **Sterling Locke**. Isso fecha o "compilador próprio que se perdeu ao longo do tempo" do bullet acima como ato do próprio autor, não como perda por desgaste. **Não muda nada do resto**: lugar e caminho da carta escondida seguem abertos (bullet seguinte; o mecanismo de entrega foi fechado depois, mesmo bullet), e Tusk **não é longevo**: a menção do líder à poção de Brunus Vetorial nesta mesma rodada foi tema ("ética acima da fama"), não mecanismo biológico compartilhado; a regra de que Brunus foi a única cobaia do próprio experimento continua intacta. Detalhe completo em `docs/design/roster-analogos/21-helion-tusk.md` §"O clímax"; tema registrado em `docs/narrative/arco-principal.md` §"Theme central".
- **Foi Tusk quem a escondeu, e não é na Forja do Vértice (princípio fechado 25/08/2026, rodada 2, decisão do líder):** perguntado entre "na Forja do Vértice", "escondida pelo próprio Tusk em outro lugar" e "o líder diz onde", a resposta foi a segunda opção, verbatim: "opcao dois, mas ainda precisamos pensar onde, como chega etc". Isso fecha que o esconderijo é ato deliberado do próprio Tusk, e que a Forja do Vértice está descartada como local. **O lugar exato e o caminho até lá continuam em aberto por decisão explícita do líder** (a redação anterior descrevia um mecanismo de forja que caiu junto com a reversão de 25/08/2026, ver `cartas-hardware-pirataria-energia.md`). Nenhum agente deve preencher lugar ou caminho por conta própria. Detalhe completo do princípio e do que segue aberto em `roster-analogos/21-helion-tusk.md`, seção "O clímax: a chegada até a Carta 21".
- **O mecanismo pelo qual a party recebe a carta foi fechado em 03/09/2026, decisão do líder por `AskUserQuestion`:** manter o canon já escrito, as vinte peças especiais são a chave, a carta está inteira. As vinte peças que dão acesso à missão-capstone são a CHAVE que abre o caminho até a SUPER, nunca matéria-prima que a compõe. A carta é original, inteira, escrita de próprio punho por Helion Tusk num compilador que ele mesmo apagou; ninguém jamais a teve em punho tempo bastante para examiná-la, a mesma razão que trava a engenharia reversa dela (bullet "Cópias piratas", abaixo). Mecanismo completo, com a cena do encaixe das vinte peças na bancada emprestada de Helion Tusk, em `docs/narrative/deep/characters/mestre-cap-21-helion-tusk.md` §6 (não repetido aqui, L-30/L-37 do projeto) e em `cartas-hardware-pirataria-energia.md` §15.6.
- **Cópias piratas existem, e são sempre inferiores (decisão do líder, 25/08/2026):** quem tenta copiar a SUPER produz sempre algo pior que o original — qualidade inferior, efeitos trocados, ou só o nome sem função nenhuma. Engenharia reversa da SUPER é impossível: a carta original nunca esteve disponível para exame, então não há mecanismo a entender e imitar. As melhores cópias piratas não passam de **20% de fidelidade**. Tratamento completo em `cartas-hardware-pirataria-energia.md` §15.
...
O set canônico de `combat.md` §9 (Stun / Poison / Corrode / Disrupt / Silence / Knockback / Break / Expose / Decrypt / Shield / Regen / Haste / Slow) **fica intacto**, sem alteração. Dois status novos entram, ambos ligados à metáfora "magia é tecnologia" (Pillar 2, camada 1 da metafísica de `technomagik.md`), evitando body-horror.
...
- **Arcabouço fechado (criador 2026-07-12, via economy-designer):**
  ...
  - **Trava anti-loop (framework, combat.md §15):** `ChargeCost(carta de Resfriamento) ≥ desconto × Duration`, ou seja, o card custa no mínimo o que economiza ao longo da própria vida (senão recastar em loop viraria lucro líquido de mana). Com -1 × 3 = 3, **o card custa 4** (não o breakeven exato 3: +1 garante que reforjar em loop nunca é neutro nem lucro). Mana sem carry-over já impede bankar.
  ...
  - **Ajuste fino (playtest):** se +1 SPD / -1 mana é sentido como impactante o bastante, e o `ChargeCost` exato do card (3 vs 4).
...
- **Custo (`ChargeCost`):** **3**, ancorado no teto da faixa COMUM já fechada em §2.2 acima ("Dano single-target forte" e "Control" ocupam 2-3; 3 é o topo desse degrau, um abaixo do território ESPECIAL ~6). Justificativa da ancoragem: o efeito não causa dano nem tira turno no momento em que é aplicado (pode desperdiçar, se a janela expirar sem a queda acontecer), então um custo baixo destruiria a decisão de quando aplicá-lo; o teto do comum preserva a tensão sem empurrar a carta para o patamar de trunfo de mestre.
  - **Revisão pillar-first, 05/09/2026 (`lead-game-designer`), pergunta ao líder, não fechada aqui:** essa ancoragem supunha uma área limpável sem a carta (a proposta de fração de HP, revogada em `02-selve-sombria.md` §11); com a reanimação ilimitada agora fechada, o `SigKill` deixou de ser um acelerador tático opcional e virou a única saída de combate do Pântano de Markov, repetida a cada zumbi da área. Cobrar o teto da faixa COMUM (3, ou 6 com `Stream`) de um efeito de progresso obrigatório e repetível contraria o próprio princípio que justifica esse teto: "sem estratégia obrigatória, sem vala da morte" deveria valer também para custo de mana, não só para presença de carta. **Conclusão deste agente:** o custo deveria descer para o chão da faixa (1-2), coerente com a régua de "risk vs reward" quando não sobra escolha real de não usar a carta; o agravamento do `Stream` (+3 em vez de +2) merece a mesma revisão pelo mesmo motivo, já que limpar vários zumbis de uma vez também deixou de ser opcional em áreas com mais de um. **Não decido o número aqui:** devolvo ao líder e ao `economy-designer` para recalibrar dentro do arcabouço já fechado (§2.2), com playtest do Pântano de Markov como critério final.
...
- **Lacuna (1), fechada em 05/09/2026, decisão do líder:** a reanimação dos zumbis do Pântano de Markov é ilimitada; sem `SigKill` (ou equivalente futuro) o zumbi não fica caído por número de golpes nenhum, e reanima sempre com o HP cheio de novo. Isso substitui a leitura anterior desta seção, que dizia faltar o número de HP de reanimação: não existe fração residual, e a área não é limpável só à base de dano. Detalhe em `docs/narrative/environments/02-selve-sombria.md` §11.
- **Lacuna (2), aberta pela revisão adversarial de 05/09/2026, proposta refinada no mesmo dia, decisão segue com o líder:** o trunfo do Gödel (`NullProof`, §2.3/§6 acima, que reescreve `multFraqueza`) continua sem nenhum caso em que seja necessário contra um zumbi, porque o `SigKill` funciona igual contra qualquer inimigo da área independente de resistência, e a reanimação ilimitada da lacuna (1) torna isso mais grave, não menos. Caminho proposto (zumbi-elite Imune à família Sônico, mais a lacuna de engenharia que essa proposta expõe no `NullProof`, mais o cuidado para não travar a área sem saída) registrado em `docs/narrative/environments/02-selve-sombria.md` §11, não repetido aqui.
...
- **Custo de Mana premium das cartas ESPECIAIS** (§2.3): **ARCABOUÇO FECHADO 2026-07-12** (via economy-designer) = `ChargeCost` fixo ≈ 6 (faixa 5-7, destrava rodada 4). Só o número exato dentro da faixa + a taxa dos trunfos-fora-da-roda ficam pra playtest.
...
- **Baseline numérica das cartas COMUNS** (§2.2): **FECHADA 2026-07-12** (via economy-designer) = ChargeCost 1/2/3 → Power 3/5/8 + arquétipos DoT/control/buff, ancorada em Trash 55 HP. É a régua que destrava tudo. AJUSTE FINO no playtest.
- **Statlines das 20 cartas ESPECIAIS:** o ARCABOUÇO está fechado (§2.3: 3 sub-categorias — ativa mana~6 / passiva com dois gastos de mana/bateria, standby + disparo, equip-only (decisão do líder, 02/09/2026) / fora-de-combate Faraday+Euler+Menger). Mas o **statline final por-carta DEPENDE de escolher o efeito A/B/C de cada mestre** (hoje os docs `roster-analogos/01..20` só RECOMENDAM uma opção; não é decisão do criador ainda). Logo o statline por-carta é **DIFERIDO até a feat de escolher os 20 efeitos** (feat do roster + criador). Existe uma tabela-rascunho provisória do economy-designer (2026-07-12, assumindo as opções recomendadas) como ponto de partida, NÃO canônica.
  - **Volta (regra de recurso) — RESOLVIDO POR COMPLETO em 31/08/2026** (efeito escolhido em `_EFEITOS-ESCOLHIDOS.md`, 2026-07-14 achado PS-Y4; especificação numérica fechada pelo líder em 31/08/2026, por `AskUserQuestion`, verbatim: *"o volta fica como decidi agora, achei melhor"*): o Volta é **leech termodinâmico** — dreno de **21% da energia do ALVO por uso** (porcentagem do alvo, não valor absoluto), **55% do drenado convertido à party**, os outros **45% perdidos como calor** (2ª lei); no uso, o jogador escolhe se o retorno vira mana ou vida, escolha cronometrada por dificuldade (Fácil 13s / Médio 8s / Difícil 5s / Hardcore 3s), e se o tempo esgota sem escolha a energia dissipa e ele não recebe nada. NÃO fura o "sem carry-over" (é dreno do inimigo, não retenção de mana própria). O exemplo antigo "mana não-gasta vira Shield" está DESCARTADO. **`VOLTA-LEECH-%` fecha aqui** — especificação completa em `_EFEITOS-ESCOLHIDOS.md` linha 14. ⚠️ **Revoga, a partir de 31/08/2026,** a leitura extraída do código do projeto anterior (`docs/design/mecanicas/cartas/volta.md`, `resources/cards/volta.gw.card`, L-01): gatilho `OnDamageDealt` sobre fração do DANO CAUSADO, `kVoltaLeechPercent = 50` — mecanismo diferente do decidido, morto.
...
- **Dados faltantes (bloqueiam número fino, não a forma):** Atk/SPD do Elite (Daemon-Guard) e demais inimigos (TBD combat.md §17); tag de inimigo "comando central" (efeito do Mises) — o CAMPO/motor está **ENTREGUE** (ver `ApEfficiency (Mises)` abaixo); a CURADORIA de quais inimigos canônicos levam a tag segue pendente como design/lore do criador (fora do motor).
...
- **Primitivas de engine que os efeitos escolhidos EXIGEM (achado PS-Y2, decisões do criador 2026-07-14):** os efeitos de `_EFEITOS-ESCOLHIDOS.md` pedem 3 primitivas ainda ausentes do motor; entram no backlog de `gameplay_engineer`:
  ...
  - **DiversityBonus (Hayek / Free-Order, "ordem espontânea") — ENTREGUE (ADR-016 addendum, CARD-ENGINE-MANIFESTO item 7, decisões do líder 2026-07-16; ver AMB-09 em `_EFEITOS-ESCOLHIDOS.md`):** passiva (dois gastos de mana/bateria, standby + disparo, números pendentes, decisão do líder, 02/09/2026), equip-only, Universal, **NUNCA pune, só premia** (mult de dano ≥ 1.0; limiar de falha só diminui, piso 0). MARCADOR fora do dispatcher (mesmo padrão `DamageQuantize`/Planck): a carta é 1 `EffectSpec` por DEGRAU (`magnitude`=limiar de assinaturas distintas, `percent`=bônus% de dano, `duration`=redução pp do limiar de falha), lidos DIRETO pelo resolvedor via `combat_state_machine.cpp::diversity_spec_of`. **Métrica de "ação diferente" (M2):** `CombatActionType` da ação, mas `UseCard` é REFINADO pela `CardFamily` da carta jogada (2 cartas da mesma família = 1 assinatura; famílias diferentes = 2). **Ledger novo `round_actions_`** (`techMagic::RoundActionEntry`, granularidade de AÇÃO via `resolve_action` — NÃO de golpe, primo do `round_hits_` do Pythagoras), zerado na mesma fronteira de rodada. **Forward-only:** a ação em resolução conta as assinaturas distintas já registradas pelo lado dela ANTES dela mesma, e SÓ DEPOIS entra no ledger (anti auto-inflação — a 1ª ação da rodada nunca se autobeneficia). **Degraus (`//PLAYTEST`):** 2 distintas = +5% dano / −1pp falha; 3 = +8% / −2pp; 4+ = +13% / −3pp (cap automático). **Beneficia o LADO INTEIRO** enquanto o dono estiver vivo e com a carta equipada, não só o dono. `mult_hayek` entra como o ÚLTIMO fator da cadeia divisiva (§6/combat.md §11); ataque básico usa `lround(raw × mult_hayek)`. Gêmeo preview↔real (`estimate_card_damage`/`preview_basic_attack_damage`) e determinismo de RNG (a passiva NUNCA sorteia nada) comprovados por teste. Catalogação em `MasterCards`/i18n diferida pra leva futura.
  - **ApEfficiency (Mises / Calc-Edge) — ENTREGUE (ADR-016 addendum, CARD-ENGINE-MANIFESTO item 9, decisões do líder 2026-07-16; ver AMB-10 em `_EFEITOS-ESCOLHIDOS.md`):** passiva **DUPLA** (dois gastos de mana/bateria, standby + disparo, números pendentes, decisão do líder, 02/09/2026), equip-only, Universal, num **ÚNICO `EffectKind`** (a 2ª face é comportamento detectado por PRESENÇA — carta equipada × tag no ator —, não um 2º kind). MARCADOR fora do dispatcher (mesmo padrão `DamageQuantize`/Planck e `DiversityBonus`/Hayek): `handle_ap_efficiency` é no-op; as duas faces plugam DIRETO no motor. O `EffectSpec` único carrega os dois parâmetros: `magnitude`=+AP da face 1; `percent`=desconto% de dano da face 2 (`kMisesAimError`=13, `//PLAYTEST`). **Face 1 ("party aloca melhor") = +1 AP só pro DONO** (não party-wide — seria forte demais), concedido em `begin_turn` DEPOIS de `refresh_resources_for_turn` (via `CombatActor::grant_bonus_ap`); NÃO muta `max_ap_`, o bônus NÃO persiste (o próximo `begin_turn` reseta). **Tag "comando central" = campo booleano no ATOR** (`CombatActor::central_command()` ↔ `EnemyTemplate::central_command`, aditivo no FINAL da struct), não um `EffectKind`/status — é PROPRIEDADE do ator (mesmo padrão de `is_boss`). A curadoria de quais inimigos canônicos levam a tag é design/lore do criador (bate na axiologia: "comando central" = lado ruim); o bestiário fica intocado, só templates de TESTE ganham a tag. **Face 2, atraso = reordenação de fila** (não perda de ação): 2ª `stable_partition` em `regroup_round_by_side` (mesma primitiva `regroup_stable` do regroup-por-lado) empurra os taggeados pro FIM do bloco do próprio lado, SÓ quando algum vivo do lado OPOSTO porta a Mises equipada; determinístico, na fronteira de rodada, não briga com Gambito/Einstein/Knockback. **Face 2, erro de mira = desconto FIXO determinístico, NÃO "chance de errar"** (chance% consumiria RNG e quebraria a paridade de contagem do canal COMUM): `mult_mises = max(0, 1 − percent/100)` entra como o ÚLTIMO fator da cadeia divisiva (por cima do `mult_hayek`) e escala o `raw` no ataque básico — gêmeo preview↔real nos dois. Determinismo de RNG idêntico com/sem Mises comprovado por `CountingRandom`. **NOTA DE RISCO (serializer):** `central_command` é o 1º campo do manifesto a tocar o formato binário `.gdt` (`u8` append-only no fim do payload, padrão do `EnemyKind::kind`); o formato NÃO tem discriminador de versão no envelope — gap sistêmico pré-existente, seguro HOJE porque não há nenhum `.gdt` persistido em disco/repo, mas quando o jogo persistir `.gdt` real (assets moddáveis `user://`) vai precisar de campo de versão + migrator antes do próximo campo aditivo. **Catalogação JÁ FEITA** (diferente do Hayek): `master_cards.cpp::mises` (registry 16→17), `master_cards_test.cpp`, i18n `CARD_EXEC_MISES_NAME` = "Calc-Edge" (pt_br/en_intl) — "não deixar a carta fora do jogo".
  - **Clone (von Neumann/Fork + Giordano Bruno/Echo-Self) — ENTREGUE, mas com MUDANÇA DE ROTA vs. a decisão original de 2026-07-14 (CARD-ENGINE-MANIFESTO item 8, ultimo item do manifesto; ver AMB-11 em `_EFEITOS-ESCOLHIDOS.md` e o addendum "CARD-ENGINE-MANIFESTO item 8" em `ADR-016`):** a decisão original (linha logo abaixo, histórico) previa "entidade-Objeto" — um 4º ator/objeto na fila. O brief de implementação (2026-07-16) mudou pra **STATUS-DRIVEN**: `StatusId::Eco` aplicado no ALIADO clonado (self incluso — Bruno é self-only), **sem nenhum ator novo na fila** (Party=3 fixo, §2, continua intacto — respeita o canon do líder sobre a fila de turnos). Ao FIM de CADA turno PRÓPRIO do portador do Eco, o motor reaplica os hits da ÚLTIMA AÇÃO DE DANO dele mesmo nesta rodada, a uma % fixa (`von Neumann` 50%/3 turnos; `Bruno` 62%/2 turnos, `//PLAYTEST`), via `take_damage` PURO (mesma anti-recursão de `RepeatLastAction`/`Reflect`/`HypotenuseCombo` — testado em composição com Ada, `techmagic_clone_test.cpp` seção 6, sem dobrar/recursar). **DECISÃO AUTÔNOMA A CONFIRMAR retroativamente:** se "entidade-Objeto" for entendido pelo líder como um alvo-atacável LITERAL (não uma metáfora de invocação), esta implementação precisa de revisão. O **visual continua OBRIGATÓRIO** (sprite-eco/fantasma translúcido num slot de batalha, atrelado à presença de `StatusId::Eco` no ator) — responsabilidade da camada de apresentação (glintfx), NÃO implementado no domain/ (vira prompt pro dev do glintfx). Cai no bug conhecido **HUD-STATUS-ICONS-STALE** (INBOX) pro ícone de status — não corrigido nesta leva, só registrado. Von Neumann também porta `EffectKind::TokenRefund` ("Construtor Universal" — refund automático 1x/batalha da 1ª especial Ativa/Híbrida jogada, marcador fora do dispatcher, mesmo padrão de `DamageQuantize`/`DiversityBonus`/`ApEfficiency`).
  - A decisão original de 2026-07-14 (entidade-Objeto reusando o sistema de modificadores/invocação) foi superada pela implementação status-driven descrita acima; seu texto foi apagado (L-24 do projeto). O requisito de visual obrigatório e a fila de turnos Party=3 intacta sobrevivem, incorporados acima.
...
- **Reconciliação da grafia "techMagic" → "TechnoMagik" — CONCLUÍDA (líder, 25/08/2026):** grafia canônica confirmada como **TechnoMagik**; corpo de `technomagik.md` (renomeado de `techmagic.md`) e o restante do corpus vivo normalizados nesta data (item `TECHMAGIC-CANON` fechado para prosa/lore). O nome do símbolo de código do executor segue pendente, fora de escopo desta rodada (não há código ainda, L-01).
- **Reconciliação da raiz Sylvarin de "Morenh"** (§8.2, nota) com o léxico-semente formal.

---

**Última revisão:** 2026-07-12, rodada 3 (via economy-designer + criador): baseline COMUM fechada (§2.2, ChargeCost 1/2/3 → Power 3/5/8); 3 sub-categorias das ESPECIAIS (§2.3, ativa/passiva/fora-de-combate); números dos 2 status destravados (§5, Sobrecarga 8/5/2, Resfriamento card 4). Statlines por-carta das 20 = rascunho provisório (`cartas-statlines-rascunho.md`), DIFERIDO até a escolha dos efeitos A/B/C do roster.
