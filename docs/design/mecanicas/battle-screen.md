...
|---|---|
| Relógio de ação por ator (`ActionClock`, `ADR-017`; reconciliado 25/08/2026, ver `combat.md §4`) | faixa do topo |
...
Os atores na arena side-view sao **sprites 2D puros**, gerados direto no **PixelLab** (mesmo pipeline dos 37 icones + 11 retratos ja produzidos). NAO ha bake 3D: o jogo e 2D-only (style-guide); o 3D era so ferramenta de bake e foi aposentado no pivot (os .glb do art-spike foram deletados). Consequencias:
...
- **D4 Relógio visível (reconciliado 25/08/2026 com `ADR-017` / `combat.md §4`, decisão do líder):** não existe mais fila ordenada por SPD; a faixa mostra os **5 atores mais próximos de ficar prontos** no relógio de ação (ordenados pelo `next_action_at` projetado, `combat.md §4`), celula = **retrato 48px** (asset nativo, sem downscale) + marca de "proximo" no 1o (menor `next_action_at`). SEM nome, SEM mini-barra (a barra de HP vive sob o ator na arena). Preparada pra o marcador "interpretando..." (cast LENTO, CARTAS-CAST-TIME) ocupar uma celula como ator-fantasma (resolução agendada no mesmo relógio, `combat.md §4`); se resolver alem de 5 casas, a 5a celula marca "+N". ⚠️ Ancoragem/render (barra de carga + LED por ator sobre a cabeça, descrita no `ADR-017`) é intenção de design, não spec executável hoje: `present/` ainda não existe (L-06, L-27).
...
- **D7 Camera + dano (reconciliado 25/08/2026 com `ADR-017` / `combat.md §4`, decisão do líder):** **camera estatica** (sem zoom/pan a cada turno; o relogio de acao troca de ator com frequencia, pan enjoa e esconde a faixa de proximos/intents), so um **highlight/seta no ator ativo**. Dano = **numero flutuante** sobre o alvo (sobe + fade ~700ms), **cor por canal** (combat.md par.11): COMUM claro, CRIT ciano + bold, FALHA dano-0 vermelho-erro `#F43F5E`, CURA verde com `+N`. Cinematografia de golpe especial = pos-M5.
