...
> ⚠️ **AVISO DE FRESCOR (2026-08-12, QA de confirmação de `MIRA-PONDERADA-PROD`): os números
> desta seção estão DESATUALIZADOS para fins de tuning.** Vitória 82,6%, Gus cai 17,4%,
> turtle-cap 2,9% e repetição de alvo 32% (tabela abaixo) foram medidos ANTES de
> `BAL-STATLINES-APLICAR`, com o trash em Atk 10. Com o statline canônico vigente (trash
> Atk 12, aplicado 2026-08-08), a revalidação registrada no próprio `MIRA-PONDERADA-PROD`
> (N=30.000/braço, seed sincronizada) encontrou vitória caindo para **~46,6%** (Gus cai
> ~53,4%, turtle-cap caiu a 0%), com a repetição de alvo praticamente intacta (~32,3%) — ou
> seja, o ALGORITMO de mira porta certo; a queda é 100% explicada pelo Atk maior, não pela
> fórmula. **Não usar os números originais abaixo para decidir tuning.**
>
> Fonte vigente, re-medida a N=240.000-300.000 por ponto sob o statline atual e a MESMA
> fórmula de produção (braço titular `F_NoF4` = Opção F): `analise-pacing-fase-a-bis-
> 20260810.md`, `analise-pacing-fase-b-20260811.md`, `analise-pacing-fase-c-20260811.md`
> (mais C-bis/C-ter de 2026-08-12, que ajustam `kMiraProvokeMultiplier` de 12,0 para 10,5).
>
> **Ressalva que essas cinco análises repetem sem exceção, e que vale também para ESTE
> documento:** a mira ponderada aqui descrita ainda NÃO está ligada em produção —
> segundo a análise de projeto, `scripted_brain.cpp` miraria `players.front()` (o
> primeiro da lista), e `pick_weighted_enemy_target`/`mira_target_weight` não teriam
> nenhum call site de produção fora dos testes. Todo número desta seção (e das cinco
> análises de pacing) descreve "o jogo COM a mira planejada ativa", não o jogo como
> ele roda hoje. Ver `TODO.md` item `MIRA-PONDERADA-PROD`.
>
> ⚠️ **Não verificado contra código.** Esta afirmação sobre `scripted_brain.cpp` e
> `pick_weighted_enemy_target`/`mira_target_weight` vem da análise de projeto, não de
> medição em código executado. O código deste projeto nasce do zero (GODS_LAWS.md,
> L-01), e a fonte que a redação original citava não existe. Revalidar quando houver
> implementação.
