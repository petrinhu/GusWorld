...
> ⚠️ **Não verificado contra código.** Toda esta seção descreve uma implementação
> (wiring, arquivos de teste, contagem de casos e assertions) contra código-fonte que
> não existe neste projeto. O código deste projeto nasce do zero (GODS_LAWS.md,
> L-01), e os arquivos que a redação original citava (`combat_state_machine.cpp`,
> `combat_records.hpp`, `combat_actor.cpp`, `cartas_comuns_engine_test.cpp`) não
> existem. A especificação de comportamento abaixo (regras, fórmulas, nomes de campo)
> continua válida como descrição de intenção; revalidar contra código quando houver
> implementação real.

- **SynergyStatus (Finalizador Opção A):** `Card::synergy_statuses`/`synergy_percent` generaliza o `multExpose` (`combat.md` §9/§11) pra qualquer `StatusId`; +40% fixo (`//PLAYTEST`, data-driven, sem hardcode) quando ≥1 status da lista está presente no alvo, sem stack por-status. Wiring descrito em `resolve_use_card`/`estimate_card_damage` (`combat_state_machine.cpp`), campos em `Card` (`combat_records.hpp`). Log diegético `[SINERGIA: alvo vulnerável, dano +N%]`. Paridade com Quantum-Lock/Planck (degraus sobre a base já amplificada) descrita, não medida.
- **Recarga de AP/mana (Elétrico-utilidade "Tavus-Overclock"):** `Card::restore_ap`/`restore_mana`; trava 1×/turno (`CombatActor::overclock_used()`, resetada no refresh de `TurnStart`, `combat_actor.cpp`); custo sempre pago antes; Silence bloqueia; log de sucesso e de bloqueio.
- Testes descritos pela redação original: `cartas_comuns_engine_test.cpp` (16 casos, 51 assertions): sinergia (dispara/ausente/não-stack/gêmeo extremos/gêmeo crítico/contagem RNG/interação Planck/log), recarga (persistência do bônus de AP/clamp de mana/reset da trava/1ª e 2ª jogada/reset no turno seguinte/custo pago antes/Silence).
- Status: 🔍 Pendente verificação (item CARTAS-COMUNS-ENGINE do TODO.md). Pré-requisito da impl das 30 comuns (CARTAS-PRODUCAO) cumprido, segundo a redação original.
...
- **Decisão do líder, 01/09/2026:** a frequência de aparte **em batalha** dispara por **evento**: o jogador joga uma daquelas cartas específicas, e o Gus comenta logo depois sobre aquela carta; depois disso ele fica **3 rodadas de batalha** em silêncio antes de poder falar de novo, para não falar sem parar se o jogador jogar várias cartas seguidas (`gus-apartes-c-arcane.md`, "Regras de disparo" e AMB-02). A trava é em **rodada**, não em minuto: diferente da regra fora de combate (shuffle bag + cooldown 8-10 min no mapa), que continua valendo sem mudança.
