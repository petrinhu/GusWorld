...
- **A chance é ÚNICA por run (decisão do líder 2026-07-10):** o puzzle só aparece na PRIMEIRA vez que HP=0 no Hardcore. Sobreviveu ao puzzle uma vez → se morrer DE NOVO depois, NÃO há puzzle: **permadeath direto**. Impl (Fase 4): flag `puzzle_used` na **ÂNCORA selada** (4º campo, junto do `run_dead` — decisão do líder 2026-07-10), setada ao resolver — a partir daí HP=0 = fim. É o que a copy do aviso comunica ("última chance de retorno", chance única).
...
**Nível de detalhe desta spec:** aqui fica só o DESIGN (o quê + por quê + o trade-off). Os detalhes de CRYPTO/implementação (derivação da chave de máquina, formato exato da âncora out-of-band incluindo o bit de unlock, mecanismo por SO, quais bytes exatos do envelope contam como "selo/cabeçalho" pro wipe por trechos) ficam pro **`security-engineer`** (cifra AEAD + machine-bind + âncora anti-rollback + wipe por trechos, com a primitiva criptográfica vinda do GlintFx — L-25), na hora de **construir** o Hardcore — que só é buildável pós-Difícil jogável (Fase 4, §6), não agora.
...
1. **Unlock do Hardcore:** mora dentro da âncora selada+machine-bound (§2.3b) — não em flag soft/editável (rejeitado pelo líder: furaria a integridade do Hardcore). É a opção recomendada, mas **ainda é um ponto que volta pro líder confirmar** (não é 100% fechado).
...
4. **O gatilho "Dormir" (Marco 2, §2.4), PARCIALMENTE FECHADO pelo líder em 25/08/2026: dormir é mecânica própria, não beat narrativo.** Razão do desenho: dormir é o Marco 2 desta mesma escada de recuperação (§2.4: 5% no respawn, 34% ao voltar à cidade, **89% ao dormir**, 100% no Hospital ou por beat narrativo). Se dormir fosse beat narrativo, os Marcos 2 e 3 colapsariam no mesmo gatilho e a escada perderia um degrau.
...
6. **Mecanismo exato da âncora out-of-band, do bit de unlock dentro dela, e da chave de machine-binding (§2.3)** — o QUÊ e o PORQUÊ estão fixados nesta spec (decisão do líder), o COMO fica a cargo do `security-engineer` (cifra AEAD, com a primitiva vinda do GlintFx — L-25) — não é uma decisão que eu (lead-game-designer) deva ou possa fechar sozinho.
