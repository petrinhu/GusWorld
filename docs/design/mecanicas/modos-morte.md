...
- **A chance é ÚNICA por run (decisão do líder 2026-07-10):** o puzzle só aparece na PRIMEIRA vez que HP=0 no Hardcore. Sobreviveu ao puzzle uma vez → se morrer DE NOVO depois, NÃO há puzzle: **permadeath direto**. Impl (Fase 4): flag `puzzle_used` no estado selado do Hardcore (âncora/save, ADR-015), setada ao resolver — a partir daí HP=0 = fim. É o que a copy do aviso comunica ("última chance de retorno", chance única).
