...
| 3 | Contaminação — Comum (original) | **1%** | escada numérica |
| 3 | Contaminação — Pirata especial | **8%** | escada numérica |
| 3 | Contaminação — Pirata comum | **21%** | escada numérica |
| 3 | Contaminação — Homebrew | **55% FIXO — FECHADO PELO LÍDER, não escala por dificuldade** (§8.2) | escada numérica |
| 3 | Propagação por cast (carta já infectada) | **13%** | escada numérica |
| 4 | `urandom` original — pesos (fraco/médio/forte/especial) | **21 / 34 / 21 / 8** (inalterado) | escada numérica |
...
| 5 | Adware Sterling — tempo até o X | **5 segundos** | escada numérica |
...
**Tabela de pesos por classe** (pesos relativos da escada numérica — 1/3/5/8/13 — convertidos em % dividindo pelo total da linha; cada classe é um sorteio independente, só roda depois que §3 já confirmou infecção):
...
A simetria (Comum ↔ Homebrew espelhados; Pirata especial ↔ Pirata comum espelhados) não é coincidência de planilha — é a MESMA escada de "quanto mais suja a proveniência, pior o payload" de §3, agora aplicada a um segundo eixo. Nenhum peso novo fora da grade numérica (1, 3, 5, 8, 13) foi inventado.
...
**Racional da proposta (`lead-game-designer`, 05/09/2026):** a reanimação ilimitada dos zumbis do Pântano de Markov (`02-selve-sombria.md` §11) faz do `SigKill` a única saída de combate da área, repetida a cada zumbi. Cobrar o teto da faixa comum de um efeito de progresso obrigatório e repetível tensiona contra o princípio que justifica esse teto ("sem estratégia obrigatória, sem vala da morte" vale também para custo de mana, não só para presença de carta). **Essa tensão não fecha sozinha o valor do custo:** quanto o teto pesa de fato depende de quantas vezes por combate o jogador precisa pagá-lo, e isso depende da vida do zumbi — número que não existe em lugar nenhum do projeto (o líder mandou adiar todo balanceamento numérico até haver combate jogável para medir). O mesmo vale para o modificador Stream: acertar vários zumbis de uma vez só pesa mais que a versão padrão se aparecerem vários por combate com frequência que justifique o incremento — outro dado que só a medição traz.
