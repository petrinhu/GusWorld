...
| `Mana` (Compilação) | vazão: ramp linear | `chargeDrawMax = 2 + contagemPropriaDeTurnos`, é a VAZÃO — taxa máxima de saque por turno da bateria ativa, com teto no CCA efetivo da bateria em uso, não um pool do ator (§5, "Estoque e vazão"). Contagem por-ator, substitui o antigo `turnoIndex` global (ver §5) |
...
```
   ...
   │    └──────┬───────┘   recalcula a vazao (mana) do ator eleito: chargeDrawMax = 2 +
...
## 4.2 Rodada (definição canônica, decisão do líder 05/09/2026)

**Decisão do líder, 05/09/2026, por `AskUserQuestion`:** rodada é a volta completa de todos. Uma rodada fecha quando todo ator vivo agiu ao menos uma vez desde o último fechamento. Fecha a lacuna que o T1 (§18.4), o T8 (§18.5) e o Período (§18.3) carregavam desde a reconciliação de 25/08/2026: o relógio de ação (§4) não tem fila nem cursor de rodada, e esses três efeitos precisavam de um "rodada" que não existia mais. A definição abaixo cria um contador PRÓPRIO, derivado do que já existe no relógio, sem reintroduzir fila nem cursor global de turno.

### Mecanismo (determinístico, replayável)

- **O que conta como "agiu":** um ator agiu quando TEVE O PRÓPRIO TURNO, ou seja, quando `CombatBus.TurnStarted(actor, contagemPropriaDeTurnos)` (§16) dispara para ele e o seu `next_action_at` é resetado (§4, "reset pós-ação"); observável pelo `last_acted_at` por-ator já canônico (§16, campo novo do estado). Não é preciso campo novo: o próprio incremento da `contagemPropriaDeTurnos` do ator já É o sinal de "agiu". "Passar" (custo 0, §5) conta como agir: o ator teve o turno e escolheu gastar zero AP, e isso já é consistente com o ramp de vazão, que cresce com a contagem própria de turnos mesmo quando o ator passa.
- **Stun conta como agir, e o ramp de vazão avança durante o Stun (decisão do líder, 05/09/2026, por `AskUserQuestion`):** quando um ator atordoado é eleito pelo relógio, `TurnStarted` dispara normalmente, o ator não pode escolher ação (turno perdido, §9), e o turno encerra. As duas consequências são canon a partir de agora: (1) o ator atordoado CONTA como tendo agido para o fechamento da rodada — a rodada nunca trava esperando quem está travado; (2) a `contagemPropriaDeTurnos` dele AVANÇA, então `chargeDrawMax = 2 + contagemPropriaDeTurnos` (§5) continua crescendo durante o Stun. Razão: Stun tira a AÇÃO, não o tempo; o atordoado não fica pra trás na curva de vazão, e o status não vira dois castigos pelo preço de um.
- **Conjunto de controle:** o motor mantém, por combate, o conjunto `AtoresQueAgiramNaRodada` (Ids de ator). A cada incremento de `contagemPropriaDeTurnos` de um ator (TurnEnd dele, incluindo Stun), o Id entra no conjunto.
- **Fechamento:** a cada entrada no conjunto, compara-se `AtoresQueAgiramNaRodada` contra o conjunto de atores VIVOS presentes no relógio de ação naquele instante. Quando os dois conjuntos coincidem, a rodada fecha: dispara `CombatBus.RoundEnded(roundIndex)`, limpa `AtoresQueAgiramNaRodada` e incrementa `roundIndex`.
- **`roundIndex`:** contador global de rodadas fechadas. Inicia em 1 na `SetupPhase` (a 1ª rodada já está em andamento desde o início do combate) e só anda para a frente. Distinto do `turnoIndex` superado (§2): aquele era cursor de fila por SPD; este só conta voltas completas, para os efeitos que precisam de paridade ou de janela de rodada (T1 e T8 em §18.4/§18.5, Período em §18.3). O relógio de ação em si (§4) continua indiferente a rodada.

### Morte ou incapacitação no meio da rodada

- **Inimigo morto (canon direto, sem inferência):** um ator que chega a HP 0 já é removido do relógio de ação (§3, nota de transição; evento `CombatBus.ActorDefeated`, §16). Ele some do conjunto de "atores vivos" contra o qual o fechamento compara; não precisa "agir" para a rodada fechar. Consequência direta: uma morte pode fechar a rodada corrente no ato, sem esperar por um turno que nunca vai acontecer.
- **Companion incapacitado, não morto (derivação por necessidade lógica, não frase literal do canon):** HP=0 de companion é incapacitação, não morte (`pillars.md` §"Companions — imortais com incapacitação"; `CombatBus.ActorIncapacitated`, evento distinto de `ActorDefeated`, §16). O canon já citado (§3) só afirma "inimigos removidos do relógio" e deixa companions incapacitados sob a guarda-chuva mais genérica "resolvido conforme regra de... incapacitação". Um companion incapacitado não pode agir; por necessidade lógica ele também sai do conjunto de "vivos" esperados para o fechamento (senão a rodada nunca fecharia, travada por alguém que fisicamente não pode ter turno). Se ele voltar a agir na mesma batalha (revive/cura em combate, quando essa mecânica existir), volta a ser esperado a partir da rodada seguinte ao retorno.

### Ator que entra em combate no meio da rodada (não ocorre no escopo canônico atual)

No canon vigente, nenhum ator passa a existir no relógio de ação depois da `SetupPhase`: o `EncounterProfile` fixa `group_size_min`/`group_size_max` em 1/1 nesta onda (`encontros-aleatorios.md`, "Grupos NÃO são implementados nesta onda: o motor lê o dado, e o dado diz 1"), o motor de combate aceita 1 a 4 atores por lado só como capacidade futura (§2, §20 "Composição"), e cartas/inimigos que invocariam ambiente ou reforços ficam explicitamente em fase posterior (§18.1, §18.10, "cartas/inimigos que invocam ambiente = fase posterior"). Logo o conjunto de atores vivos que uma rodada espera é fixado inteiro na `SetupPhase` e só encolhe (por morte ou incapacitação, acima); nunca cresce, nesta fase do jogo. O modificador Object (§8) reforça o mesmo achado por outro ângulo: a entidade persistente que ele cria em campo (armadilha, totem, célula-tática) "tem comportamento próprio", mas o canon nunca declara se essa entidade ganha turno próprio no relógio de ação. Esta seção não escreve regra especulativa para quando isso deixar de ser verdade (reforços, invocações, entidade de Object com turno próprio, `group_size` > 1): a pergunta que fica, para quem chegar depois não precisar redescobri-la, é se o recém-chegado entra JÁ CONTADO em `AtoresQueAgiramNaRodada` (isento, não atrasa o fechamento em curso) ou entra no conjunto de "vivos" que precisam agir (a rodada corrente só fecha depois dele também agir). Vai ao líder quando o escopo abrir para mais de um ator por lado, para reforço, ou para invocação com turno próprio (L-32, sem abstração pro futuro em tarefa cirúrgica).
...
- `chargeDrawMax = 2 + contagemPropriaDeTurnos` é a **vazão**: o quanto o ator PODE sacar
...
> **Granularidade e telegraph (canon 2026-06-03, F2-E.11; fechado em 05/09/2026 pela decisão do líder em §4.2).** "Turnos" do ciclo = **rodadas** no sentido canônico de §4.2 (fecha quando todo ator vivo agiu ao menos uma vez desde o fechamento anterior; contador `roundIndex`). O dia avança 1 fase a cada rodada fechada, no mesmo ritmo independente do tamanho da party (o fechamento de rodada já é, por definição, indiferente a quantos atores existem). **Telegraph = 2 rodadas** (as próprias fases Crepúsculo/Aurora, de 2 rodadas, são a janela de aviso da fase forte seguinte). Parametrizável (`EnvironmentClock`).
...
- **T1 Pavimento Tesselado** (cidade forte; variante na Selve): Criptográfico ×1.3 / Sônico ×0.66. **DUAL, alternando por rodada** (rodada = §4.2): `roundIndex` **ÍMPAR = Branco (lapidado)** → +1 dado de Scan grátis a quem scaneia; `roundIndex` **PAR = Preto (bruto)** → a 1ª carta Cripto da rodada aplica Expose magnitude 13 grátis no alvo. Telegraph: o quadriculado pulsa branco/preto conforme a paridade de `roundIndex`. **Fechado em 05/09/2026 (decisão do líder, §4.2):** a alternância lê o contador `roundIndex` (rodadas fechadas, §4.2), não mais o `turnoIndex` superado.
...
- **T8 Elevação Dominante**: Cinético ×1.3 / Sônico ×0.66. O 1º ator DA PARTY a entrar no conjunto `AtoresQueAgiramNaRodada` (§4.2) na rodada corrente ganha Haste magnitude 1 até o fechamento dessa rodada (`CombatBus.RoundEnded`, §4.2) + revela o intent de +1 inimigo grátis. Premia SPD / Gambito-reordenar (chegar cedo ao próprio `next_action_at`, §4, aumenta a chance de ser esse primeiro). **Fechado em 05/09/2026 (decisão do líder, §4.2):** o gatilho é o primeiro membro da party a completar um turno dentro da rodada, não mais "topo da fila"; reaplica a cada rodada nova (o bônus expira no fechamento, e o 1º da party a agir na rodada seguinte reabre o prêmio).
