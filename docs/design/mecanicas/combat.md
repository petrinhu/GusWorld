...
> `ReorderActor` cru continua existindo só como primitiva de FRONTEIRA (`RegroupStable`), que opera sobre a fila inteira fora de um turno em andamento — nunca em reordenação intra-rodada.

> **Correção de implementação 2 (COMBATE-FILA-CURSOR-FIX, decisão do líder 2026-07-27/28).**
> O portão acima não fechou `RecomputeBySpeed`: ele reordenava a fila INTEIRA por SPD — inclusive a região `[0, cursor)` de quem já agiu — preservando só a IDENTIDADE do `current()` (`cursor = index_of(current)`). Consequência (achado QA, reproduzido dirigindo a FSM de verdade): um ator PENDENTE podia acabar classificado como já-agido e perder o turno, ou um ator JÁ-AGIDO podia voltar pra frente do cursor e agir DUAS VEZES na mesma rodada (com AP/mana recarregados 2x — exploit de combate). `RecomputeBySpeed` agora **preserva a partição**: ordena por SPD dentro de `[0, cursor())` — já-agidos — e dentro de `(cursor(), fim]` — pendentes — de forma independente, e o `current()` (slot do cursor) fica **fixo no próprio lugar**, fora dos dois sorts. Não é um sort inclusivo de `[0, cursor()]` com o cursor recalculado depois por `index_of` — se o current entrasse nesse sort e saísse mais rápido que algum já-agido, o cursor teria que recuar pra acompanhar, reabrindo como "pendente" quem já tinha jogado (a mesma classe de bug, espelhada). Fixar o current no próprio slot é a única construção que preserva os dois invariantes ao mesmo tempo — identidade de `current()` e "cada ator age exatamente 1x por rodada" — pra qualquer posição do cursor, inclusive o caso-limite do cursor no último slot da rodada. O efeito de velocidade aparece na hora pra quem ainda vai agir (a ordem entre os pendentes muda); ninguém age duas vezes nem é pulado.
...
>
> ⚠️ **SUPERADO em 25/08/2026 (decisão do líder; parecer do CTO em `PARECER-ADR-017.md`).** O texto abaixo é o registro histórico de COMBATE-FILA-CURSOR-FIX (2026-07-27/28, ver §4), que descrevia o recompute de Haste/Slow sobre a fila array+cursor do modelo 1B. O `ADR-017` substitui essa estrutura pelo relógio acima; a classe de bug que o texto corrige (repetir ou perder turno) some por construção no relógio (§4). Fica citado como histórico do problema (L-24), não como especificação vigente:
>
> "O recompute dispara SEMPRE no próprio tick do ator afetado (TurnStart ao aplicar, TurnEnd ao expirar) — ou seja, sempre com o cursor apontando pra ele mesmo. Desde COMBATE-FILA-CURSOR-FIX (2026-07-27/28, ver §4), esse recompute PRESERVA A PARTIÇÃO: o ator fica fixo no próprio slot, e a nova SPD só reordena quem já agiu (entre si) e quem falta agir (entre si) — nunca faz alguém repetir turno ou perder o turno na mesma rodada."
> - **Decrypt:** dispela qualquer buff benéfico; SEM lockout (ver linha acima).
> - **Knockback (status consumido one-shot ao disparar, `RemoveStatus`):** no relógio de ação (§4, reconciliado 25/08/2026), soma ticks ao `next_action_at` do alvo afetado — adia quando ele volta a ficar pronto, sem tocar em mais ninguém. Como o instante corrente só anda para a frente (§4), o ator afetado não pode nem "repetir o turno" nem "pular" um vizinho: a classe de bug que a correção abaixo consertava à mão some por construção. Continua distinto de Stun (que tira o próximo turno do ator por completo, não apenas o adia).
>
> ⚠️ **SUPERADO em 25/08/2026 (decisão do líder; parecer do CTO em `PARECER-ADR-017.md`).** O texto abaixo é o registro histórico de COMBATE-FILA-CURSOR-FIX (2026-07-15, ver §4), que descrevia o Knockback como operação sobre a fila array+cursor do modelo 1B. O `ADR-017` substitui essa estrutura pelo relógio acima. Fica citado como histórico do problema (L-24), não como especificação vigente:
>
> "`DelayCurrent(1)` no PRÓPRIO slot do cursor — ADIA o turno do alvo em vez de saltá-lo na fila crua: o vizinho que jogaria em seguida vira `current()` e age primeiro; o alvo (status consumido one-shot, `RemoveStatus`) age logo depois. Cada ator age EXATAMENTE 1x na rodada (comportamento antigo, `ReorderActor(alvo, +1)`, tinha um bug latente: empurrava o alvo pra TRÁS do cursor e o vizinho pendente perdia a rodada inteira — corrigido). Encadeia (2 vizinhos adjacentes com Knockback resolvem os 2 links no mesmo `TurnStart`, loop bounded por `count()`)."
...
- **Statline do Bento "Requiem" Chevalier — HP 55 / Atk 10 / Def 13 / SPD 5, família Cinético** — proposto pelo `lead-game-designer` e **aprovado pelo líder em 2026-08-11** (item `BENTO-STATLINE-COMBATE`). Ele existe porque o Bento é o tanque canônico do grupo e, até aqui, toda a onda de balanceamento (Fases A/A-bis/B/C do PACING-SIM e a spec de Provocar) mediu a defesa do grupo usando a **Jaci como proxy** — o gate G4 da Fase C-bis (o provocador tem de cair MAIS provocando do que numa luta normal) empatava 0%×0% porque a Jaci-proxy não caía em nenhum dos dois cenários. Número a número:
  ...
  - **SPD 5 é o mais lento da party** (canon: cálculo lento, armadura pesada). Fica abaixo do Sentinela-Bit (SPD 8, `//SIM`), então o Bento age depois do trash na ordem bruta de iniciativa.
