...
Não decido entre as duas (L-14). Pergunta formal em §3.
...
3. **EE-9 (§2.9) — "100 inimigos" é contagem cumulativa de eventos ou de entradas únicas?** O Bestiary só tem ~22 espécies catalogáveis (`entries-fichas-bestiary.md`). Se for entrada única, 100 é impossível hoje: ou o catálogo cresce, ou a leitura (A) da §2.9 é a correta e o contador soma reencontros/repetições, não espécies distintas.
...
7. **EE-18 (§2.18) — o efeito (Token-de-Coragem + NPC) repete a cada cena que atingir 10 derrotas, ou é um evento único de campanha?** "Cena específica" no singular sugere um evento único; o propósito de apoio ao jogador sugere um sistema geral. As duas leituras têm consequência de estado diferente (um `id` vs. um conjunto de cenas já atendidas).
