...
**Aplicado em `docs/narrative/diary/entries-fichas-bestiary.md` §5 (item `D32` do `TODO.md`, 05/09/2026):** a estimativa "~22 espécies × 5 estágios ≈ 110" desta seção não se confirmou por medição. O corpo real de §5 cataloga **19 espécies** (5 cidade, 5 Selve, 6 cross-settings, 3 bosses), das quais 17 completam os 5 estágios e 2 (Patch-Zero, Anomalia Glitch) travam no estágio 2 por exceção de glitch já descrita. Isso dá **89 eventos de documentação possíveis no jogo inteiro**, mesmo documentando tudo, abaixo do limiar de 100. Tensão numérica não resolvida, levada ao líder de novo em `entries-fichas-bestiary.md` §5: crescer o catálogo em 2-3 espécies, ou fazer o contador somar também reencontros de estágio além do primeiro. Não decido entre as duas (L-14).
