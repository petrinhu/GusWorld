...
3. **Reocorrência do gatilho após já destravada = no-op idempotente.** Se a condição volta a ser satisfeita depois que o `id` já está no conjunto do save (ex.: o jogador documenta o 101º inimigo, ou morre na mesma cena além do limiar de dificuldade do save, §2.12), nada acontece: inserir um `id` já presente num conjunto não muda o conjunto. É a mesma garantia que o `D17` já exige para o baú (abrir duas vezes não gera duas cartas) — aqui aplicada ao mesmo formato de dado.
...
**Estado inválido:** coberto pela regra 3 do §0 (reocorrência após destravado = no-op). Um estado inválido próprio: se o save for carregado com o contador **maior** que 144 mas o `id` **ainda fora** do conjunto de destravadas (ex.: save migrado de uma versão do jogo anterior à existência desta conquista), a avaliação relê o contador armazenado e destrava no primeiro carregamento seguinte — não é preciso "voltar no tempo" e recontar os eventos, só reavaliar o valor já registrado contra o limiar.

⚠️ **Tensão numérica a levar ao líder, não resolvida aqui:** `docs/narrative/diary/entries-fichas-bestiary.md` (§"Sumário das fichas catalogadas") fixa o total catalogável do Bestiary em **"~22 inimigos turn-based"**. A meta de 144 do EE-9 é mais de 6× esse número. Duas leituras possíveis, e as duas são compatíveis com o texto de EE-9 ("documentou 144 inimigos"), mas dão contadores diferentes:

- **(A) Contagem cumulativa de eventos**, contando toda vez que um inimigo é documentado (incluindo reencontros com o mesmo inimigo já catalogado antes — cada abate ou cada "1º combate" novo soma, mesmo repetindo espécie). Compatível com "144" mesmo havendo só ~22 espécies.
- **(B) Contagem de entradas ÚNICAS completas do catálogo.** Neste caso 144 é **matematicamente impossível** com ~22 espécies catalogáveis, e o número do corpus está errado ou o catálogo precisa crescer.

Não decido entre as duas (L-14). Pergunta formal em §3. O líder escolheu nenhuma das duas leituras: criou o conceito de **documentação multi-estágio por espécie** (avistamento, primeiro combate, análise completa, fraqueza descoberta, item de drop identificado), mantendo o limiar em **144**.

**Aplicado em `docs/narrative/diary/entries-fichas-bestiary.md` §5 (item `D32` do `TODO.md`, 05/09/2026):** o corpo real de §5 cataloga **20 espécies** (5 cidade, 6 Selve, 6 cross-settings, 3 bosses), das quais **14** completam os 5 estágios, **1** (Voz Cromada Sterling, sem loot material) trava no estágio 4, e **5** (Patch-Zero, Anomalia Glitch, exceção de glitch já descrita, mais Sterling Locke Fase 1, Sterling Locke Fase 2 e o Mestre pró-Sterling Asmódico da cena do arco Bento, os três últimos por serem encontros não repetíveis, sem os "3-6 combates adicionais" que o estágio 3 exige) travam no estágio 2. Isso dá **84 eventos de documentação possíveis** com o catálogo medido até aqui (14×5 + 1×4 + 5×2), mesmo documentando tudo, abaixo do limiar de 144.

**Escopo do catálogo e do limiar, decisão do líder em 06/09/2026** (razão completa e nota de escopo em `entries-fichas-bestiary.md` §5, não duplicada aqui): o catálogo do Bestiário cresce até **144 espécies distintas**, do inimigo mais simples ao chefe final, e o limiar desta conquista sobe junto: os dois números são o mesmo, **144**, o termo de Fibonacci seguinte a 89 (100 não pertence à sequência que o projeto já usa em todo o resto do vocabulário numérico: 13, 21, 34, 55, 89, 144). A dica desta conquista já registra isso acima ("Você documentou 144 inimigos"). O teto de 84 acima é o teto do catálogo de hoje, não do jogo terminado, e não se recalcula aqui: as espécies novas ainda não existem. Se o sistema de 5 estágios continua alimentando o contador desta conquista, ou se o contador passa a somar 1 evento por espécie documentada, é pergunta aberta ao líder com as duas opções concretas descritas em `entries-fichas-bestiary.md` §5.
...
**O que o corpus diz** (`comic-reliefs/homenagens-diegeticas.md`, EE-18): "Sistema oculto: dependendo da dificuldade escolhida no save, NPC aparece no save ou hospital" com uma fala de encorajamento, oferece **Token-de-Coragem** e grava uma entrada de Diário. Há também uma mensagem opcional de carregamento. O efeito escala por dificuldade (cura percentual, sem Token no Hardcore) — magnitude completa adiante nesta seção, na linha "Destravável mais de uma vez".
...
**Gatilho de domínio:** a **dificuldade escolhida no save** (L-25, lida do envelope selado). O corte `C-06` (L-29) proíbe qualquer gatilho por desempenho do jogador (mesmo quando o ajuste é uma ajuda ao jogador travado, não uma facilitação disfarçada do lado do inimigo, é o mesmo tipo de gatilho que o corte veta), então a concessão nunca depende de quantas vezes Gus foi derrotado na cena. A escala por dificuldade da cura (tabela abaixo) e a concessão de uma vez por identidade de cena valem como descrito. **Não decidido aqui, e não inventado por presunção de simetria com §2.12:** o instante exato, dentro da cena, em que a dificuldade do save dispara a concessão (ex.: ao entrar na cena, ao primeiro combate dela, outro evento de domínio já nomeado); fica como pergunta nova ao líder.

**Estado mínimo a contar:** a dificuldade do save (já lida do envelope selado, L-25) e o conjunto de identidades de cena que já receberam o Token (para a idempotência por cena, ver "Estado inválido" abaixo). O mapa `id_cena → contagem de derrotas` de §2.12 é próprio daquela conquista e não alimenta esta linha.

**Destravável mais de uma vez:** conforme a regra 2 do §0, a **conquista** (se vier a existir uma) não retrava. O **efeito narrativo** (a fala do NPC, o Token-de-Coragem) — que É explicitamente descrito no corpus — repete a cada cena distinta em que a dificuldade do save dispara a concessão: o Token é concedido **uma vez por identidade de cena**, não uma vez por campanha; magnitude e escopo no bloco de §3, pergunta 7.

**Estado inválido:** mesma regra 3 do §0 para o aspecto conquista; para o aspecto sistema de suporte (entrega de item), a idempotência também deveria valer por cena — reentrar na cena depois de já ter recebido o Token naquela identidade de cena não deveria entregar um segundo.
