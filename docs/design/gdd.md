...
Versão: 0.2 (pré-produção, F1.12-REFRESH 2026-06-02; higienização de stack 2026-06-23). Escopo solo G1, engine própria C++23 sobre GlintFx, PC Linux v1 (Windows pós-v1), single-player.
...
- **Companions com afinidade:** banidos apenas simulador de namoro e medidor de afeto romântico. Afinidade de NPC como estado de reação social está fora da cerca: decisão do líder de 04/08/2026, em `docs/narrative/characters/gus.md`, manda o NPC que insiste em assunto induzido perder afinidade e mudar de estado de diálogo.
...
- **Tradução:** inglês entra no lançamento; conteúdo em pt-br e inglês na 1.0, com a estrutura de internacionalização (de pé desde F2-S.11) já em uso pleno. Ver `B9`/`D19` no `TODO.md`.
- Mod support / in-game editor.
- **Achievements:** o jogo tem conquistas internas; **placar e ranking entre jogadores continuam cortados** (`C-08`, `GODS_LAWS.md`) — o jogo é single-player e offline (`C-01`), um ranking não teria com quem comparar. **O que continua de fora, por ora:** integração com a Steam, por depender do SDK proprietário do Steamworks, incompatível com a AGPL-3.0 herdada do GlintFx; decidida quando a distribuição for real.
- Tutorial wall-of-text / cinemática de abertura > 90s.
- Mais de 8h de campanha (escopo G1 mata).

Crafting (profundo e de cartas) não é corte: ver "Crafting de cartas" em `cartas-technomagik.md` §2.2-2.4 e `deck-mao-sistema.md`. Economia de cidade também não é corte, mas é conteúdo a especificar, não prometido no escopo (`economia.md` §0.3).
