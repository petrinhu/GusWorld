...
TEST_CASE("load_system_settings: diretorio/arquivo AUSENTE devolve defaults "
    ...
    const gus::test_support::ScopedTempDir dir_guard(
        "gusworld_settings_test_ausente", gus::test_support::TempDirCreate::kDeferCreation);
...
TEST_CASE("save_system_settings + load_system_settings: roundtrip real em disco",
          "[settings_file_store]") {
    const gus::test_support::ScopedTempDir dir_guard(
        "gusworld_settings_test_roundtrip", gus::test_support::TempDirCreate::kDeferCreation);
    const std::filesystem::path& dir = dir_guard.path();
...
TEST_CASE("save_system_settings: cria o diretorio com permissao 0700 e o "
          ...
          "[settings_file_store]") {
    const gus::test_support::ScopedTempDir dir_guard(
        "gusworld_settings_test_perms", gus::test_support::TempDirCreate::kDeferCreation);
    const std::filesystem::path& dir = dir_guard.path();
    SystemSettings s;
...
TEST_CASE("load_system_settings: arquivo CORROMPIDO (JSON malformado) degrada "
          ...
          "[settings_file_store]") {
    const gus::test_support::ScopedTempDir dir_guard(
        "gusworld_settings_test_corrompido", gus::test_support::TempDirCreate::kDeferCreation);
    const std::filesystem::path& dir = dir_guard.path();
...
TEST_CASE("save_system_settings: sobrescreve um arquivo existente (mudanca de "
          ...
          "[settings_file_store]") {
    const gus::test_support::ScopedTempDir dir_guard(
        "gusworld_settings_test_sobrescreve", gus::test_support::TempDirCreate::kDeferCreation);
    const std::filesystem::path& dir = dir_guard.path();
    SystemSettings s1;
...
TEST_CASE("settings_file_path: nome do arquivo e settings.json dentro do dir dado",
          "[settings_file_store]") {
    const gus::test_support::ScopedTempDir dir_guard(
        "gusworld_settings_test_path", gus::test_support::TempDirCreate::kDeferCreation);
    const std::filesystem::path& dir = dir_guard.path();
