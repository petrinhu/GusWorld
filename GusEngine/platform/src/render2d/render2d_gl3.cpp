...
struct Render2dGl3::Impl {
    ...
    // Atlas de fonte por face (regular/bold): D2D-2-SENTINELA, cache SOB DEMANDA por
    // (face, cell_px) inteiro - nao mais um unico bake fixo de 16px com downscale na GPU
    // (o defeito original). Ver o mesmo desenho em Render2dSdl::BakedFontSize/FontFace
    // (render2d_sdl.hpp) - duplicado aqui porque este backend nao expoe a struct no
    // header (PImpl: Impl e privado inteiro no .cpp).
    ...
    BakedFontSize* ensure_font(bool bold, int cell_px) {
        ...
        // downscale na GPU, o defeito original).
        ...
        const GLuint tex = upload_rgba(rgba.data(), w, h, /*nearest=*/true);
        if (tex == 0) {
