...
// PATCH GUSWORLD (ADR-009 GL3): stb_image para o LoadTexture (PNG). So o HEADER aqui; a
// IMPLEMENTACAO (STB_IMAGE_IMPLEMENTATION) ja existe uma vez no Render2dSdl/Render2dGl3.
#include "stb_image.h"
...
{
	// PATCH GUSWORLD (ADR-009 GL3): o LoadTexture original do backend so le TGA. Nossos
	// assets sao PNG. Trocamos por stb_image (RGBA8 do buffer em memoria, a mesma lib do
	// Render2dGl3), pre-multiplicamos o alfa e delegamos ao GenerateTexture (que sobe a
	// textura via OpenGL). Sem dependencia de SDL_image nem TGA-only.
	...
	stbi_uc* decoded = stbi_load_from_memory(reinterpret_cast<const stbi_uc*>(buffer.get()),
		static_cast<int>(buffer_size), &w, &h, &channels, 4);
	if (!decoded)
	{
		Rml::Log::Message(Rml::Log::LT_ERROR, "stb_image nao decodificou a textura: %s", source.c_str());
		return false;
	}

	const size_t image_size = static_cast<size_t>(w) * static_cast<size_t>(h) * 4;
	// Pre-multiplica o alfa (o GL3 compoe com premultiplied alpha).
	...
	Rml::TextureHandle handle = GenerateTexture({decoded, image_size}, texture_dimensions);
