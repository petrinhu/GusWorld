...
// (ARTE-RESP-4DIR, 2026-07-23) - anda 1 tick em cada direcao cardinal (South, North,
// East, West), para (dx=dy=0, entra em idle) e despeja 1 PNG por direcao. Nao faz
// parte do jogo; descartavel apos a verificacao.
...
int main() {
    ...
    SDL_Window* window = SDL_CreateWindow("idle_facing_probe", 640, 480, SDL_WINDOW_HIDDEN);
    if (window == nullptr) {
    ...
    const std::string gus_dir =
        gus::app::screens::resolve_sprites_dir(std::string(gus::core::assets::kGusSpritesDir));
    ...
    // parado (dx=dy=0) pra estabilizar no idle/breathing daquela direcao.
    const Case cases[] = {
        {"south", 0, 1},
        {"north", 0, -1},
        {"east", 1, 0},
    ...
    for (const Case& c : cases) {
        ...
        const std::string out_path =
            "/var/tmp/gusworld_idle_facing_" + std::string(c.label) + ".png";
