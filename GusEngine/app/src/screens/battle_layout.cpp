...
// Centraliza N slots empilhados (cada um kActorSlotH de altura, gap kActorSlotGapY)
// na banda vertical [top, bottom]. Devolve o Y do canto SUPERIOR do primeiro slot.
// Se a coluna nao couber (muitos slots), ancora no topo da banda (clamp).
float stacked_top_y(int count, int band_top, int band_bottom) noexcept {
    if (count <= 0) {
        return static_cast<float>(band_top);
    }
    const int total_h =
        count * kActorSlotH + (count - 1) * kActorSlotGapY;
...
ArenaLayout arena_layout(int party_count, int enemy_count,
    ...
    {
        const float top_y =
            stacked_top_y(out.party_count, kArenaTop, kArenaBottom);
        for (int i = 0; i < out.party_count; ++i) {
            const float y =
                top_y + static_cast<float>(i) * (kActorSlotH + kActorSlotGapY);
    ...
    {
        const float top_y =
            stacked_top_y(out.enemy_count, kArenaTop, kArenaBottom);
