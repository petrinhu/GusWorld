...
// Escreve o RML do dialogo (build_npc_dialogue_rml) no stage dir e o retrato do
// speaker_id ATUAL copiado flat (MESMA receita de RETRATO-VIVO do cockpit,
// battle_preview.cpp: copy_file pro stage + referencia por nome flat). GENERICO: o
// arquivo-fonte do retrato vem de npc_dialogue_portrait_file (speaker_id -> nome;
// excecoes cadastradas la, ex. bertoldo) - funciona pra QUALQUER speaker_id, nao so o
// Bertoldo. Se o arquivo-fonte nao existir em disco (asset ausente/headless),
// copy_file falha silenciosamente (ec setado, ignorado) - o glintfx so nao desenha a
// imagem (decorator sem textura valida), MESMA degradacao segura do resto do app/
// (nunca crasha).
//
// FONT-EXTEND-GLITCH (2026-07-29): a fonte NAO e mais injetada aqui via @font-face de
// string - NpcDialogueScreen::enter() registra a familia 1x via
...
// abaixo (load_font_face resolve o path pela MESMA BaseUrlFileInterface que a
...
// write_npc_dialogue_rml_file injetava antes. Chamado 1x em enter() (registro e
// process-wide RmlUi state, ver ui_layer.hpp), DEPOIS do stage ja ter os .ttf (o
// PRIMEIRO write_npc_dialogue_rml_file os copia) e ANTES de ui_->load(). `family`
