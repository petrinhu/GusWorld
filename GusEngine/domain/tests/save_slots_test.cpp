...
//
// REQUISITO DO LIDER (ADR-006 + 2026-06-21): 1 slot de AUTO-SAVE + 5 slots de save
// MANUAL = 6 no total. O auto-save sobrescreve o proprio slot; os 5 manuais o
// jogador escolhe. O C# de referencia (game/scripts/.../SaveManager.cs) usava
// 1 autosave (slot 0) + 4 manuais (1..4); aqui ADAPTAMOS para 1 + 5 (1..5).
//
// O mapeamento slot -> nome logico ("autosave", "save_1".."save_5") e a fronteira
...
//            adaptada de 1+4 para 1+5), ADR-006.
...
// ---- contagem de slots: 1 auto + 5 manuais = 6 ----------------------------

TEST_CASE("slots: politica 1 autosave + 5 manuais (6 no total)",
          "[domain][save][slots]") {
    REQUIRE(kManualSlotCount == 5);
    REQUIRE(kSlotCount == 6);
    REQUIRE(kAutosaveSlot == 0);
}

// ---- validade de slot ------------------------------------------------------

TEST_CASE("slots: slots 0..5 sao validos, fora da faixa nao",
          "[domain][save][slots]") {
    REQUIRE(is_valid_slot(0));
    REQUIRE(is_valid_slot(1));
    REQUIRE(is_valid_slot(5));
    REQUIRE_FALSE(is_valid_slot(-1));
    REQUIRE_FALSE(is_valid_slot(6));
    REQUIRE_FALSE(is_valid_slot(99));
}

// ---- autosave vs manual ----------------------------------------------------

TEST_CASE("slots: so o slot 0 e o auto-save; 1..5 sao manuais",
          "[domain][save][slots]") {
    REQUIRE(is_autosave(0));
    REQUIRE_FALSE(is_autosave(1));
    REQUIRE_FALSE(is_autosave(5));
}
...
TEST_CASE("slots: nomes logicos dos manuais sao save_1..save_5",
          "[domain][save][slots]") {
...
TEST_CASE("slots: nome logico de slot invalido lanca (fail-fast)",
    ...
    REQUIRE_THROWS_AS(slot_logical_name(6), std::out_of_range);
}

TEST_CASE("slots: nomes logicos sao todos distintos (6 nomes unicos)",
          "[domain][save][slots]") {
