...
| F5-BK.AUDIT | F5-Livro | **Verificação profunda canon textual** (pré-tradução). Subagent `revisor-textual` aplicado em todos docs canon publicáveis: drafts BIBLE-V1-* + BIBLE-V2-* + deep-lore (eras + factions + characters + settings + magic + ontologia + antagonists + stinger + antologia) + canon central (lore-bible + timeline + factions + in-world-docs + characters + sinopse + CHARS + PLACES). Sliding window 1000 pal + overlap 20% + checklist 10 categorias (palavras proibidas + nomes trocados + cronologia + ortografia + gramática + coesão + voz autoral + easter eggs + cross-doc + sugestões). Output relatório por arquivo + cascata fix issues CRÍTICOS. Subagent canon em `~/.claude/agents/revisor-textual.md` | Alta | F5-BK.2 | Alta | ⏳ Pendente | — |
