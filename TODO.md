...
| F2-S.MIG.1 | F2-Setup | **Migração GDScript → C# .NET 8 AOT (ADR-002).** Reescrever event_bus.gd → 4 buses (engine/foundation/buses/{GameStateBus,PlayerBus,CombatBus,UIBus}.cs) + localization.gd + md_translation_loader.gd → engine/foundation/localization/{Localization,MdTranslationLoader}.cs. Migrar validate_autoloads.gd → validate_autoloads.cs (game/tools/). Setup .csproj Engine + Game separados. Habilitar Mono em project.godot. CI atualizar com dotnet SDK 8 (image mcr.microsoft.com/dotnet/sdk:8.0). Migrar test_event_bus.gd para xUnit (engine/foundation/buses/tests/). Reativar AutoLoads. Validar AOT export Linux + Windows. Custo estimado 12-24h | Alta | F2-S.MIG.0 | Alta | ⏳ Pendente | — |
| F2-S.MIG | F2-Setup | **Migração GDScript → C# AOT (PARENT task).** Sub-tasks: F2-S.MIG.0 (repo setup ✅) + F2-S.MIG.1 (code migration pending). Cross-ref ADR-002 | Alta | F2-S.MIG.1 | Alta | 🔄 Em andamento | — |
