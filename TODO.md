...
| F2-S.MIG | F2-Setup | **Migração GDScript → C# .NET 8 AOT (ADR-002).** Reescrever event_bus.gd + localization.gd + md_translation_loader.gd em C#. Manter validate_autoloads.gd em GDScript (tool standalone). Setup .csproj + .sln (Godot gera auto). Habilitar Mono em project.godot. CI atualizar com dotnet SDK 8. Migrar test_event_bus.gd para xUnit (.csproj test project). Validar AOT export Linux + Windows. Custo estimado 12-24h | Alta | F2-S.10, ADR-002 | Alta | ⏳ Pendente | — |
