...
| F2-S.TECH-REVIEW | F2-Setup | **Revisar 3 docs tech/*.md sob lens C# AOT (pós-ADR-002):** architecture.md (tooling, build flow, .csproj), engine-modules.md (C# classes em vez de GDScript), build.md (dotnet restore + build + AOT export). Aplicar audit incremental de software-architect anterior + adaptar pra ADR-002 | Alta | F2-S.MIG | Média | ⏳ Pendente | — |
