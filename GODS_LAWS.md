...
**Revogação de 25/08/2026, por decisão do líder após argumentação contra (lei das leis):** o **`C-03`** (sem crafting e sem economia complexa) e o **`C-13`** (cartas nunca craftadas) **foram revogados e apagados desta tabela**, pela L-24. **O GusWorld TEM crafting**, e ele alcança **também as cartas**. A decisão restaura canon do projeto anterior, onde `craft()` existia implementado e testado no sistema de deck (commits `aeaa65ed`, `db84275a`, `d200ae4f`, de julho e agosto de 2026): lá o corte e o crafting coexistiam, e para cá viemos só com o corte. ⚠️ **Confirmação de 28/08/2026, pelo líder:** este corte era **composto**, e a revogação alcança as **duas** metades — o crafting **e a economia complexa**. A pergunta se levantou porque a revogação de 25/08 afirmou só o crafting com todas as letras, e um agente poderia ler a outra metade como viva ou como morta com igual apoio no texto. **Motivo medido, não estimado:** o corpus já especificava **886 linhas** de economia (`docs/design/mecanicas/economia.md`, 476 linhas, e `comidas-ingredientes-craft.md`, 410), com fonte, ralo, cadeia de craft e trade-off — o corte já estava contradito pelo próprio material, como nas outras sete vezes em que corte e corpus divergiram. **Os números `C-03` e `C-13` não são reaproveitados** e a cerca passa a ter **catorze** cortes, do `C-01` ao `C-16` com dois buracos — mesma convenção do `ADR-010` apagado. Renumerar quebraria toda citação existente a `C-08`, `C-15` e irmãos, e por isso é proibido.
**Segunda revogação de 25/08/2026, também após argumentação contra:** o **`C-08`** (sem conquistas e sem placar) foi **revogado pela metade**. **O jogo TEM conquistas.** ⚠️ **Correção de alcance em 28/08/2026, pelo líder:** o corte era **composto**, e a revogação de 25/08 registrou-o como apagado inteiro quando o líder tinha afirmado só a metade das conquistas. **O placar continua cortado**, e por isso o `C-08` **volta à tabela estreitado**, banindo só o placar — não é número reaproveitado, é o mesmo corte com a metade que sobreviveu. **Motivo dado por ele:** o jogo é single-player e offline (`C-01`), um ranking não teria com quem comparar, e a varredura não achou **nenhum** placar especificado no corpus — as oito menções a "placar" são contagem interna de documento de design, do tipo *"PLACAR FINAL: 19 rápidas / 11 lentas"*, e não recurso de jogo. O líder registrou que a decisão é dele e anterior ao arquivamento do projeto anterior; a auditoria dos cortes já havia achado o corpus contradizendo o corte em silêncio, com quatro menções a conquista destravando e banner aparecendo em `docs/narrative/comic-reliefs.md` e sete easter eggs indexados como conquistas. ⚠️ **O que ficou DE FORA, e o motivo, para ninguém reabrir achando que foi esquecimento:** **conquista da Steam não entra agora.** Ela exige o **SDK proprietário do Steamworks**, e a documentação oficial da Valve diz que licença GPL *"can be problematic when combining code with the Steamworks SDK"*. A **L-08** deste projeto é AGPL-3.0-or-later, e não por preferência: ela é **herdada do GlintFx**, porque o executável linkado a ele é obra combinada. Trocar a licença do jogo não resolveria; teria de se trocar a do framework. **Decisão do líder em 25/08/2026: conquista interna agora, integração com a Steam decidida quando a distribuição for real.** Publicar na Steam **sem** o SDK continua possível e não colide com nada.

| # | O corte |
|---|---|
| **C-01** | Sem multiplayer e sem cooperativo. Single-player puro. |
| **C-02** | Sem mundo **aberto** e sem mundo **persistente**. ⚠️ **Segunda correção de alcance, 28/08/2026, pelo líder:** o corte dizia que *"a estrutura é hub central mais incursões radiais"*, e **isso deixou de ser afirmado aqui**. A **forma da conectividade** é do `docs/design/mundo-topologia.md`, que desenha **13 áreas em grafo, com dobradiças** ligando regiões entre si — a Orla Recursiva unindo cidade e Selve, por exemplo. **O documento de topologia vence a frase do corte.** **Motivo dado por ele:** é onde o trabalho de fato aconteceu — o corte tinha uma frase, o documento tem treze áreas desenhadas com ligações nomeadas. ⚠️ **E é a sétima vez, em 25 e 28/08/2026, que corte e corpus divergem — e a sétima em que o CORPUS estava certo.** O que este corte protege continua de pé: **sem mundo aberto, sem persistência**. O que ele deixou de ditar é a **geografia**. ⚠️ **Alcance corrigido em 25/08/2026, pelo líder:** o corte é de **estrutura**, não de **acesso**. A decisão dele de **12/07/2026**, canônica em `docs/design/gdd.md` §7.1, diz que *"o jogador pode tentar chegar a qualquer área desde o início"*, e o gating **nunca é trava dura**. A transcrição de 21/08 recuperou a forma anterior a essa revisão e travou `docs/design/mundo-topologia.md` contra o `gdd.md` por quatro dias. |
| **C-04** | Sem dublagem. ⚠️ **Alcance confirmado em 25/08/2026, pelo líder:** o corte é **amplo** — **sem voz nenhuma no jogo** — e alcança também o voice-over assistivo opcional do Diário, que `docs/narrative/diary/ui-spec.md` classificava como *"UI assistiva, não personagem"*. A distinção entre dublagem de personagem e leitura em voz neutra **não abre exceção**. **Porta que ele deixou aberta, registrada para não se perder:** voz pode voltar como **add-on pós-lançamento**, fora da 1.0 e fora desta cerca. |
...
| **C-08** | Sem placar e sem ranking entre jogadores. ⚠️ **Corte estreitado em 28/08/2026, pelo líder:** a metade das **conquistas caiu** — o jogo TEM conquistas, ver a segunda revogação acima. O que sobrevive deste corte é só o placar. |
| **C-09** | **ALTERADO em 25/08/2026, pelo líder, após argumentação contra (lei das leis): o inglês entra NO LANÇAMENTO.** Conteúdo em **pt-br e inglês** na 1.0; a estrutura de internacionalização, que já estava de pé, deixa de ser só estrutura. **O texto anterior dizia** *"Conteúdo em pt-br; tradução real fica para depois do lançamento"* — e a promessa `G1: pt-br + en` que o `ui-spec.md` já fazia deixa de ser resíduo e vira canon. ⚠️ **Fatura medida e aceita por ele em 25/08/2026:** `en_intl.md` tem 1.650 palavras contra 2.833 de `pt_br.md`, e só o Diário são 54.772 palavras de prosa jogável; o `B9` passa a **nascer bilíngue**, com dois locales no formato selado desde a v1. |
| **C-10** | Sem certificação de console agora. Verificação para Steam Deck fica para depois. |
| **C-11** | Sem **romance**, sem **sistema de moralidade** e sem múltiplos finais. Um final, com variantes mínimas. ⚠️ **Alcance corrigido em 25/08/2026, pelo líder.** **(a) Exceção afetiva, decisão dele de 18/07/2026**, canônica em `docs/narrative/characters/party.md`: *"um afeto platônico infantil, velado e nunca revelado/correspondido em texto, típico da idade, é permitido com sutileza"*. Continua valendo o que o corte já dizia: sem triângulo, sem namoro, sem fan-service. **(b) O placar de comedimento NÃO é sistema de moralidade:** é economia de conduta, e está fora da cerca. A transcrição de 21/08 recuperou a forma anterior à revisão de 18/07 e deixava sob suspeita tanto prosa nova quanto o bônus de clímax. |
...
| **C-14** | Sem conteúdo pago adicional, sem passe de temporada, sem serviço contínuo. ⚠️ **Alcance corrigido em 25/08/2026, pelo líder:** o corte é sobre **pago**. **Expansão gratuita pós-lançamento não atravessa a cerca.** Consequência: o pin do subterrâneo lacrado em `docs/narrative/diary/entries-mapas-timeline.md`, hoje marcado *"possível arco DLC pós-jogo"*, é **renomeado** para tirar a palavra "DLC", que carrega pago no uso comum — a possibilidade fica, **sem compromisso de produzir**. |
| **C-15** | **A campanha tem escopo FECHADO, definido por conteúdo — e a duração NÃO é fixada por número enquanto não houver o que jogar.** ⚠️ **Reescrito em 28/08/2026, por decisão do líder.** O corte dizia *"campanha principal de 4 a 8 horas, mais cerca de 2 horas de puzzle opcional"*. Ele derrubou o número, verbatim: *"o corte C-15 foi coisa sua que escreveu esse tempo de campanha. EU NUNCA iria escolher um tempo tão pequeno com tanto trabalho que estou tendo"*. **A proveniência foi medida antes de reescrever, e sustenta o que ele disse:** o número entrou no commit `81a8ef1`, o de **fundação do repositório**, vindo da transcrição em bloco do grupo `Cortes-G1` do projeto anterior. ⚠️ **Nenhum dos treze cortes tem verbatim do líder — zero.** Todos herdam a mesma atribuição coletiva. E **esse mecanismo já falhou seis vezes em dezesseis**: quatro tiveram alcance corrigido por ele e três foram revogados, todos em 25/08/2026. **Taxa de erro medida da transcrição: 37%**, pela razão já conhecida — ela copiou o retrato do dia em que cada corte nasceu, enquanto o líder seguia revisando no corpus, e as revisões nunca voltaram. **O que a cerca continua protegendo:** escopo infinito. Mecânica nova ainda precisa caber num jogo com fim, e não passa só por ser boa. **O que ela deixou de afirmar:** um número de horas que ninguém mediu, porque **não existe cena jogável para medir** — e foi exatamente assim que o 4 a 8 nasceu. A duração se **mede** quando houver o que jogar, e aí vira decisão dele com dado na mão. |
