...
**Board M0-M9 (migração da engine, ver ROADMAP.md/TODO.md para o board vivo):** M0-M8 entregues — **M7 (paridade jogável) FECHADO 2026-07-21** pelo playthrough ao vivo (Gus): loop completo (andar/NPC/combate→Victory/save/load) rodou 100% na engine C++/SDL3+glintfx sem Godot, zero erro. **M8 (decommission Godot/C#) FECHADO 2026-07-22:** dados vivos migrados pra `resources/` (F1), submódulo `engine/` removido (F2), `game/` apagado — 172 arquivos, 20816 linhas (F3) — e a lixeira física limpa (30M); repo compila e roda sem nenhum bit do stack antigo, CI Windows verde na F1. Tag `pre-m8-godot-legacy` preserva o legado por registro. **M9 (higienização) FECHADO 2026-07-28**, e com ele o board M0-M9 inteiro: executado em 2026-07-22, auditado de forma independente, achado único corrigido e reconferido, aval do líder dado; 3 feedbacks de polish do playtest na INBOX (M7-FB1/2/3). Em paralelo ao board, a onda `CARDS` desenvolve o motor de cartas techMagic ([ADR-016](docs/tech/adr/ADR-016-techmagic-effect-engine-data-driven.md)).
...
**Comunicação entre sessões (autocomm, 2026-07-16):** as 3 sessões Claude do líder (`gusworld` este projeto, `glintfx` a lib de UI, `site` a revista) trocam mensagens por um bus git — repo `petrinhu/gusworld_ia_autocomm`, clone em `~/IDrive/Documentos/projetos_claudebrain/gusworld_ia_autocomm/`. **No início da sessão / quando o líder pedir: `git pull` + ler `inbox/gusworld/` + arquivar as lidas.** Enviar = `.md` em `inbox/<destinatário>/` + push. Detalhe: memória `reference_autocomm` + `PROTOCOL.md` do bus. Repo **PRIVADO** (verificado 2026-07-28: `gh repo view petrinhu/gusworld_ia_autocomm --json isPrivate` → `true`; este texto dizia PÚBLICO desde 2026-07-16, por erro, e o `PROTOCOL.md` do bus já tinha sido corrigido em 2026-07-17). **Zero dado sensível continua valendo igual**, porque a regra é sobre **VERSIONAR**, não sobre a visibilidade: nome de menor e segredo nunca são versionados, em repo nenhum. Repo privado é camada a mais, não substituto da regra. **Regra (líder 2026-07-21): TODA contribuição do Gus (playtest/QA/ideia/edge-case/pergunta técnica) é reportada ao `site` pelo bus (`inbox/site/`) como registro editorial — o beco + o desdobramento técnico —, além de agir nela. Sempre só "Gus Dragon", nunca o nome real. Memória `feedback_contribuicoes_gus_site_bus`.**
...
- **Arquitetura:** 4 camadas dentro de `GusEngine/` — `core/` (POCO genérico: tempo, cripto, player, spatial, animação, utilidades matemáticas, caminhos de asset — NÃO há ECS: party de 7 + turnos não justifica, ver ADR-020) → `domain/` (POCO das regras do jogo: save, i18n, progressão, templates, combate, diálogo) → `platform/` (única fronteira que toca bibliotecas externas: SDL3/GL/RmlUi/miniaudio — janela, render2d, input, áudio, arquivos) → `app/` (telas do jogo, gameplay, ferramentas internas). Regra: cada camada só depende das de baixo, nunca o contrário; gate de CI proíbe violação (inclusive `<glintfx` vazando pra `core`/`domain`).
...
- **Fronteira glintfx ⇄ GusWorld (diretriz do líder 2026-07-21):** repassar ao **glintfx** _todas as funções de framework 2D genérico_ (janela/loop/render2d/input/gamepad/áudio/fonte/asset/math2d/i18n-loader); o GusWorld consome, NÃO reimplementa nem internaliza dep que caberia no glintfx. Régua: **genérico-reutilizável → glintfx (vira PROMPT, não implemento); específico-do-jogo → GusWorld** (`domain/` POCO + `app/`). A regra do jogo (combate/cartas/save-crypto/lore) NUNCA vai pro glintfx (acoplamento reverso). O **gate** dispara no planejamento de cada onda (Caetano), na fronteira `platform/`, ANTES de: (1) FetchContent/vendor novo, (2) código em `platform/` tocando SDL/GL/áudio/input, (3) reimplementar utilitário de engine. Doc canônico: `docs/tech/glintfx-boundary.md`. Passivo conhecido: casca SDL própria → App mode do glintfx.
- **⛔ GLINTFX É A ÚNICA DEPENDÊNCIA DE FRAMEWORK. LEI, canonizada pelo líder em 2026-07-29** (ele precisou repetir "inúmeras vezes" antes de virar texto canônico, o que é falha de registro nossa, não dele). Três proibições, e as três são absolutas:
  1. **PROIBIDO contornar lacuna do glintfx.** Achou lacuna: **PARA a fatia**, sinaliza pelo bus, **AGUARDA** o fix/feature. Não improvisa, não "resolve por enquanto", não deixa as duas formas convivendo.
  2. **PROIBIDO reimplementar em casa** o que caberia no glintfx. Reimplementar é tão ruim quanto contornar: nos dois casos a lacuna deixa de existir para eles, nós carregamos código que não é nosso, e a próxima sessão reimplementa de novo. (Foi por esta regra que `SDL_Log` e `SDL_GetTicksNS` viraram **pedido**, depois de eu ter dito ao glintfx que resolveríamos em casa. Retificado no mesmo dia.)
  3. **OBRIGATÓRIO sinalizar TODAS as necessidades ao glintfx**, não só as que doem agora. O bus é canal **automático**: enviar não pede autorização do líder, só um aviso de uma linha a ele depois. Varrer PII e em-dash antes de enviar segue valendo.
  **Aproveitar cada toque:** ao editar um arquivo por qualquer motivo, **limpar as camadas dele na mesma passada** (ordem do líder, 2026-07-29: *"aproveite cada correção... evitando retrabalhos"*), em **commit separado** do motivo original, para a limpeza ser revertível sozinha. Limite: aproveita-se o que já dá para migrar HOJE; não se abre arquivo novo só para limpar, e não se antecipa fatia bloqueada.
  **Alvo:** zero dependência de framework fora do glintfx. O que sobra legitimamente é a **biblioteca padrão de C++** e o que é **regra do jogo** (`monocypher` no save-crypto, e o domínio POCO). Estado em 2026-07-29: **32 arquivos de produção do `app/` ainda tocam SDL3 direto** — quebra do contrato de camadas, com plano em `docs/tech/plano-camadas-sdl.md` e gate de teto-que-só-cai em `tools/check.sh`.
...
│   ├── core/            # POCO puro: time, crypto, player, spatial, anim, math_util, asset_paths
...
1. ~~**M9 (higienização)**~~ **FECHADO 2026-07-28.** Executado em 2026-07-22 (purga de 31 libs vendorizadas mortas; só `monocypher` e `stb` de pé) e **em verificação**. Auditoria independente de 2026-07-28 deu **PARCIAL**, com um único achado: o `ACKNOWLEDGMENTS.md`, linkado do README público, ainda creditava glm, EnTT, Box2D, fmt e miniaudio como vendorizadas em `third_party/`, libs que o próprio M9 apagou. Achado corrigido no mesmo dia, reconferido em re-auditoria, e o aval do líder foi dado em 2026-07-28: **o board M0-M9 da migração da engine está fechado**.
